...
module LLMLL.CodegenHs
  ...
  , mapLlmllPrimType
  , runtimePreamble
    -- * HTTP-GET-1: the one conditional preamble block, its imports, its
    -- package.yaml entries and the predicate that gates all three. Exported so
    -- the WASI-RT completeness fold in Spec.hs can fold over
    -- runtimePreamble ++ httpGetPreamble, and so the dependency names can be
    -- pinned against compiler/generated-deps.txt.
  , httpGetPreamble
  , httpGetImports
  , httpGetDeps
  , usesHttpGet
...
generateHaskell modName stmts =
  let imports   = [imp | SImport imp <- stmts]
      ...
      -- HTTP-GET-1. One predicate, computed ONCE and threaded to the three
      -- places that must agree: the imports, the preamble splice and the
      -- package.yaml entries. generateHaskellMulti concatenates every module's
      -- statements before calling here, so a call in an imported module is seen.
      httpGet   = usesHttpGet stmts

      libHs     = emitLibHs modName hackageMs httpGet stmts
      mainHs    = if hasMain then Just (emitMainHs modName stmts) else Nothing
      pkgYaml   = emitPackageYaml modName hasMain httpGet hackageMs
...
emitLibHs :: Text -> [Text] -> Bool -> [Statement] -> Text
emitLibHs _modName hackagePkgs httpGet stmts = T.unlines $
  ...
  -- HTTP-GET-1. The imports, the preamble body and the package.yaml entries are
  -- all conditional on the SAME predicate (usesHttpGet, computed once in
  -- generateHaskell). An unconditional import with a conditional dependency
  -- would not compile for every other program.
  (if httpGet then httpGetImports else []) ++
  [ ""
  , "-- ---------------------------------------------------------------------------"
  ...
  runtimePreamble ++
  -- HTTP-GET-1. The one preamble block that is NOT part of the runtimePreamble
  -- CAF, because its body needs http-client, which only a program that calls
  -- wasi.http.get pays for. The WASI-RT completeness fold in Spec.hs folds over
  -- runtimePreamble ++ httpGetPreamble, so the name stays covered.
  (if httpGet then httpGetPreamble else []) ++
  [ ""
  , "-- ---------------------------------------------------------------------------"
  , "-- Program"
  , "-- ---------------------------------------------------------------------------"
...
runtimePreamble =
  ...
  , "-- Out-of-range endpoints CLAMP, matching the out-of-range convention"
  , "-- string_char_at already implements below (return the empty result rather"
  , "-- than index off the end). `from` and `to` are each clamped into [0, n],"
  , "-- and the result is the half-open [from, to) window of what survives; a"
  , "-- transposed pair (to <= from) still yields \"\", as the spec's string"
  , "-- builtin table documents."
  , "-- The clamp on `from` is what keeps the LENGTH right: the unclamped form"
  , "-- `take (to - from) (drop from s)` fed a negative `from` into the take"
  , "-- count while `drop` no-opped on it, so string_slice \"abc\" (-1) 1"
  , "-- returned \"ab\" -- two characters for a one-character window."
  , "string_slice s from to ="
  , "  let n  = length s"
  , "      lo = min n (max 0 from)"
  , "      hi = min n (max 0 to)"
  , "  in if hi <= lo then \"\" else take (hi - lo) (drop lo s)"
  , ""
...
-- ---------------------------------------------------------------------------
-- §13.9 wasi.http.get (HTTP-GET-1) — the one CONDITIONAL preamble block
-- ---------------------------------------------------------------------------

-- | HTTP-GET-1. Does any statement body apply wasi.http.get? Computed ONCE in
-- generateHaskell and threaded to the imports, the preamble splice and the
-- package.yaml entries, which must agree or the generated project does not
-- compile. Bodies only, via stmtExprs: a type, a contract or a pattern cannot
-- hold the call. callsName is total over Expr, so a call nested in a branch or
-- a match arm is found.
usesHttpGet :: [Statement] -> Bool
usesHttpGet = any (any (callsName "wasi.http.get") . stmtExprs)

-- | The four package.yaml entries a fetching program adds. Pinned against
-- compiler/generated-deps.txt (Spec.hs HG-16), which the CI Stack cache key
-- hashes, so the cache learns this group instead of rebuilding it on every run
-- while reporting a hit.
--
-- http-types is already in http-client's closure; it is listed so that
-- `import Network.HTTP.Types.Status` resolves as a DIRECT dependency, which
-- stack requires. filepath is a GHC boot package (takeDirectory).
httpGetDeps :: [Text]
httpGetDeps = ["http-client", "http-client-tls", "http-types", "filepath"]

-- | HTTP-GET-1 imports. renameFile and openBinaryTempFile come from modules the
-- unconditional header already imports (directory, base); they are imported
-- HERE, as a second import of the same module, so a non-fetching program
-- carries no unused import. methodGet is http-types' Method constant: the
-- generated module has no OverloadedStrings, so a bare "GET" would not type.
httpGetImports :: [Text]
httpGetImports =
  [ "import Network.HTTP.Client (newManager, parseRequest, withResponse, responseStatus, responseBody, brRead, method, redirectCount, responseTimeout, responseTimeoutMicro, HttpException)"
...
-- | HTTP-GET-1 (§13.9). The contract is docs/design/http-get-1-proposal.md
-- section 4, clause by clause:
--
--   4.1 scheme: a url not beginning http:// or https:// answers RErr before
--       any request (the parser would also refuse it; two independent guards);
--   4.2 redirects: followed, bounded at 10, the final status decides;
--   4.3 the body streams into a temporary in dest's directory, binary, never
--       decoded, never held whole;
--   4.4 rename onto dest and RNone only when the body reader reached end of
--       stream WITHOUT an exception AND the final status is 2xx;
--   4.5 every other case removes the temporary, leaves dest as it was, and
--       answers RErr prefixed "wasi.http.get: ";
--   4.6 a 60-second whole-transfer budget (responseTimeoutMicro);
--   4.7 certificate validation ON (tlsManagerSettings), no insecure mode;
--   4.8 method GET, set explicitly, because parseRequest honours a leading
--       method word in the string;
--   4.9 dest's parent is not created; a missing parent is RErr.
--
-- llmll_publish_io catches IOException ONLY. http-client throws HttpException,
-- which is not one, so this body catches it itself; without that `try` a
-- refused connection would be an uncaught exception rather than an RErr value,
-- breaking the no-crash property every other wasi body keeps.
--
-- A body that ends before its declared length raises inside brRead, so a
-- truncated 200 never reaches the rename: that is the conjunction in 4.4, and
-- it is the clause that protects the campaign's provenance root (a SHA-256 pin
-- over dest). A rule that renamed on 2xx alone would pin a partial download.
httpGetPreamble :: [Text]
httpGetPreamble =
  [ ""
  , "-- HTTP-GET-1 (§13.9): fetch url to dest as BYTES. RNone on a 2xx with the"
  , "-- complete body renamed onto dest; RErr otherwise with dest UNCHANGED."
  , "wasi_http_get :: String -> String -> IO ()"
  , "wasi_http_get url dest = llmll_publish_io $"
  , "  if not (isPrefixOf \"http://\" url || isPrefixOf \"https://\" url)"
  , "    then return (RErr \"wasi.http.get: unsupported scheme\")"
  , "    else do"
  , "      r <- try (llmll_http_fetch url dest)"
  , "      return (either (\\e -> RErr (\"wasi.http.get: \" ++ show (e :: HttpException))) id r)"
  , ""
  , "llmll_http_fetch :: String -> String -> IO Response"
  , "llmll_http_fetch url dest = do"
  , "  mgr <- newManager tlsManagerSettings"
  , "  req0 <- parseRequest url"
  , "  let req = req0 { method = methodGet"
  , "                 , redirectCount = 10"
  , "                 , responseTimeout = responseTimeoutMicro 60000000 }"
  , "  (tmp, h) <- openBinaryTempFile (takeDirectory dest) \".wasi-http-get\""
  , "  let cleanup = hClose h >> removeFile tmp"
  , "      body = do"
  , "        outcome <- withResponse req mgr $ \\resp -> do"
  , "          let code = statusCode (responseStatus resp)"
  , "              loop = do"
  , "                chunk <- brRead (responseBody resp)"
  , "                if BS.null chunk then return () else BS.hPut h chunk >> loop"
  , "          r <- try loop"
  , "          return (either (\\e -> Left (code, e :: HttpException)) (const (Right code)) r)"
  , "        hClose h"
  , "        case outcome of"
  , "          Left (c, e) -> do"
  , "            removeFile tmp"
  , "            return (RErr (\"wasi.http.get: incomplete transfer (HTTP \" ++ show c ++ \"): \" ++ show e))"
  , "          Right c"
  , "            | c >= 200 && c < 300 -> renameFile tmp dest >> return RNone"
  , "            | otherwise -> do"
  , "                removeFile tmp"
  , "                return (RErr (\"wasi.http.get: HTTP \" ++ show c))"
  -- Clause 4.6, two layers. responseTimeout above covers the connection and
  -- the response headers (http-client applies it to nothing else); this
  -- System.Timeout wrapper covers the whole transfer, body included. When the
  -- wrapper fires, the exception it throws into `body` already ran `cleanup`,
  -- so the Nothing arm must NOT clean again (removeFile on the removed
  -- temporary would raise and reach llmll_publish_io as the wrong RErr).
  --
  -- MEASURED 2026-09-07 (scripts/tests/test_http_get_1.py, the two hung-server
  -- cells), in two runs of the built program. Run one had responseTimeout
  -- ALONE: a listener that never answers, and one that stalls after one body
  -- byte, both held the process past 130 s, so the library's budget covered
  -- neither shape here (the reason was not established). Run two added this
  -- wrapper: both shapes answer RErr at about 60 s. So System.Timeout DOES
  -- interrupt a blocked socket read in the generated program's RTS. That is
  -- the discriminator PROC-TIMEOUT-1 leaves implicit: waitForProcess is a
  -- blocking safe foreign call the non-threaded RTS cannot interrupt, while a
  -- socket read goes through the IO manager and can be. The budget is REAL,
  -- clause 4.6 stands as written, and this wrapper is the layer that delivers
  -- it; do not remove it on the argument that responseTimeout already covers
  -- the header phase, because measured, it did not.
  , "  res <- timeout 60000000 (body `onException` cleanup)"
  , "  case res of"
  , "    Nothing -> return (RErr \"wasi.http.get: budget of 60 seconds exceeded\")"
  , "    Just r  -> return r"
  ]
...
emitPackageYaml modName hasMain httpGet hackagePkgs =
  ...
  -- from 31 to 33 packages. http-client + http-client-tls (HTTP-GET-1) are NOT
  -- in this unconditional list: they move the closure to 75, so they are
  -- emitted further down only for a program that calls wasi.http.get.
  , "  - process"
  , "  - cryptohash-sha256"
  , "  - bytestring"
  ...
  -- HTTP-GET-1. Four entries, emitted ONLY when the program calls wasi.http.get
  -- (usesHttpGet), because they are the one dependency group that is NOT a
  -- compiler dependency and NOT in the CI snapshot cache by default. Measured
  -- 2026-09-07 on lts-22.43: http-client + http-client-tls move a generated
  -- project's closure from 33 to 75 packages (41 new: crypton, tls, four
  -- crypton-x509-*, three asn1-*, network, socks, pem, hourglass, cereal, ...),
  -- a 40-second cold build on Apple silicon. Every program that does not fetch
  -- keeps the 33-package list above, byte for byte (Spec.hs HG-14). The names
  -- are pinned against compiler/generated-deps.txt, which the CI Stack cache
  -- key hashes, so the cache learns this group (Spec.hs HG-16).
  (if httpGet then map ("  - " <>) httpGetDeps else []) ++
