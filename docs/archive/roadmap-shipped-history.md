# LLMLL Compiler Roadmap — Detailed Shipped History

> **Last updated:** 2026-06-21
> **Purpose:** Detailed per-version implementation history split out of the live engineering roadmap ([`../compiler-team-roadmap.md`](../compiler-team-roadmap.md)) under **DOC-CONSOLIDATE M5** (large-cut; see [`../UPDATE-PROTOCOL.md`](../UPDATE-PROTOCOL.md) §Archive policy).
> **Why this file exists:** ~75% of the roadmap was shipped history, which buried the active/next work. The live roadmap now keeps a compact one-line-per-version [Shipped Releases](../compiler-team-roadmap.md#shipped-releases) summary and links each version to its detailed section here. This file holds the full per-item provenance (IDs, acceptance criteria, commit SHAs, dates, design-doc links).
> **Canonical narrative:** [`../../CHANGELOG.md`](../../CHANGELOG.md). When this file and the CHANGELOG disagree on a fact, the CHANGELOG wins.

This is append-only historical detail. Versions are ordered newest-first. Nothing here is deleted on a release pass; new detail is appended at the top when a version's compact summary is demoted from the live roadmap.
...
> **Note on the obligation-report design intent (preserved from the original top-of-roadmap v0.10 section, deduped here).** The primary artifact is a structured JSON obligation report for every hole or failed verification step, combining three independent feedback channels into a single agent loop — type obligations (from the type checker / `--sketch`), contract obligations (from the verifier / liquid-fixpoint), and trust obligations (from the trust report / evidence model). Indexed types (`Vect n a`, GADTs, dependent elimination) were explicitly **not** in scope for v0.10; the release achieves ~80% of the agent-facing benefit of Idris-style development through richer obligation reporting, without changes to Algorithm W. The Tier-1 benchmark gate was `withdraw`/`double` (arithmetic candidates) + `safe-first` (branch obligations); Tier-2 (`clamp`/`abs` conditional synthesis) gated the deferred OBLIG-4 extension. Path conditions are re-exported from v0.9's `bodyToPred` `FlatPath` guards; for hole-bearing functions a presentation-only guard-walker in `ObligationAssembly.hs` collects accumulated path conditions per OBLIG-0 spec §4.2.3.
...
Binary test: do `effectful`'s C shims compile under `wasm32-wasi`? Result: **GO** — no C shims, no linker errors, correct execution. See [`wasm-investigations/effectful-wasm-spike.md`](wasm-investigations/effectful-wasm-spike.md).
...
> **Source:** [lead-agent.md](shipped-design-specs/lead-agent.md)
...
> **Source:** [agent-prompt-semantics-gap.md §1](shipped-design-specs/agent-prompt-semantics-gap.md) — parametricity gap
>
...
> **Source:** [`agent-prompt-semantics-gap.md §4 Option C`](shipped-design-specs/agent-prompt-semantics-gap.md)
...
> **Source:** [invariant-discovery.md §6](shipped-design-specs/invariant-discovery.md) *(originally `docs/design/invariant-discovery.md`; archived to `shipped-design-specs/` post-DOC-CONSOLIDATE)*
...
> **Source:** [`agent-prompt-semantics-gap.md §4 Option B`](shipped-design-specs/agent-prompt-semantics-gap.md)
...
> **Source:** [`agent-prompt-semantics-gap.md`](shipped-design-specs/agent-prompt-semantics-gap.md) — reviewed and approved by Language Team and Professor.
...
**Open questions (from [`agent-orchestration.md`](../design/agent-orchestration.md) and [`agent-prompt-semantics-gap.md`](shipped-design-specs/agent-prompt-semantics-gap.md)):**
...
> **Source:** [`verification-debate-action-items.md`](shipped-design-specs/verification-debate-action-items.md) — items surfaced by external formal methods review.
...
- **Result:** Conditional GO — see [`wasm-investigations/wasm-poc-report.md`](wasm-investigations/wasm-poc-report.md). ~6-7 days engineering for v0.4.
...
> **Note:** The `?delegate` checkout/patch *compiler primitives* (`Checkout.hs`, `PatchApply.hs`, `JsonPointer.hs`, `llmll checkout`, `llmll patch`) shipped in v0.3. The agent orchestrator (`llmll-orchestra`) is scoped separately — see [`agent-orchestration.md`](../design/agent-orchestration.md).
...
## Appendix — Summary: Version Plan and Critical Path (historical)
...
### v0.8–v0.10 critical-path diagram (historical)
...
### Per-version result digests (v0.7 → v0.10.8)
...
**v0.10.6 patch shipped (2026-05-14):** OBLIG-PBT-4 `:subject` / `:subjects` keyword metadata on `(check ...)` blocks (`cb2e71f`) + F-033 body-side static-eval coverage extension (`unwrap` clause in `Contracts.hs:evalBuiltinApp`, `IORef`-threaded body-discard counter through `runQC`, refined `gaveUpDiag`) + F-034 residual `evalBuiltinApp` coverage (five missing clauses `list-empty`/`list-prepend`/`list-filter`/`int-to-string`/`string-concat-many`; correctness fix on `list-head`/`list-tail` Success-wrapping). `LLMLL.md §4.4.5` PBT-Lift rule extended with the annotated-subject premise (`PBT-Lift-Annotated`). JSON-AST `schemaVersion` bumped `0.4.0 → 0.5.0` additively for `CheckDecl.subjects`. 640 Haskell + 37 Python tests. No milestone advancement. See `CHANGELOG.md` v0.10.6.

**v0.10.5 patch shipped (2026-05-13):** PBT complex-type generators + static-evaluator extensions (`262e1a2` — closes F-032) + PBT-to-trust-report write-back (`fb236c9` — `PBTPassed` lifts post clause of the singleton head-position contracted callee to `DLTested n`; `pbt_witnesses` SHA-256 hashes persisted in `.verified.json`; `trust_report_version` 1.0.0 → 1.1.0 additive; PBT-Lift rule formalised in `LLMLL.md §4.4.5`; closes F-033). 614 Haskell + 37 Python tests. No milestone advancement. See `CHANGELOG.md` v0.10.5.

**v0.10.4 patch shipped (2026-05-13):** R6d trust-report tier-count profile aggregate (`bb1bd98` — `TierProfile` + `aggregateTiers` in `TrustReport.hs`; new `docs/llmll-trust-report.schema.json` versioned independently at `trust_report_version: "1.0.0"`) + `LLMLL.md §4.4.4` spec note (`bbab67b`) + repair-loop harness Cred + H1 bifurcation (`90a5bb9` — closes §LT-A / F-026 / F-027) + repair-loop matrix runner (`5895792`). 594 Haskell + 37 Python tests. No milestone advancement. See `CHANGELOG.md` v0.10.4.

**v0.10.3 patch shipped (2026-05-12):** cross-module PBT visibility (MOD-PBT-1 / F-018 — `(open ...)`-targeted `def-logic` now reaches the PBT FuncEnv) + spec pedagogy corrections (§2.5 Naming Conventions, §3.3 / §9 / §13.5 match-arm canonical form, §3.2 / §3.3 unit-payload vs nullary constructor) + roadmap disambiguation of the one-shot correctness criterion. 589 Haskell + 37 Python tests. No milestone advancement. See `CHANGELOG.md` v0.10.3.
...
**v0.10.1 result (patch):** `llmll version` command + `--version` flag. Exit code fixes: `check`/`holes` rc=1 on parse errors, `--help` rc=0 on all 17 subcommands. Structural + transitive `expandAlias` with cycle guard. `compatibleExpanded` helper replaces 13 `compatibleWith` call sites. Unsound `TCustom`/`TSumType` bridge removed. `?delegate-async` normalization. `DelegationError` parse-time normalization. ADT constructor auto-registration with collision detection. `withFunctionContext` combinator. macOS build warning suppression. 570 Haskell + 37 Python tests (+14 alias resolution tests).

**v0.10 result:** All 8 items shipped (2026-05-03). OBLIG-0 ✅ (design spec Rev 8), MOD-1 ✅, OBLIG-1 ✅ (enriched CheckoutToken — 7 new fields), OBLIG-2 ✅ (ObligationAssembly.hs — 800+ lines, --obligation-report flag), OBLIG-3 ✅, OBLIG-4 ✅ (arithmetic repair suggestions, cap-8), OBLIG-5 ✅, OBLIG-B ✅ (3 benchmark programs, 11 golden tests). GuardClassifier.hs extracted from FixpointEmit.hs. 3 bugs fixed (F6, F7, R2). 556 Haskell + 37 Python tests.

**v0.9.1 result (module system hardening):** Professor's review audit (2026-05-01). Spec restructured (§8.3 ordering, §8.5 namespace resolution, §8.6 open semantics, §8.7 export scope). Cycle detection fixed (`Set` → list stack + visit-order slicing). `life_sexp` example corrected. `ModuleEnv` TODO annotated. 11 new module system tests (M-01–M-07). 474 Haskell + 37 Python tests. 4 items deferred: MOD-1 (v0.10), MOD-2/3/4/5 (future).

**v0.9 result:** All 8 items shipped (2026-05-01). COMP-0 ✅ → COMP-6 ✅, COMP-T ✅ (18 golden tests). 4 source files + test suite updated. 452 Haskell + 37 Python tests.

**v0.8.1b result:** All 10 items shipped (commit `bf98797`, 2026-05-01). EVID-0 ✅ → EVID-T ✅. 14 source files + test suite updated. Hard break: no backward compat for old `.verified.json`. 322 Haskell + 37 Python tests.
...
**v0.8.0 result:** All 10 items shipped. BODY-VC-0 ✅ → BODY-VC-3 ✅. BODY-VC-T (25 tests). EOp soundness fix and clause-level emission tracking closed two critical soundness gaps. SUPP-DEBT, EVENT-LOG, SPEC-FOUNDATION, SPEC-EFFECTS, SPEC-TRUST all shipped. 320 Haskell + 37 Python tests.
...
### What Changed from LLMLL.md §14 (historical)
...
| **v0.8.0** | *(new, 2026-04-28)* | **Faithfulness Core:** BODY-VC + SUPP-DEBT + EVENT-LOG + SPEC-FOUNDATION. No external blockers. — **shipped (2026-04-29)**. |
| **v0.8.1a** | *(new, 2026-04-30)* | **Documentation Boundary Clarity** — docs only. — **shipped (2026-04-30)**. |
| **v0.8.1b** | *(new, 2026-04-30)* | **Evidence Model Refactor:** Four-tier `DisplayLevel` partial order replaces `VerificationLevel` total order. Hard break for `.verified.json`. 322 tests. — **shipped (2026-05-01)**. |
| **v0.9** | *(shipped, 2026-05-01)* | **Compositional Verification:** Assume-guarantee `EApp` encoding. `EMatch` on `Result`. SCC recursive fallback. `--strict-verified-core` mode. 452 tests. **v0.9.1:** Module system hardening. 474 tests. — **shipped (2026-05-01)**. |
| **v0.10** | *(shipped, 2026-05-03)* | **Obligation-Guided Agent Coding:** Structured obligation reports (JSON). Three channels: type, contract, trust. `EMatch` branch obligations. Repair suggestions. Benchmark suite (B1/B3/B5). 556 tests. — **shipped (2026-05-03)**. |
...
### Items Removed from Scope (historical)
