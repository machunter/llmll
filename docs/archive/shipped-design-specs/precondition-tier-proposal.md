...
> **Status:** **BUILT & COMPLETE — green (893 tests), firsthand-confirmed (2026-06-19), uncommitted on branch `feat/composition-admit-verified`.** Position B end-to-end: classifiers + `effective_level` + the transitive callee meet (`calleeMinLevel`) all read post-side (`teEffectiveLevel` converged onto `teEffectivePostLevel`); first-class persisted `caller_obligations` axis + `carries_caller_obligations` boolean; `trust_report_version 1.4.0`. **Callee-meet completion (open-Q1 resolved):** a callee's pre is the caller's discharged-at-call-site obligation, not an inherited floor → a composer that discharges + proves its post reaches `verified` (`safe-withdraw` confirmed verified, was asserted). Four soundness checks PASSED (`refutedClosure`, `--strict-verified-core`, `callee_tier`→post-side, `teEffectiveLevel` convergence). Escaped-obligation propagation non-strict-core-only (TP-PRE-6). `tier_profile_pre` still honestly shows asserted pres. **FORCES the demo re-script** (withdraw-demo `verified: 2 → 3`; runbook/DemoPost stale on branch — re-script in progress). Conjunct-(d) still unenforced (B doesn't need it; separate item). See "Rev 3" below.
