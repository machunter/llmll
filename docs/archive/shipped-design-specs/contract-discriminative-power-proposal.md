...
> **Version:** Rev 5 — §8 gate Outcome 0 recorded; CE follow-up complete
> **Date:** 2026-05-23 (Rev 1); 2026-05-25 (Rev 2); 2026-05-27 (Rev 3); 2026-05-27 (Rev 4); 2026-05-29 (Rev 5)
...
> **Reviewed:** Professor review at [`contract-discriminative-power-review.md`](../professor-reviews/contract-discriminative-power-review.md) (Rev 1, 2026-05-25); recommendation `approve with revisions`. Seven gaps and two author-question answers folded into this Rev 2. Standalone review awaits doc-lead M2 fold-and-archive.
> **Status:** Settled (Rev 5) — CE follow-up complete (commit `3af3c06`, 2026-05-29): `WarnSpecTooTightForOmega` rename + `CDPScopeCoreOnly` default shipped; §8 gate Outcome 0 recorded; pending doc-lead promotion
...
- Introduce the optional `(spec-entropy :strict | :intentional | :unknown)` annotation honoring the healthy-diversity-vs-underspecification tension at [`docs/design/invariant-discovery-review.md §4.1:267-279`](../professor-reviews/invariant-discovery-review.md)
...
- **Behavior-equivalence semantic decision procedure.** The metric reports observational equivalence over the observation set `Ω`; a true semantic-equivalence check requires the richer fragment that lives outside QF-LIA. See Risk #1 below and [`invariant-discovery-review.md §4.2`](../professor-reviews/invariant-discovery-review.md).
- **LLM-generated candidate enumeration.** v0.11 extends the trivial-body enumeration to type-compatible candidates over the existing prelude; v0.12+ widens to LLM-generated candidates per the Phase B item at [`invariant-discovery-review.md §5`](../professor-reviews/invariant-discovery-review.md).
...
- **Transitive CDP scope (cross-module measurement).** `computeCDPFor` measures only the entry module's contracted functions in v0.11; [`compiler/src/LLMLL/CDP.hs:computeCDPFor`](../../compiler/src/LLMLL/CDP.hs) iterates the entry-module statement list and does not walk the module cache. Entry-module-only is the explicit conservative default: it avoids double-measuring a function that appears as an entry-module function under one fixture and as a cross-module import under another, and keeps `computeCDPFor` stateless with respect to module-cache topology. Transitive scope — measuring contracted functions in the module cache alongside the entry module — is a v0.12+ widening, contingent on (i) a canonical module-qualified function name as the cross-fixture stable key and (ii) an explicit `--cdp-scope=transitive` opt-in at the tool boundary so scope changes are never silent between runs.
...
- **Cross-proposal shipping under LT-INV gate outcomes.** Per [`v0.11-cross-proposal-rollback-discipline.md`](v0.11-cross-proposal-rollback-discipline.md) §2: Outcome 0 (gate passes) ships LT-CDP as proposed; Outcome 1 (LT-INV opt-in-only) reports `discriminative_axis` only under the `--grammar=core-inversion` flag; Outcome 2 (LT-INV retracted) ships LT-CDP against `def-logic` with the body-faithful set as implicit scope. Schema bump under Outcomes 0 and 1 is `trust_report_version 1.1.0 → 1.2.0` (additive); Outcome 2 ships independently and coordinates with LT-PPR per the C-2 settlement. **Gate adjudicated Outcome 0 (§8 PASSED 2026-05-28; postmortem-004 at [`experiments/minimal-agent/findings/postmortem-004-s8-gate-post-arm-rerun.md`](../../../experiments/minimal-agent/findings/postmortem-004-s8-gate-post-arm-rerun.md); language-team adjudication commit `5cab1b7`).** The CDP-0 baseline run `20260527T154040Z` (`cdp-discriminating-weak`, 11/26 defined, 4 midrange; see [`experiments/cdp-0/findings.md §F-004`](../../../experiments/cdp-0/findings.md)) established the pre-gate DP distribution used in the gate measurement; it is the *baseline*, not the gate outcome. The §8 gate ran against post-arm data in postmortem-004: axis (d) 10/10 `def`/`def-shell` across five passing solutions, 0 `def-logic`, in-session adaptation confirmed. `CDPScope` default flipped to `CDPScopeCoreOnly` (CE patch commit `3af3c06`, 2026-05-29).
- **CDP-0 measurement scope and `defined_fraction` denominator (Rev 3, F-007 resolution).** CDP-0 measurement scope is entry-module contracted functions; `defined_fraction` is computed over entry-module functions only. Cross-module imports appearing as `not-requested` in the trust report are excluded from the denominator. The rationale: `computeCDPFor` iterates entry-module `stmts` by design; including cross-module `not-requested` entries would penalize fixtures whose entry modules import heavily from a shared module cache — a property of module architecture, not of measurement quality. The current compiler implementation (`compiler/src/LLMLL/CDP.hs:computeCDPFor`) already behaves per this policy; this note makes the policy explicit for harness consumers comparing `defined_fraction` across corpus shapes. **Pre/post §8 comparability constraint:** both the pre-inversion baseline and the post-inversion gate measurement must use the same scope (entry-module only in v0.11); mixing scope disciplines between runs would invalidate the pre/post comparison. The `cdp_scope` field in each run's `baseline.json` records the operative scope for auditability.

---

## 3. Surface

LT-CDP introduces no new keyword. The optional `(spec-entropy ...)` annotation per memo §2 and per [`invariant-discovery-review.md §4.1`](../professor-reviews/invariant-discovery-review.md)'s required-mitigation note:
...
Per [`critique-2026-05-23-triage.md`](../../design/critique-2026-05-23-triage.md) §3.2, the formal anchor is:
...
The v0.11 trivial-body extension consists of the following candidates, evaluated per contract per emitFixpoint pass. The enumeration is *closed* for v0.11; v0.12+ widens to LLM-generated candidates per [`docs/design/invariant-discovery-review.md §5`](../professor-reviews/invariant-discovery-review.md).
...
The trust-report JSON per [`docs/design/critique-2026-05-23-triage.md`](../../design/critique-2026-05-23-triage.md) §3.2 gains the CDP field per function. The current per-function entry shape (post-OBLIG-PBT-3 at v1.1.0) gains a parallel `discriminative_axis` block alongside the existing evidence fields:
...
- **`warnings`** — typed diagnostic strings using the enumeration below (Rev 2, per the professor review's Gap #7). The Rev 2 enumeration distinguishes *why* DP is not reported as a numeric score; downstream consumers must distinguish "DP undefined (enumeration too small)" from "DP not measured (def-shell out of v0.11 scope)" to avoid conflating non-applicability with measurement weakness:
  ...
  - `"spec-too-tight-for-omega"` — no candidate satisfies the contract, but the post-condition carries DLVerified or DLContractChecked evidence: the spec is provably correct and tight with respect to the §4.3.1 trivial-body enumeration. `score` is suppressed. Consumers should treat this as a strong-spec signal, not a defect. Emitted only when `--trust-report` is active; without it, `"spec-inconsistent"` is used conservatively and a notice is written to stderr. *(Renamed from `"vacuous-over-omega"` per F-005 adjudication at Rev 4 below; the compiler-side constructor is `WarnSpecTooTightForOmega`.)*
...
**F-005 scope-policy adjudication (2026-05-27, Rev 4).** Option B adopted. The `warnings` field string for the `WarnVacuousOverOmega` condition — `|⟦S⟧_Ω| = 0` on a function carrying DLVerified or DLContractChecked evidence — is renamed from `"vacuous-over-omega"` to `"spec-too-tight-for-omega"`: the rename communicates the diagnostic's intent (the specification is tight enough that the §4.3.1 trivial-body enumeration cannot satisfy it) rather than its observational mechanics (a vacuous denotation over `Ω`), which is correct but non-communicative as a user-facing label. `"spec-inconsistent"` is reserved for the distinct, rarer case where the `pre`/`post` conjunction is *semantically* unsatisfiable — detectable without enumeration and independent of `Ω` (e.g., a solver UNSAT on the pre+post combination), which is a contract bug rather than a tight-spec signal. A compiler-engineer follow-up patch renames `WarnVacuousOverOmega` to `WarnSpecTooTightForOmega` in [`compiler/src/LLMLL/WeaknessCheck.hs`](../../compiler/src/LLMLL/WeaknessCheck.hs) to align the constructor name with the settled user-facing label; no other compiler change is required.
