...
> **Version:** Rev 2 — professor review folded (coherence question settled; monotonicity claim corrected to two named properties; the post-clause tier interaction resolved inward via the §3.4.1 introduction/elimination split + §3.4.5 firewall). Rev 1 (2026-06-20) was the initial drift-closure proposal.
...
> **Status:** Settled (Rev 2) — code-track; awaiting `compiler-engineer` hand-off. The `checkExpr`-at-return prerequisite is built; the surface (parsers + schema) and the `FixpointEmit` return-refinement join are the remaining build.
