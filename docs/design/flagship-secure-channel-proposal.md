...
**Fix shape:** an **A-normalization pass over the body AST** (desugar non-tail calls into `let`s) before body-VC emission, reusing the existing `let` + callee-post machinery. Confirm the fallback site in `FixpointEmit`/`bodyToPredM`. Bounded (~1 day + version bump).
