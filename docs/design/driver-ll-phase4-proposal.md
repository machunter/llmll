...
> **MEASUREMENT EPOCH, added at Rev 8. Every count in this section was measured against
> `aa08051~1` and none of them holds at HEAD.** The section states its numbers in the present
> tense and the tree moved underneath them, which is a defect in this document rather than
> drift in the reference: **the repair that moved them is the one this section ordered.** Task #8
> (`aa08051`, shipped v0.14.84) introduced the second raising form, so the sites that were
> `require()` calls recording `stopped` are now `require_spec()` calls. Measured: **46 call sites
> before, 39 after**, and the raise count rose correspondingly. The partition below ("of the 46:
> 9 spec-defined, 26 …") is therefore over an encoding that no longer exists.
>
> **The argument is unaffected and the arithmetic is not.** The classification rule, its polarity
> guard, and the nine spec-defined clauses are all statements about clauses, which the repair
> re-encoded rather than renumbered. Sub-phase 4a does not read these counts. **Sub-phase 4b
> does**, and it must re-measure before using them. A count stated without its epoch reads as
> current, which is the same failure this line recorded against version numbers on 2026-08-05
> and wrote into `docs/UPDATE-PROTOCOL.md`.
