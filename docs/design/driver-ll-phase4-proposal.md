...
**Amended at Rev 11, from the port: the tolerance is at three sites, and the third is over a
different artifact.** F-20 named `:713` (stage F) and `:732` (stage G), both indexing `core_ids` on
`core.json`. `:482`, inside `check_dispositioned` itself, is a third occurrence of the same shape:
`rows: Any = doc["rows"] if isinstance(doc, dict) and "rows" in doc else doc`. **The generalization
is the shape, not the artifact**, and stating it as "F-20's tolerance" without that distinction
invites a reader to think `:482` also concerns `core.json`; it concerns the dispositions document,
whose producer is a different agent invocation. The narrowing must therefore cover all three or one
arm silently keeps the tolerance, and the n=0-live-runs null result covers `core.json`'s two sites
only. Whether a live agent emits a bare-list dispositions document is a **separate** unmeasured
question with the same shape and the same loud-on-surprise disposition.
