...
status: "Rev 18, 2026-09-12. SUB-PHASE 4e READ driver-spec SECTION 8 WRONGLY, AND THIS PROPOSAL'S OWN SECTION 5 REFUTED IT BEFORE SECTION 8 DID. 4e settled that the checkout brief is the agent's only input, calling that the sole-channel discipline; `wave.llmll`'s header and cover cell W1 pin it. Section 8 does not say that. Its second paragraph forbids three things by name: another agent's directory, another agent's output, and a worked answer. A rendered task statement is none of the three. Its THIRD paragraph settles the term by definition rather than by inference, which is the reading the professor supplied and this proposal had not used: the driver MUST be able to report any file in an agent's directory that was not among ITS DECLARED INPUTS. So a declared input is a driver-side commitment subject to audit, and not a spec-side enumeration. A rendered prompt is a declared input because the driver declares it. THE INTERNAL REFUTATION IS TIGHTER AND CAME FIRST. Section 5 item 1 settled `--agent-exe` plus repeatable `--agent-arg`, with `{prompt}`, `{out}` and `{workdir}` SUBSTITUTED PER ARGUMENT. 4e built a stage that substitutes nothing and appends two paths instead, so stage M was built outside this proposal's own agent-invocation contract and no reading of driver-spec is needed to find the defect. MEASURED, not argued: with the campaign's real argument vector the wave hands the agent `--model`, `claude-opus-5`, the literal string `{prompt}`, the literal string `{out}`, the brief path and the body path; the committed wrapper exits 2 on the unreadable literal before it reaches a model. THE SCOPE IS WIDER THAN A MISSING PROMPT, which is how the first draft of this amendment framed it. Section 8 names TWO structural mechanisms and the port has NEITHER: no pristine copy for the agent to check its own work against, and no isolation audit, which is `FS-ISOLATION-1`. That row changes content below. The pristine copy is the sharper half, because `FS-COPY-1` in section 14 shipped `wasi.fs.copy` CITING driver-spec section 8 lines 336-337 for that clause, and 4e then used the builtin for its backup and restore and never for the agent's copy. THE PORT CANNOT BE STRUCTURALLY STRONGER THAN THE REFERENCE, and the professor's open question assumed it could. `wasi.proc.run` takes an executable, an argument vector, a working directory, three handle paths and a timeout. It takes no confinement parameter, and `LLMLL.md`'s effect summary names it an opaque boundary at the top element, because it runs an arbitrary program and can reach anything the catalog names and more. The capability clause bounds the parent and says nothing about the child. So the port reproduces the reference's posture exactly and files the gap; a confining spawn is a language change, and `theory-questions.md` Q-008 asks what it would cost. ONE ARGUMENT IN THE FIRST DRAFT IS WITHDRAWN. It justified the repair partly by the retry-budget comparison class, and the clause 2 pre-registration sections 4.3, 4.4 and 7 make that class agent-determined, reported rather than thresholded, and null on divergence. Repairing stage M makes that class readable and cannot make it informative. WHAT REV 18 DOES NOT MOVE: no proved core, the serial-wave decision, the Rev 15 contention finding, and no clause of section 2.4. Every function involved is `def-shell` and emits no verification condition. Rev 17, SETTLED. REV 17 IS ONE BULLET AND CORRECTS AN AMBIGUITY, NOT AN ERROR: section 2.4 thresholded item 1 said terminal state over all fifteen stages, and the two counts in play differ while both are correct. driver-spec says run fifteen stages in order and rfc-swarm README lists them as A through O with no G2; the implementations dispatch SIXTEEN, the fifteen plus G2 the artifact audit, whose kind is gate. MEASURED: registry.llmll stage-count returns 16 and the reference defines 16 stage_ functions. The bullet now binds to stage-count, because a comparator iterating the specification fifteen would skip a gate. No other clause moves. REV 16 MADE TWO CORRECTIONS AND ADDS NO NEW DESIGN. FIRST, SECTION 2.4 IS AMENDED: clause 2 is LIVE, and reading it as a replay of committed artifacts deletes its content, because clauses 1a and 1b already exercise all fifteen stages hermetically over a stub agent and a stub llmll (section 2.3). Rev 15 then required agreement across six decision classes, and agreement is UNOBTAINABLE for five of them even when the port is exact: gate J computes its verdict from the inventory the agent produced and stage L from the frozen set the agent produced, so a live run that re-samples the agent compares incomparable inputs. Section 2.4 now splits a THRESHOLDED set (terminal state over fifteen stages, zero FFI, bounded authority, every halt naming its clause on its output, no complete over a shape the reference rejects, and no stopped-for-failed in either direction) from a REPORTED set (all six classes, recorded with n, divergence a finding rather than a failure), reusing the campaign idiom reported-NOT-thresholded from experiments/rfc-swarm/PRE-REGISTRATION.md. Stage status leaves the thresholded set because clause 1a owns it, which is why no committed run carrying no MANIFEST.json is not a gap. The target is fixed as RFC 826: TFTP is disqualified by its own RESULTS.md as hand-authored except for the fill wave, rfc4648 stopped at gate J, and rfc826 ran all fifteen stages with no human authoring. SECOND, section 9 corrects TWO cells that read none new: 4d activated five callerless oracle and shape cores, and 4f landed report.omission-free? which is both new and activated. Rev 15 folds the sub-phase 4e harness leg, three MEASUREMENTS against compiler v0.14.87, and the first of them refutes this document's own injection design. F-27: SECTION 10 CASE 1'S PREMISE IS REFUTED AND THE STUB IS RETIRED. The premise conflated two propositions this document treated as one, (P1) the wave is serial and (P2) `_apply` releases its token and re-checkouts under the lock immediately before patching, and CONTENTION-UNREACHABILITY FOLLOWS FROM P2 ALONE. P1 without P2 produces contention and P2 without P1 avoids it, which is what `_apply` is built for at N agents. The CAS is per-file against the brief's source_hash, so any brief outstanding across a successful patch is stale: two holes checked out before either patch, both briefs at one source_hash, gives PatchSuccess then PatchAuthError/stale from the real compiler with no stub, no threads and no second writer. So THE FIRST PROFESSOR ROUND (folded at Rev 2) REACHED A CORRECT CONCLUSION BY THE WRONG INVARIANT: Rev 1's witness was unsatisfiable under the reference's protocol, but because of the re-checkout discipline and not because the wave is serial. Rev 1 said reachable, Rev 2 said unreachable-so-inject-a-stub, and the measurement says reachable-without-a-stub by a mechanism neither turn considered. The case's arithmetic is UNCHANGED; only the mechanism producing the first rejection moves. The 4e acceptance clause consequently gains a REQUIREMENT THE PRIOR WORDING DID NOT NEED, that the fixture tree carry at least two holes and the cover cell hold two briefs simultaneously, without which the contention clause is vacuous for the THIRD time by a third mechanism. F-28: SECTION 7'S FIRST OWED STATEMENT IS NARROWED, NOT RETIRED, AND WAS PARTLY WRONG. `llmll patch` emits JSON on STDOUT with a stable five-way `result` discriminator and stderr was EMPTY on every invocation measured, so the channel is structured and the substring-over-stderr description was false. What remains owed is the sub-discrimination INSIDE PatchAuthError, which is NOT a synonym for contention: `invalid or expired checkout token` carries the same constructor, and only the three `obligation context is stale` messages are contention. THE REFERENCE'S PREDICATE IS OVER-BROAD AND THE PORT MUST NOT INHERIT IT, retrying on any PatchAuthError and so charging a non-contention auth failure to the protocol budget, which under [S9-SEPARATE] is the budget that never depletes. The staleness message carries U+2014 EM DASH immediately after the ASCII lexeme `stale`, so a fuller-phrase match is FS-ENCODING-1 territory. Also recorded: `checkout` splits its streams the OTHER way, brief as JSON on stdout and failure as prose on stderr, so the two commands share no convention. THIRD MEASUREMENT, and the sharpest: SECTION 7'S SECOND OWED STATEMENT IS UPGRADED FROM `UNPROVED` TO `DENOTES SOMETHING OTHER THAN WHAT THE POST IT FEEDS SAYS`, a category error rather than a gap. A def-shell whose body calls an UNFILLED sibling hole patches cleanly, verifies SAFE, appears in the body-faithful set, and persists body_faithful true with a verified_hash while the callee's body is still a hole node. That is sound assume-guarantee and the trust report is correct, but [S9-FAITHFUL] says `proved against its own body rather than assumed` and the body-faithful line means the VC was EMITTED body-faithfully, which is compatible with every callee being an unfilled hole whose contract was assumed. CRUX-FILL-ACCEPTS-ASSUMED WOULD NOT HAVE CAUGHT IT, a refute crux perturbing the proved decision and not the shell-side mapping into it, so the crux passing is not evidence the mapping is right; that is section 3.5's eliminative-versus-corroborative distinction applied one layer out. The resolution is FORCED rather than chosen, the per-fill bar being unable to see the callee graph because the check that would see it rejects a correct body whose siblings are merely unfinished (the strict-sibling wall), so PER-FILL ACCEPTANCE IS PROVISIONAL and the end-of-wave whole-tree --strict-verified-core is the closing check. Residue filed as Q-005 in theory-questions.md. The compiler-side INVITATION is separately closed (HOLE-STATUS-SIBLING: a sibling hole now reads status `unfilled` rather than `filled`, brief_version 0.12.3), which changes how often an agent writes such a body and nothing about what the bar measures. Rev 15 introduces ZERO new proof obligations: every change is to the trust channel, the posts of fill.next-error-budget, fill.is-finding and fill.fill-accepted are untouched and remain QF-LIA, and the abstraction functions are observations of another process's output, so they are not Lean candidates either. Rev 14, SETTLED. Rev 14 adds SECTION 9.3, the three settlements sub-phase 4d needs, and corrects a scope reading this document is the authority on: 4d IS H, K AND N per section 9's table, and a Rev 14 turn designed against a five-stage 4d before checking it, the 'five remaining stages' figure being a count of what is unported rather than a scope for one sub-phase. THE FORWARD HAZARD REV 12 ASSERTED IS REFUTED: registry.stage-tag-count needs NO widening, because test_driver_ll_4c.py's multi-invocation predicate counts any call whose attribute is `run` and therefore counts subprocess.run beside ctx.agent.run. Split by receiver, the technique halt_census.py already uses to close F-24, stage D holds one agent delegation plus a two-element tuple loop, K holds one plus one subprocess.run, L holds ZERO plus two, and M holds one of each, so THE ONLY GENUINELY MULTI-INVOCATION STAGE IS D and the D-only table is correct for the whole pipeline rather than only for what is ported. K takes 1, and L delegates to no agent at all, being gate-class. The tripwire consequently FIRES ON THE WRONG REPAIR the moment K is marked ported, directing the porter to widen a table whose K entry is already right, so correcting the census is a 4d PREREQUISITE and not a 4d task. M is the one stage with variable multiplicity, holes times attempts, which no [i: int] -> int entry can carry and which the test cannot see either, its loop predicate requiring an ast.Tuple where range(...) is a Call. This is the seventh instance of risk 3c and the first where the search had ALREADY BEEN EXECUTED and frozen into an assertion, which is why executing it was not enough. SECTION 6.5'S OPEN ITEM IS DISCHARGED and the answer is the one that section predicted: token-during's body is a three-case match over a nullary three-arm Phase, both posts are guarded implications over a single arm each, and crux-token-held-across-call perturbs the AgentWorking arm and is refuted by the post over that same arm, so THE CORE PROVES A PHASE-INDEXED INVARIANT AND NOT AN ORDERING. Section 10:371-373's release-then-obtain-fresh ordering is therefore a SHELL OBLIGATION and section 7's conditional fourth statement is owed unconditionally. The whole-wave closure is NOT A TRACE INDUCTION, which is how token.llmll's header and the driver README both describe it: a memoryless function of the current phase yields the trace property pointwise once the labelling is granted, so what is missing is not an induction over fills but the labelling, which is a refinement mapping over the port rather than a theorem about the module and is not Lean-dischargeable. Correcting both comments is owed with 4e, together with strengthening per-step to PER-STEP AND SINGLE-THREADED, the labelling being a function only while at most one hole is live, which makes the serial wave a precondition of the abstraction and not merely a throughput choice. Two 4e findings are recorded now because they were measured now: _verify_fn RETURNS body_faithful AS ITS OWN FIELD and discards it inside ok, so three of fill-accepted's four booleans have a shell-computed abstraction function available and only termination-proved has none, which corrects a Rev 14 claim that two were unavailable, made by reading the consumer's use of ok and inferring the producer's granularity from it; and the driver already declares TWO DISTINCT TYPES NAMED Phase, the sequencer's seventeen-arm payload-carrying control-flow sum and the token module's three-arm nullary lifecycle sum, a collision that becomes unavoidable at 4e's call site. Also settled for 4d, and CORRECTED WITHIN REV 14 BEFORE ANY CODE LANDED: 4d BUILDS THE FOURTH OUTCOME ARM. Settlement 2 first stated the opposite by counting two raising forms out of three. Stage H holds the reference's ONLY require_written, which raises PartialHalt and records `stopped`, and section 3.5 has carried that site since Rev 5 as the one divergent halt firing after its stage wrote a declared output. 4b built two arms, 4c built three, 4d completes all four, and the first construction of PartialThenHalt by a ported stage is 4d's sharpest acceptance criterion. WRITE-BEFORE-HALT IS PART OF THE OBLIGATION: H's disposition is a consequence of the write having happened, stage.llmll constrains the outcome and cannot see the ordering that produced it, so a port halting before writing would record `failed` and violate nothing proved. Section 3.5.1's rule is extended from \"cite the condition, never the line\" to ALSO COUNT ALL THREE RAISING FORMS, two consecutive readers having counted two while holding the document that warns the third exists. The surviving residue is that require_spec is reachable from none of the three and that SPEC-DEFINED HALTS ARE A GATE-CLASS PHENOMENON with exactly one exception, check_dispositioned, which is the site 4c found; what that regularity does not license is the inference drawn from it, `stopped` having two sources and not one. And STAGE K'S VALIDATOR IS THE COMPILER, K running llmll check on the agent's authored roots and halting on its return code, which makes it the first ported stage whose halt reads a process result rather than an artifact, needs no validate.llmll or shape.llmll clause, and extends PROC-TIMEOUT-1 past the agent path. Rev 13 CLOSES THE SECTION 3.6 CENSUS, the largest owed item in the phase for three sessions, by COMPUTING it with a durable instrument (experiments/rfc-swarm/tools/halt_census.py) instead of deriving it once by hand and keying it to an epoch. Section 3.6's line-keyed table is REPLACED rather than corrected: all five rows are undecidable on their own terms because line keys cannot be followed across an epoch, which vindicates Rev 10's refusal to renumber. n = 88 halt sites (a different denominator from section 3.5's, and not comparable). THE AXES DISAGREE AT THIRTEEN SITES, not one, so the eliminative-check argument strengthens. \"Six of the nine spec-defined sites are corroborated\" REPRODUCES EXACTLY and is invariant under all three readings of \"wrote some artifacts\" and under a doubling of the population, making it the only figure that did not depend on which halts the instrument could see. And section 3.6.1's claim that NO CENSUS COULD HAVE FOUND the two-dispositions-per-line site is REFUTED: a per-site census cannot, a three-value-lattice census can, and it found twelve instances without being told the first one existed. Rev 12 corrects Rev 11's own F-20 amendment, which was SHORT BY ONE: the dict-or-list tolerance census is FOUR sites over three artifacts, check_audit's `audited` being a fourth that belongs to stage G2 and sits outside 4c. Three readers running have stated this count short, each enumerating the artifact they were porting, so it is now COMPUTED by test_driver_ll_4c.py rather than restated in prose. Rev 12 also records that two readings of registry.stage-tag-count were both refuted (it is neither declared outputs nor static call sites; stage D declares two outputs and holds ONE agent.run executed twice by a loop) and that K, L and M are multi-invocation and unported, so the D-only tag table is a forward hazard now asserted by a test. Rev 11 folds TWO AMENDMENTS THE 4c PORT EARNED, both to section 9.2, neither changing what 4c does and neither touching a proved post: item 1 was SILENT on 4c constructing ConditionUnmet and the silence read as denial, 4c building THREE of Outcome's four arms where 4b built two because check_dispositioned:493 is require_spec rather than require; and F-20's dict-or-list tolerance is at THREE sites, the third being :482 over the dispositions document rather than core.json, so the generalization is the SHAPE and not the artifact and the n=0-live-runs null result covers core.json only. [V7-ONLY-TWO] stays TRUE as written, being about verdict-outcome's codomain rather than the port, so validate.llmll's prose was left alone. Sub-phase 4c is IMPLEMENTED AND MERGED against Rev 10. Rev 10 folds the sub-phase 4c harness leg: three MEASUREMENTS at `d15b2ff` (v0.14.87), not predictions, and the first of them refutes this document's own 4c acceptance clause. F-18: SECTION 9's 4c CLAUSE IS UNSATISFIABLE UNDER BOTH READINGS, which is section 1.2's defect recurring at sub-phase granularity. Stage E's seven Phase 3 pins hold 7 for 7 over the COMMITTED pair and 0 for 7 over stub-D output (compared 43 against 2, unmatched (1,10) against (0,0), kappa 0.9378 against 1.0), and the two Jaccard pins are not merely wrong-valued but UNADDRESSABLE, the stub's source being `spec.txt` so the `line_coverage` key is `SPEC` and the RFC1350/RFC1123 projections return MISSING. A live reading is stochastic and cannot pin (1,10); the only reading that holds is over the committed pair, which is what Phase 3 already established and does not exercise D at all. The clause is REPLACED: downstream ported stages RUN over 4c's own output, rather than pinned values reproducing over it. F-19: SECTION 3.6'S ONE-DISAGREEING-SITE CLAIM IS MEASURED FALSE. There are at least two and they resolve in OPPOSITE directions. Stage D's `check_extraction` halt on loop iteration b is the second: extractor A's `extraction.json` is declared, present, and passes the driver's own validator, so section 4:146's 'wrote some of its artifacts' is satisfied by a valid SIBLING and the artifact-state axis says `stopped`, while the reference records failed/Errored. A discriminating test with two live outcomes ELIMINATED the artifact-state-wins reading: the rule fitting both sites is HOLD-THE-EXISTING-VALUE, and section 3.6 read as though artifact-state won at :866 on the merits. The site is also invisible to a per-site census, one source line carrying different artifact-state dispositions on its two iterations, which is the loop-granularity analogue of F-16's deferred row-granularity limit. F-20: the bare-list arm at F:713 and G:732 has NO producer in the tree (stub, sole committed artifact and the stage-F template's own example are all dicts), so it is dead in-tree though not unreachable, and the port narrows to the dict shape and REJECTS a bare list rather than accepting it silently. Section 9.2 is NEW and settles 4c in five items, of which the sharpest is that [V7-NO-PARTIAL] and [V7-ONLY-TWO] are SOUND FOR 4c UNCHANGED and need no widening, `verdict-of` taking no artifact set, so the sibling-state question belongs at the call site rather than inside the shared facility. ALSO RECORDED, a reference defect and not a port one: stage E's docstring says the script 'stops if they are unadjudicated' and it does not stop, `reconcile.py`'s only non-zero exit being a missing `source` field, and since F, G and G2 all read `extraction-a.json`, the B-side extraction has no downstream consumer beyond the reconciler's report, making the (1,10) disagreement this port PINS inert in the reference pipeline. DEFERRED, not repaired: section 3.6's table is line-keyed and its keys are stale at HEAD, and re-deriving it needs a fresh census rather than a renumbering, since :809 is now a `for` statement inside `_pinned_sources` and the two G2 requires it names fire BEFORE G2 writes anything. Rev 9, SETTLED. Rev 9 folds an AST call-site census of section 3.5 at HEAD (v0.14.86) that REFUTED REV 8'S OWN EPOCH STAMP, and settles the three things sub-phase 4b needed. THE STAMP WAS WRONG THREE WAYS AND IT IS THIS DOCUMENT'S ERROR: it said the halt-site set dropped from 46 to 39, and it never dropped, being 46 at aa08051~1, 46 at aa08051 and 46 at HEAD, with ten conditions RE-ENCODED and none added or removed. The 39 came from `grep -cF 'require('` returning 40 and subtracting only the definition, missing three DOCSTRING mentions; the real call-site count is 36. The raise count also did not rise at Task #8 but at c10081d. A grep produced a wrong count inside the correction of a wrong count, which is risk 3c at its most literal. A THIRD RAISING FORM EXISTS, `require_written` to `PartialHalt` to `PartialThenHalt`, which neither section 3.5 nor the stamp mentioned. THE SUBSTANTIVE RESULT UNDER THE ARITHMETIC: the rule survives intact and the shipped repair ENCODED it rather than replacing it, all nine spec-defined sites being `require_spec` at HEAD carrying the clauses section 3.5 names, 9 for 9, with the polarity guard holding. Section 3.5.1 is NEW and forbids keying on the nine line numbers: two of them now point at sites with the OPPOSITE disposition, live `require()` calls recording `failed` where the section files them as `stopped`, so a reader lands on a plausible-looking site and nothing signals the miss. Consumers key on condition text and clause, which the repair made stable by requiring a citation at every raise site. Section 9.1 is NEW and settles 4b: the two unguarded reads in stages B and I are ported as decisions recording `failed` rather than reproduced as crashes (the port has no host exception to fall through, and read_manifest set the precedent at 4a); stage I has NO VALIDATOR either, so section 6.2's claim that stage O is the only such delegated stage is false, and 4b discloses that rather than inventing one; and every reachable halt in B, C and I records `failed`, so a `stopped` anywhere in 4b is wrong by construction. Also filed as a null result: section 3.1's row 3 does not describe the reference, since AgentRunner.run raises on a non-zero exit BEFORE the output-existence check, so a stage producing a valid declared output and exiting non-zero records `failed`, not `complete`. DEFERRED, not settled: F-16, a row-granularity limit where two sites halt `stopped` on non-core rows because their halt-mandate binds the gate rather than the stage; it does not block 4b, neither site being in B, C or I. Rev 8: SUB-PHASE 4a IS SHIPPED (`2b82464`, v0.14.85) and the phase now runs at 4b. Rev 8 settles the four findings Rev 7 filed and deliberately held, three of which were predictions; the port executed them and ALL THREE ARE CONFIRMED. (1) `wasi.fs.sha256` collapses presence and digest, which is now the third statement under section 7's `skip.may-skip` bullet, and it was worse than a disclosure gap: it made the port's own T7 scenario NON-DISCRIMINATING, hardcoding `artifacts-present` left all fifteen scenarios green on the one cell that exists to witness driver-spec section 5's condition (b) alone. Repaired in the port. The reference does NOT share the defect, established by mutation rather than reading: it computes its digest loop under the guard `if recorded and artifacts:`, so on an absent artifact (c) is vacuously satisfied rather than falsified. (2) Section 4's sum encoding makes `:on-done` usable, retiring the RC-4 workaround. (3) The port decides all three corrupt-manifest shapes with one total constructor-decidable predicate where the reference discriminates by Python exception site, so the port is BETTER than the artifact clause 1b checks it against; recorded rather than suppressed, 1b being a conformance claim and not a fidelity claim. (4) Section 3.5's counts are stamped with their measurement epoch: 46 call sites at `aa08051~1`, 39 at HEAD, moved by the repair section 3.5 itself ordered, so the argument stands and the arithmetic does not; 4b must re-measure. TWO FINDINGS THE PROPOSAL DID NOT PREDICT: the eleven-cell cover is stronger than its count states, since mutating each conjunct separately shows T7 and T6 die to their own conjunct and survive the other's, so the cells are SEPARABLE and not merely both present; and a perturbation that crashes is not a perturbation that refutes, since hardcoding presence at its definition yields an unhandled FileNotFoundError rather than a wrong skip, so a mutation must sit where the decision is read and not where the value is produced. Sub-phase 4a shipped with NO SHIM: PROC-BOUNDARY-1 was closed first, so argv arrives as flags and the process exits through `:status`, and nothing sits between the assertions and the program under test. Rev 7 folded the harness leg and unblocked sub-phase 4a. Rev 7 folds the execution of Rev 6's five checkable predictions (harness findings F-9 through F-11, compiler v0.14.84): four held and one is refuted. HELD: T7 re-runs the stage and prints no reason line, executed rather than read, so the eleven-cell cover is now eleven of eleven covered; the sequencer does crash on a corrupt manifest; it is NOT a section 4 violation, which is the point Rev 6 argued against the fourth professor round; and T7's silence is conformant and deliberately not asserted by the test. REFUTED, and it was this proposal's own claim: the reference CANNOT reach T7 through its own write path, because AgentRunner.run raises StageFailure when a delegated agent exits 0 having written nothing, so 'silence is not success' is already enforced for the eleven agent-delegated stages. Rev 6 grepped `stage.outputs`, the declared list on the Stage record, and the check is written against `out_name` one call frame down. Section 2.3 finding 3 is struck and replaced; section 10 case 3 is annotated as already-implemented rather than divergent; the surviving residue is the two mechanical and three gate stages, which have no generic presence check and are unmeasured rather than closed. SHORT BY ONE: Rev 6 named two corrupt-manifest shapes and there are three, each surviving the guard that catches the one before it, the third being a well-formed object with a list at `stages` that dies at the resume gate's own indexing expression. Added as section 10 case 18 with the method lesson. Risk 3c is new and is the reason this revision exists: four refuted mechanism claims across this phase, all the same shape, a grep over the name the document uses where the code enforces the same obligation under a different name one frame away, and all four caught by executing rather than by re-reading. Rev 6 folds the fourth professor round and repairs the citation surface. Five substantive changes. (1) Clause 1a was UNSATISFIABLE as written: it claimed the manifest sequences are identical, and a complete row carries a wall-clock `seconds` field while the three halt rows carry Python exception text as `detail`, so the reference is not identical to itself. §2.1 gains an abstraction function alpha, per field, with the criterion that a field survives alpha when some specification obligation mentions it. (2) §2.3 is rewritten as a product over three axes rather than a hand-enumerated list, because the count moved at every revision. Eleven cells, not nine. Two were missing: T4 (a halt after writing output, whose PartialThenHalt constructor alpha retains, and which §9's own four-arm acceptance requires) and T7 (a complete record with a declared artifact absent, driver-spec §5's skip condition (b) failing alone). NOTHING TESTS T7, the reference runs the stage correctly and silently, and the cell is the only witness for `may-skip`'s presence conjunct. (3) The cover models the per-stage machine and 4a lands the sequencer above it: an unreadable or non-object MANIFEST.json tracebacks out of the reference before any stage runs. That is NOT a §4 violation, §4 being scoped to a stage that halts, and it is routed as a reference repair rather than a §3 divergence, since every avoidable divergence weakens the retirement argument. (4) `FS-STAT-1` is re-scoped: an mtime does not close it, because `advancing` takes an age in seconds and monotonic reports nanoseconds since an unspecified epoch. The row now proposes a clamped-age builtin, which discharges the first precondition conjunct by construction where the shell-side subtraction would let clock skew violate it. (5) `CLAUSE-INDEP-1` is filed: `[S5-PRESENCE]` follows propositionally from `[S5-SKIP]`, so no mutation refutes it alone. The machinery ships already (CDP vacuity gate v0.14.13, `LLMLL.Feasibility` v0.14.52) and the clause-side dual is the cheapest of the three. Also: seven stale line citations repaired, five into compiler/src/LLMLL/ and all four of §10 case 6's; `FS-ENCODING-1` and `FS-COPY-1` marked shipped at v0.14.84 with the former's mechanism claim corrected, the release having measured it false; the emitMatch hole tagged `MATCH-CATCHALL-1`; and the instruction that the port reproduce the reference's silence at T7 WITHDRAWN, it having contradicted §2.1's own definition of the clause-1a observable. Rev 5, SETTLED. Rev 5 folds the third professor round and a sweep of driver-spec §8 through §15.4, the sections the truncated heading grep had left unread. Four substantive changes. (1) §3.5's classification rule was two-way over a FOUR-constructor Outcome: it never referenced PartialThenHalt, which stage.llmll proves maps to Stopped under [S4-PARTIAL] citing §4:146-147, and which Phase 3 already constructs. Measured over the write-before-require ordering of every stage, exactly one of the 38 divergent sites fires after its stage wrote a DECLARED output: :866, stage H. It becomes PartialThenHalt and stays stopped, so the divergence set is 37. The same clause independently corroborates six of the nine spec-defined sites and contradicts one, which is the profile of an eliminative check rather than an agreement. (2) §6 gains driver-spec §8, §10, §12 and §13 conformance. Stage O is the ONLY delegated stage with no validator at all, and §13 is its specification; its eight MUSTs split on §15.1:512-515's enumeration rather than on decidability, one being mechanizable (§13:443-446, perturbation omission) and seven disclosure-only. (3) §14's Blocks column is corrected twice: FS-COPY-1 blocks driver-spec §8:336-337, which requires the checking copy to be the original unmodified subject, so it is conformance and not ergonomics; FS-STAT-1 blocks §12:405-428 via a §15.2:522-524 capability gap, the §15.1:511 proof obligation being already discharged. (4) One target-spec defect filed: §15.1:509's range sentence contradicts its own enumeration at :512-515, and §13's prose MUSTs consequently fit none of the three tiers §15.1:504-505 requires every obligation to occupy. driver-spec is pinned under §14:473-474, so this is recorded rather than repaired. §15 records two corrections to this proposal's own prior turns, both misreadings of a citation audit this proposal itself produced. Rev 4, SETTLED. Rev 4 is the first revision driven by EXECUTING the harness rather than reading it, and it closes the one item Rev 3 left un-implementable. §3.5 is new: the halt surface is 46 require() sites plus three AgentRunner raises, not the four conditions §3.4 described, and the disposition attaches to the CLAUSE rather than the stage or the validator, because check_dispositioned holds six checks of which exactly one (:355, the closed-barrier condition, driver-spec §6:229-231) is spec-defined. §3.5 gives the classification rule with a polarity guard (§14:484-491 states two checks that MUST be reported and MUST NOT halt), enumerates the nine spec-defined sites by clause, and resolves all three sites the harness could not classify -- :967, :866 and :1045 -- to failed, which unblocks the Python-side repair. Divergence set is 38, not 4. Three corrections to Rev 3, two of them to this proposal's own claims: §10 case 5 was WRONG (it derived 'no holes to fill -> stopped' from the driver's own source, and driver-spec has no clause about an empty hole set, so §4:129's residual gives failed -- the third instance of the error shape §3.3 records twice, and the first originating here); §2.3's coverage claim counted STUB_MODE values rather than tests, and the measured cover was six of eight with a ninth transition omitted; and the citation-resolution mandate is §14:479-483, not §7, which Rev 3 missed because a heading grep truncated at section 9 and §10 through §15.4 were never read. One finding PARTLY REFUTED: stage H is not unspecified, since §7:313-316's catalogue clause covers it; only its acceptance bar is driver-defined, which narrows what Phase 5 §15.4 owes. Rev 3, SETTLED. Revises `driver-in-llmll-campaign.md` §Phase 4 on two structural corrections: the phase's stage enumeration was short by one (stage G was assigned to no phase), and its acceptance clause ('a complete run reproduces a committed campaign's artifacts') is not satisfiable against this tree, measured. Replaced by two claims plus a derived transition-cover scenario set. Rev 3 settles the delegated-stage disposition on its THIRD derivation: Rev 1 argued from §4's residual clause, Rev 2 argued from §7's wording and was refuted on the specification's own use of 'fails' as a verb, and Rev 3 rests the conclusion on §4:132-136's verdict-versus-accident criterion, which neither prior derivation touched. The conclusion did not move; the argument did, twice. Rev 3 also reframes the finding against the Python driver from a wrong constant to a missing distinction: it has ONE halt channel where the specification defines TWO, and `stage.record-outcome`'s `Outcome` type already models both. Rev 2 folded the first professor round (serial wave makes the contention branch unreachable, so the Rev 1 positive witness was unsatisfiable; state is a sum over phases, not a flat product; `wasi.fs.copy` split out from the encoding fix). Rev 3 folds the second (the §7 lexical refutation, accepted; its replacement conclusion, rejected; the citation-clause over-reading, withdrawn; the abstraction-function disclosure, adopted). Both professor rounds were conversational; no standalone review file exists, and their findings are folded in §15."
date: 2026-08-04
...
One real campaign run against a target already run in Python. **Live is the whole of what this
clause adds.** Clauses 1a and 1b already exercise all fifteen stages hermetically, over a stub agent
and a stub `llmll` (§2.3), so a replay of committed artifacts through the LLMLL driver is clause 1a's
oracle and not this one. Reading this clause as a replay deletes its content and leaves the campaign
with no live evidence at all.

**Rev 16 splits an obligation Rev 15 stated as one sentence, because agreement is unobtainable for
five of the six decision classes and that is not a defect in the port.** Gate J's verdict is computed
from the inventory the agent produced, and stage L's exit status from the frozen set the agent
produced. A live run re-samples the agent, so the inputs differ and the verdicts are incomparable
**even when the port is exact**. "Judged on decisions rather than bytes" states how to compare. It
does not state what must match.

**Thresholded. The clause fails if any of these is unmet.**

- The run reaches a terminal state over **every stage the registry enumerates**, which is
  `registry.llmll`'s `stage-count` and not the specification's count. **Rev 17 states this because
  the two counts differ and both are correct.** driver-spec's own sentence is "run fifteen stages
  in order", and [`../../experiments/rfc-swarm/README.md`](../../experiments/rfc-swarm/README.md)
  lists those fifteen as A through O with no G2. The implementations dispatch **sixteen**: the
  fifteen plus **G2, the artifact audit**, whose kind is `gate`. Measured: `stage-count` returns 16
  and the reference defines 16 `stage_` functions. A comparator that iterates the specification's
  fifteen skips a gate, which is the reading this bullet exists to prevent.
- Zero FFI declarations, and bounded authority end to end.
- Every halting stage names its clause **on its output**, per driver-spec §4:139-143.
- No stage records `complete` where the reference's own shape predicates reject the declared
  artifact (§3.1 row 2).
- No stage records `stopped` where the reference recorded `failed`, or the reverse. driver-spec
  §4:132-136 calls this the one confusion this pipeline cannot afford. **The asymmetry is
  deliberate**: a port that halts MORE than the reference is conservative and reportable, and a port
  that halts LESS has lost a check, which is the direction §8.1's retirement decision rests on.

**Reported, NOT thresholded.** All six decision classes, recorded with n and set beside the committed
run: each stage's recorded status, each gate's verdict and the condition it names, the reconciliation
figures stage E pins, the coverage exit status stage L pins, the wave's filled-versus-finding
partition and its retry-budget accounting. A divergence in a reported class is a finding and not a
failure. The idiom is the campaign's own, carried by
[`../../experiments/rfc-swarm/PRE-REGISTRATION.md`](../../experiments/rfc-swarm/PRE-REGISTRATION.md)
and by both committed `gate.json` files.

**Stage status leaves the thresholded set, because clause 1a owns it.** α of the manifest is checked
hermetically over the full transition cover (§2.1, §2.3). Neither committed run carries a
`MANIFEST.json`, and that is not a gap in clause 2: the class was never clause 2's to own.

Artifact *shape* is checked by the same predicates the Python driver applies; artifact *content* is
not compared.
...
**The target is RFC 826, and the deciding fact is not its artifact count.**
[`../../experiments/rfc-swarm/runs/rfc826/RESULTS.md`](../../experiments/rfc-swarm/runs/rfc826/RESULTS.md)
records that the run "put a fresh RFC through all fifteen stages with no human authoring at any
step". TFTP is disqualified as an oracle by that same file: its inventory, dispositions and root
contracts were authored by hand and only its fill wave was agent-driven.
[`../../experiments/rfc-swarm/runs/rfc4648/`](../../experiments/rfc-swarm/runs/rfc4648/) stopped at
gate J and never reached the wave or the mutation stages. **One figure in rfc826's RESULTS.md is
known incorrect**: its comparison column cites TFTP coverage at 62/65 = 95.4%, re-derived later as
78.5%. The ARP column is unaffected and the TFTP column must not be cited.

**The reference run's model is not recoverable.** That file records its agent command once, as
`claude -p` with no model named, and `PROVENANCE.json` records the RFC source only: url, file,
sha256, lines. Every reported class is therefore confounded with model change, and that confound is
the reason those classes are reported rather than thresholded.
...
**Computed at Rev 13**, by
[`halt_census.py`](../../experiments/rfc-swarm/tools/halt_census.py) over
[`rfc_to_implementation.py`](../../scripts/rfc_to_implementation.py) at HEAD. The instrument is
durable and re-runnable, keyed by **(stage, call chain, site)** and never by line, and it is
interprocedural because the halts that matter fire inside helpers and methods the stage calls rather
than in the handler's own body. **Re-run it rather than re-deriving this by hand**; deriving it by
hand once, and keying the result to an epoch, is what put this section in the state Rev 10 recorded.

**n = 88 halt sites reachable from a stage handler.** That denominator is not §3.5's: §3.5 counts
source sites, this counts (stage, site) pairs, so a helper called by one stage contributes once and a
site no stage reaches contributes zero. The two numbers are not comparable and neither corrects the
other.

| Stage | Halts | Own body | Via helper or method | `require_spec` | Artifact-state |
|---|---|---|---|---|---|
| B | 6 | 1 | 5 | 0 | 6 pre-write |
| C | 5 | 1 | 4 | 0 | 5 pre-write |
| **D** | 13 | 0 | 13 | 0 | 1 pre-write, **12 BOTH** |
| E | 1 | 1 | 0 | 0 | 1 pre-write |
| F | 5 | 1 | 4 | 0 | 5 pre-write |
| G | 10 | 0 | 10 | 1 | 10 pre-write |
| G2 | 18 | 5 | 13 | 5 | 15 pre-write, 3 post-write |
| H | 7 | 3 | 4 | 0 | 6 pre-write, 1 post-write |
| I | 4 | 0 | 4 | 0 | 4 pre-write |
| J | 2 | 2 | 0 | 2 | 2 post-write |
| K | 5 | 1 | 4 | 0 | 5 pre-write |
| L | 1 | 1 | 0 | 1 | 1 post-write |
| M | 2 | 2 | 0 | 0 | 2 pre-write |
| N | 5 | 1 | 4 | 0 | 5 pre-write |
| O | 4 | 0 | 4 | 0 | 4 pre-write |

**`BOTH` is a value, not an imprecision.** A site inside stage D's extractor loop is reached
pre-write on the first extractor and post-write on the second, so it carries two artifact-state
dispositions. §3.6.1 found one such site by hand; the census finds twelve.

**The reading is the one §3.6.1 settled, and the census implements it rather than choosing it.**
"Wrote some artifacts" (§4:146-147) counts only writes to **this stage's own declared outputs**.
That was settled below on the stage E discriminating case, and it matters more than it looks: the
choice moves **29 of the 88 sites**, so a census under the any-file reading would disagree with this
one about a third of the population.
...
them. That figure reproduces exactly, and it is **invariant under all three readings of "wrote some
artifacts" and under the doubling of the population** when the census learned to follow method calls.
It is the one quantity here that does not depend on which halts the instrument can see, which is why
it survived a table whose every key did not.

**The axes disagree at thirteen sites, not one.** Nineteen of the 88 are post-write or `BOTH`; six of
those are `require_spec` (the axes agree) and thirteen are not: stage D's eight `check_extraction`
halts, the four D reaches through `Ctx.prompt` and `AgentRunner.run`, and stage H's `require_written`
halt. **The eliminative-check argument therefore gets stronger, not weaker.** The disagreement is a
population rather than an anomaly, and at thirteen sites the question stops being "which reading wins
here" and becomes "which axis the port implements", which §3.5 answers for the port and §4:146-147
answers for the spec.

**Rev 10, kept as history: this table used to be line-keyed and its keys went stale.** The record of
why is worth more than the table was. The keys were line numbers, which §3.5.1 forbids for §3.5's
sites and which this table was left carrying. At `d15b2ff`, `:809` was a `for` statement inside
`_pinned_sources` rather than a write, and the two G2 conditions the first row named fired **inside
`_pinned_sources`, which G2 calls before writing anything of its own**. Rev 10 declined to renumber,
on the ground that guessing which sites moved is how the Rev 8 stamp went wrong, and it was right to:
at HEAD, `:811` and `:815` are plain `require` calls at pre-write, the **opposite of the old first
row on both axes**. That is not evidence the row was wrong when written. It is evidence that the keys
cannot be followed, which is exactly §3.5.1's hazard, and it is why **the census replaces the table
rather than correcting it**. None of the five old rows is confirmed and none is refuted; they are
undecidable on their own terms.
...
> **Rev 13: "at least two" was a lower bound and is now a number. There are thirteen**, computed over
> a population of 88 rather than 46; twelve of them are stage D's and one is stage H's. Everything
> below is unrevised and holds: the rule this section settles, the reading it settles, and the
> direction each site resolves in were all established on the two sites it names, and eleven more
> instances of the same shape do not disturb any of it. What the count changes is the weight: a rule
> chosen to fit two sites now fits thirteen.
...
**Why no census would have found this — REFUTED at Rev 13, and this is the interesting one.** The
claim was that `:656` is **one source line with two artifact-state dispositions**, depending on which
iteration of `for tag in ("a", "b")` is executing, and that a per-site census cannot represent that.
The premise is right and the conclusion was wrong. A census keyed **per site** cannot represent it; a
census whose artifact-state value is drawn from a **three-value lattice** can, because "reached at
pre-write on one iteration and post-write on another" is a representable value rather than a
contradiction. `halt_census.py` reports it as `BOTH`, was not told this site existed, and found
**twelve** of them in stage D. So the limitation was in the instrument this section imagined, not in
censuses; it remains the loop-granularity analogue of F-16's deferred row-granularity limit, and
F-16's may well yield to the same move. The cell is now pinned permanently by
...
### 5.1 AMENDED at Rev 18: item 1 was not applied at 4e

**Item 1 is settled and stage M does not implement it.** Every other delegated stage maps its
argument vector through the substitution item 1 names. The wave appends `brief.json` and
`body.json` to the operator's arguments and substitutes nothing, so one `--agent-arg` list now
serves two incompatible calling conventions.

**The witness is measured rather than argued.** Run the `wave` sub-command with the argument vector
the 2026-09-11 campaign actually used, and the agent receives `--model`, `claude-opus-5`, the
literal string `{prompt}`, the literal string `{out}`, the brief path and the body path. The
committed wrapper tests the third argument for readability and exits 2 on the literal, before any
model is reached.

**No gate could see it.** Cover cell W1 passes no `--agent-arg`, so the untemplated case is the only
one the cover exercises; `driver_ll_cover.py` stopped selecting stage M when the fold landed; and
the clause 2 run never reached stage M, because stage M was stubbed. A cover cell that passes
`--agent-arg` is owed.

**Item 1 is not the whole repair.** driver-spec §8 names two structural isolation mechanisms and the
port has neither; §14's `FS-ISOLATION-1` row carries that. The reference also renders `{{errors}}`,
the previous attempt's compiler output, into each retry. The port clears the token and the body and
carries only the two budgets, so attempt n+1 receives byte-identical input to attempt n. That makes
the port's retry budget a sampling loop where the reference's is a repair loop. driver-spec §9
constrains the budget and is silent on the input, so this is a fidelity divergence and not a §9
violation.
...
**SETTLED at Rev 14, and the answer is the one this section expected.** `token-during` declares a
total function from phase to token state; its body is a three-case `match` over a nullary three-arm
`Phase`, and both posts are guarded implications over a single arm each. `crux-token-held-across-call.llmll`
perturbs the `AgentWorking` arm from `Released` to `Held` and is refuted by the post over that same
arm. No post, and no line of the body, mentions two phases, a sequence, or a transition. **The core
proves a phase-indexed invariant and does not prove an ordering.** Therefore §10:371-373's ordering
is a shell obligation, and §7's conditional fourth statement is now owed unconditionally.

This is the same conclusion three reading paths reached independently: this section's own prediction
from the first professor round, the fourth round's request to confirm it against the body, and a Rev
14 reading which arrived at it from the opposite direction by asking what a whole-wave closure would
need. The closure is **not** a trace induction, which is how `token.llmll:3-4` and the driver README
both describe it. A memoryless function of the current phase yields the trace property pointwise
once the labelling is granted, so what is missing is not an induction over fills but the labelling
itself. That is a refinement mapping in Abadi and Lamport's sense, it is a property of the port
rather than of the module, and no proof assistant can discharge it because it quantifies over the
driver's states rather than over a `def`'s inputs. **Correcting those two comments is owed with 4e**,
and the correction also strengthens "per-step" to "per-step and single-threaded": the labelling is a
function only while at most one hole is live, so the serial wave is a precondition of the abstraction
and not merely a throughput choice.
...
- **`fill.next-error-budget` / `fill.is-finding`.** **Narrowed at Rev 15, measured.** `contention` is
  NOT a substring test over the whole channel: `llmll patch` emits JSON on **stdout** carrying a
  five-way `result` discriminator, `PatchSuccess | PatchTypeError | PatchVerifyError |
  PatchApplyError | PatchAuthError` ([`PatchApply.hs:119-150`](../../compiler/src/LLMLL/PatchApply.hs)),
  and stderr was empty on every invocation measured. The port keys on the discriminator. What
  remains owed is the sub-discrimination INSIDE `PatchAuthError`, which is not a synonym for
  contention: `:389` produces `invalid or expired checkout token` under the same constructor, and
  only the three `obligation context is stale …` messages (`:469`, `:481`, `:485`) are contention.
  Telling them apart is a substring test over a compiler-internal string with no stability contract.
  Key on the ASCII lexeme `stale` and never on the full phrase, which carries **U+2014 EM DASH**
  immediately after it and is `FS-ENCODING-1` territory. **The reference's predicate is over-broad
  and the port must not inherit it**: `_apply` retries on `"stale" in err or "PatchAuthError" in err`
  ([`rfc_to_implementation.py:1367-1369`](../../scripts/rfc_to_implementation.py)), so an auth
  failure that is not contention is charged to the protocol budget, which under `[S9-SEPARATE]` is
  the budget that never depletes. Note also that `checkout` splits its streams the OTHER way, brief
  as JSON on stdout and failure as prose on stderr with exit 1, so the two commands share no
  convention and a port must not assume one.
- **`fill.fill-accepted`.** `verifies`, `body-faithful` and `termination-proved` are substring and
  ...
  **Rev 15 upgrades this from "unproved" to "denotes something other than what the post it feeds
  says", which is a category error rather than a gap.** Measured end to end at v0.14.87: a
  `def-shell` whose body calls an UNFILLED sibling hole patches cleanly, verifies SAFE, appears in
  the `body-faithful:` set, and persists `body_faithful: true` with a `verified_hash` in the sidecar
  while the callee's body is still a hole node. That is sound assume-guarantee and the trust report
  is correct, the callee reading `asserted`. But `[S9-FAITHFUL]` says the fill is "proved against
  its own body rather than assumed", and the `body-faithful:` line means the function's VC was
  EMITTED body-faithfully, which is compatible with every callee being an unfilled hole whose
  contract was assumed. The two are different propositions and the port was about to equate them.
  **`crux-fill-accepts-assumed` would not have caught this**, because a refute crux perturbs the
  proved decision and not the shell-side mapping into it, so the crux passing is not evidence the
  mapping is right; that is §3.5's eliminative-versus-corroborative distinction applied one layer
  out. The resolution is forced rather than chosen: the per-fill bar cannot see the callee graph,
  because the check that would see it rejects a correct body whose siblings are merely unfinished
  (the strict-sibling wall), which is why `_verify_fn` avoids it. So **per-fill acceptance is
  PROVISIONAL and the end-of-wave whole-tree `--strict-verified-core` is the closing check**, and
  `fill-accepted`'s `body-faithful` argument carries the provisional reading. The residue is filed
  as `Q-005` in [`theory-questions.md`](theory-questions.md). The compiler-side INVITATION is
  closed (HOLE-STATUS-SIBLING: a sibling hole now reads `status: "unfilled"` rather than `"filled"`,
  `brief_version` 0.12.3); 4e must not read that as fixing the accounting, since it changes how
  often an agent writes such a body and nothing about what the bar measures.
...
- **A further statement IS owed at 4e**, for §10:371-373's token ordering. Rev 14 discharged the
  condition it was waiting on: `token-during`'s body is a three-case `match` over a nullary `Phase`,
  its two posts are guarded implications over single arms, and `crux-token-held-across-call.llmll`
  perturbs one arm and is caught by the post over that same arm. Nothing in the module expresses a
  sequence, a transition, or a pair of phases. So the core proves a **phase-indexed invariant and
  not an ordering**, §6.5's conditional fires, and **§10:371-373's release-then-obtain-fresh ordering
  is a shell obligation**. What the shell computes and hands across the seam is the claim that the
  phase it labels a moment with is the phase that moment is in; the proved core is then a lookup
  table over that labelling. The statement's failure mode is that a port which released and
  re-obtained in the wrong order would still satisfy every post, because no post mentions order.
...
| **4d** | H, K, N | **`oracle.probe-established?`, `feasibility-established?`, `outcome-as-expected?`, `matrix-complete?` and `shape.probe-rows-conform?`.** Rev 16 corrects this cell, which read "none new": those five cores shipped **callerless** at v0.14.88 and 4d gave them their callers. The column asks what a sub-phase ACTIVATES, and activating a callerless core is the case the column exists to record | Probe-verifies / mutant-refutes polarity reproduces; stage N retains unrealisable entries in the denominator |
| **4e** | M, serial, **with contention produced by two outstanding briefs (Rev 15; no stub)** | `fill.fill-accepted`, `fill.next-error-budget`, `fill.is-finding`, `token.token-during` | Both retry budgets fire and are separately counted. **The fixture tree carries at least two holes and the cover cell HOLDS TWO BRIEFS SIMULTANEOUSLY**, without which the contention clause is vacuous for the third time by a third mechanism (Rev 15 risk 1). A hole exhausting its semantic budget is a finding and never hinted; §7's statements are written, and §6.5's `token.llmll` read is resolved either way. **CORRECTED at Rev 18: the never-hinted clause is right and 4e's mechanism for it is wrong.** 4e read it as the brief being the agent's only input, and cover cell W1 asserts a two-argument invocation as the proof. driver-spec §8 forbids a peer's directory, a peer's output and a worked answer; a rendered task statement is none of those, and §8's third paragraph makes a declared input a driver-side commitment subject to audit. W1 is correct that no side channel exists and wrong that a two-argument argv is what establishes it. The cell is rewritten against §8's wording rather than deleted |
| **4f** | O **with its §13 validator**, phase close, gap inventory | **`report.omission-free?`** (`report.llmll`), the §13 set-difference check over stage N's `kill-matrix.json`. Rev 16 corrects this cell, which read "none new" while 4f landed a core that is both NEW and activated in the same sub-phase. It is the first such case; 4d's cell above records the other kind | Clauses 1a, 1b and 2; the perturbation-omission check of §6.2 fires on an omitted survivor and records `PartialThenHalt`; the seven disclosure-only §13 clauses are listed as unchecked in the phase close |
...
**Amended at Rev 11, from the port: 4c constructs `ConditionUnmet`, and this item's silence read as
denial.** The item settles what 4c does **not** construct and stops there, which invited the reading
that 4c builds the same two `Outcome` arms 4b built. It builds **three**. `check_dispositioned` holds
six checks and five of them are `require`, recording `failed` (`:483`, `:486`, `:487`, `:490`,
`:499`). The sixth, `:493`, is **`require_spec`** (defined at `:358`) citing `driver-spec sec
6:229-231`, so §3.5's rule makes it spec-defined and it records **`stopped`**. This is measured, not
read: the rig asserts `m["G"]["status"] == "stopped"` in
`test_exclusion_outside_the_barrier_list_halts_the_run`, mode `bad-barrier`.

Two consequences, and neither widens a proved post. **`[V7-ONLY-TWO]` stays true exactly as stated**,
being a property of `verdict-outcome`'s codomain rather than an invariant of the port, so
`validate.llmll`'s prose is correct and was **left alone**: a shipped proved module is not where a
later sub-phase's arithmetic goes. And the barrier halt must **not** route through `verdict-outcome`,
which `[V7-NO-STOP]` correctly forbids for the three stages 4b ported, none of which has a
spec-defined halt; it uses the sequencer's existing `halt-with … ConditionUnmet` channel instead.
The item's original claim, that 4c constructs no `PartialThenHalt`, is unaffected and still holds.
...
**Amended at Rev 11 and CORRECTED at Rev 12: the tolerance is at FOUR sites over three artifacts.**
F-20 named `:713` (stage F) and `:732` (stage G), both indexing `core_ids` on `core.json`. `:482`,
inside `check_dispositioned`, is a third of the same shape over the dispositions document's `rows`.
`:456`, inside `check_audit`, is a **fourth**, over the audit document's `audited`, and it belongs
to **stage G2** and so sits outside sub-phase 4c entirely. **The generalization is the shape, not
the artifact**, and the count has now been stated short three times running for the same reason:
each reader enumerated the artifact they were porting. The census is computed rather than
remembered at
[`test_driver_ll_4c.py`](../../scripts/tests/test_driver_ll_4c.py)`::test_the_dict_or_list_tolerance_census_is_four_sites`,
which fails on a fifth. The narrowing must cover every site or one arm silently keeps the
tolerance, and the n=0-live-runs null result covers `core.json`'s **two** sites only; whether a
live agent emits a bare-list dispositions or audit document are two **separate** unmeasured
questions with the same shape and the same loud-on-surprise disposition.
...
### 9.3 Three settlements sub-phase 4d needs

*(Added at Rev 14, all three measured at `b9cbf00`. **Sub-phase 4d is H, K and N**, per §9's table.
The "five remaining stages" reading is a count of what is unported and not a scope for 4d: M lands
at 4e with the wave and the two proved cores, O at 4f with its §13 validator. A Rev 14 turn designed
against the five-stage reading before checking the table, which is the failure §9's table exists to
prevent.)*

**1. `stage-tag-count` needs no widening, and the test asserting that it will is counting
`subprocess.run`.** `test_driver_ll_4c.py`'s forward-hazard test classifies a stage as
multi-invocation when `ast.walk` finds more than one `ast.Call` whose `func.attr == "run"`. That
predicate counts `subprocess.run` beside `ctx.agent.run`, and these handlers shell out. Split by
receiver, which is the technique `halt_census.py` already uses to close F-24, the same walk gives: D
one `ctx.agent.run` plus a two-element tuple loop, K one `ctx.agent.run` plus one `subprocess.run`,
L **zero** `ctx.agent.run` plus two `subprocess.run`, M one of each. **The reference's only genuinely
multi-invocation stage is D.** So `(if (= i 3) 2 1)` is correct for every stage in the pipeline and
not merely for those ported: K takes 1, and L delegates to no agent at all, being `gate`-class. The
test's two docstrings state K's and L's counts wrongly and are rewritten with the predicate.

M is the one stage with variable multiplicity, and no `[i: int] -> int` entry can carry it: its
single `ctx.agent.run` sits inside a `range(1, semantic_retries + 1)` retry loop inside a per-hole
closure dispatched over the hole list, so the count is holes times attempts and both factors are
data-dependent. The test cannot see that loop either, its `loops` predicate requiring an `ast.Tuple`
where `range(...)` is a `Call`, so M is currently detected only through the `.run` overcount.

The consequence is a live tripwire that fires on the wrong repair. When K is marked ported,
`unported_multi` becomes `{"L","M"}`, the assertion `== {"K","L","M"}` fails, and the message directs
the porter to widen a table whose K entry is already right. **Correcting the census is a 4d
prerequisite, not a 4d task.** Receiver qualification is the minimum and is not sound alone: an
aliased receiver defeats it and nothing would notice, so the corrected predicate must also assert the
receiver is unaliased. This is the seventh instance in this phase of a search over the name a
document uses where the code means something one attribute away, and the first where the search had
already been executed and frozen into an assertion, which is why executing it was not enough.

**2. 4d builds the FOURTH `Outcome` arm, and the first statement of this settlement said the
opposite by counting two raising forms out of three.** Stage H holds a **`require_written`**, which
raises `PartialHalt` and records `stopped` whatever defined the condition. It is the **only
`require_written` call site in the reference**, its own definition says so, and §3.5 has carried the
site since Rev 5 as the one divergent halt that fires after its stage wrote a declared output. H
writes `feasibility.json`, its declared output, and only then halts on the probe-polarity bar, which
is §4:146-147 and is why the disposition is `stopped` rather than `failed`.

Counted over all three forms and interprocedurally, 4d is: H two `require` plus **one
`require_written`**, K one `require`, N one `require`, with the shared `prompt` helper contributing
one `require` on every path and `_sources_text` one more on K's. Zero `require_spec` between them,
which is true and was never the question. **4b built two arms, 4c built three, and 4d completes all
four.** `stage.record-outcome` proved all four at 4a, so no proved core moves; what is new is the
first construction of `PartialThenHalt` by a ported stage, and it is 4d's sharpest acceptance
criterion.

The ordering is part of the obligation, not an incidental. H's disposition is a consequence of the
write having already happened, so a port that halted before writing would record `failed`, violate
nothing proved, and be wrong. `stage.llmll` constrains the outcome and cannot see the ordering that
produced it, so **write-before-halt is a construction discipline at this site** and the cover cell
must assert `feasibility.json` present alongside the `stopped`.

The residual observation survives and is worth keeping: `require_spec` is reachable from none of the
three, and the file's four `require_spec`-holding functions are `stage_G2_audit` (five),
`stage_J_gate` (two), `stage_L_coverage` (one) and `check_dispositioned` (one), so **spec-defined
halts are a `gate`-class phenomenon with exactly one exception**, that exception being stage G's
helper, which is the site 4c found. What that regularity does not license is the inference drawn
from it. `stopped` has two sources, `require_spec` and `require_written`, and enumerating one of
them is how this settlement was stated backwards. §3.5.1's rule is extended accordingly: cite the
condition rather than the line, **and count all three raising forms**. Two consecutive readers
counted two, one of them holding the document that warns the third exists.

**3. Stage K's validator is the compiler, and K is the first ported stage whose halt reads an exit
code rather than an artifact.** K delegates, then runs `llmll check` on the agent's authored
`roots.llmll` as a subprocess and halts on its return code. K therefore needs no clause in
`validate.llmll` or `shape.llmll`: its acceptance oracle is the compiler's own exit status, a
stronger check than any shape predicate and already reachable through the `wasi.proc` capability the
sequencer declares. Three consequences. The halt is a `require` over a process result, so §3.5's rule
gives `failed`. `PROC-TIMEOUT-1` reaches further than the agent path, since a pathological input to
`llmll check` cannot be bounded either, and no cover cell may claim it is. And K's hole count is a
`count("?")` over source text used **only in a log line**; the port must not promote it to a gate,
because that number is false the moment a `?` appears inside a string literal.

K also holds two unguarded reads, one over stage G's dispositioned inventory including an unguarded
`["rows"]` key access, and one over stage B's `scope.md`. Both are ported as guarded decisions
recording `failed`/`Errored` on §9.1 item 1's settled precedent, which is application rather than
anything new.

**Two findings 4e inherits, recorded here because they were measured now.** First, `_verify_fn`
returns `body_faithful` as its own field and computes `ok` as a conjunction that discards it, so
three of `fill-accepted`'s four booleans have a shell-computed abstraction function available and
only `termination-proved` has none. A Rev 14 turn asserted that two were unavailable, having read the
consumer's use of `ok` and inferred the producer's granularity from it. Second, the driver already
declares **two distinct types named `Phase`**: the sequencer's seventeen-arm control-flow sum, whose
arms carry payloads and include `Delegate`, and the token module's three-arm nullary lifecycle sum.
They model different things, the sequencer does not import the token module today, and the collision
becomes unavoidable at 4e's call site, where the mapping §6.5 now routes to the shell has to name
both. Renaming the sequencer's is the cheaper side, the token module's being the one carrying
`:source`-bearing spec obligations.
...
**REACHABLE WITHOUT INJECTION, AND WITHOUT CONCURRENCY. Rev 15, measured; the stub is retired.**
The premise above conflates two propositions this document treated as one: (P1) the wave is serial,
at most one hole live; and (P2) `_apply` releases its token and takes a fresh checkout under the lock
immediately before patching. **Contention-unreachability follows from P2 alone.** P1 without P2
produces contention, and P2 without P1 avoids it, which is exactly what `_apply` is built for at N
agents ("Agents think in parallel; patches serialize"). The CAS is per-file and compares the brief's
`source_hash` against the current source, so ANY brief outstanding across a successful patch is
stale. A wave holding two briefs at once therefore gets `contention = true` from the real compiler.
Measured at v0.14.87, two holes checked out before either patch, both briefs at one `source_hash`:

    patch A -> {"result":"PatchSuccess","statements":2}
    patch B -> {"result":"PatchAuthError",
                "message":"obligation context is stale … (source file changed)"}

No stub, no threads, no second writer, no tree mutation. The stub would have tested the driver's
handling of a string the driver's own author chose, where the two-brief construction tests the
compiler's actual rejection. **The first professor round (folded at Rev 2) reached a correct
conclusion by the wrong invariant**: Rev 1's positive witness was indeed unsatisfiable under the
reference's protocol, but because of `_apply`'s re-checkout discipline and not because the wave is
serial. Third time this case has moved, and the first time the mechanism was measured rather than
argued.

**The arithmetic below is unchanged.** Only the mechanism producing the first rejection moves.
...
- `scripts/tests/test_rfc_pipeline_integration.py` — the stub agent moves from env to argv. **The
  §10 case 1 contention injection is STRUCK at Rev 15**: contention is reachable against the real
  compiler by holding two briefs across one patch, so the stub `llmll` gains nothing here and 4e's
  cover cell drives the real binary instead. **The scenario set is eleven cells
...
| `FS-STAT-1` | **Re-scoped in Rev 6.** `liveness.advancing` has no callable data source. **An mtime does not close this**, and Rev 5's routing note said it would: `advancing` takes an **age in seconds** (`liveness.llmll:6`) and monotonic reports nanoseconds since an *unspecified* epoch, so no subtraction is defined. Proposed shape: `wasi.fs.stat` returns the artifact's **age in seconds, clamped at zero**, as `RCode`, carrying `Caps {EFsRead, ENonDet}`, both labels already in the closed catalog (`ObligationAssembly.hs:399`), so CAP-PROC's admissibility rule holds with no new `Response` arm. The clamp discharges `[S12-DOM]`'s first conjunct **by construction** [**CORRECTED 2026-08-19, and again 2026-09-05: BOTH HALVES OF THIS CLAUSE ARE WRONG.** `fs-capability-trio-proposal.md` Rev 2 §4 WITHDRAWS the clamp, because clamping a negative age to zero reports maximal freshness to a liveness check; `RErr` replaces it. And the clamp never discharged anything "by construction": a `Command` result reached no channel until `RESP-FACT-1` shipped at v0.17.0. The live design is the trio proposal §4, not this sentence]; the rejected alternative (epoch mtime plus a new `wasi.clock.realtime`, subtracted shell-side) lets clock skew drive the age negative, turning a stale-liveness read into a precondition violation. Two consequences for the row: `[S12-DOM]`'s second conjunct stays shell-side (§7), and an age-returning builtin is a **candidate** first firing site for `W-REPLAY-INERT` (roadmap `:51`), whose in-tree firing population is recorded as zero. Also: `RCode` now carries exit statuses, monotonic nanoseconds and clamped ages, with the unit nowhere in the type. That is a scope boundary LLMLL has chosen (no dimension-indexed numerics; cf. Kennedy, POPL 1997), and the in-scope move is to name the unit in the declared contract | **driver-spec §12:405-428, via a §15.2:522-524 capability gap.** The §15.1:511 proof obligation is discharged; the capability that would feed the proved predicate does not exist. Deferral must be tiered under §15.1:504-505 |
| `FS-ISOLATION-1` | `audit_blindness` (`:1644-1668`) implements driver-spec §8:330-332 and the port defers it. **WIDENED at Rev 18, and the row understated it.** §8 names TWO structural mechanisms, not one. The audit is the second; the first is §8:336-337's pristine copy, which the agent checks its own work against and which the port does not provision at stage M. The port therefore has NEITHER, and §8's first paragraph requires independence that is structural rather than instructed. **One of the two is not expressible in LLMLL today**: `wasi.proc.run` takes no confinement parameter and `LLMLL.md`'s effect summary names it an opaque boundary at the top element, so the capability clause bounds the parent and says nothing about the child. `FS-COPY-1` above shipped `wasi.fs.copy` for §8:336-337 and 4e used it for backup and restore instead | Nothing in Phase 4; like `--status`, a spec obligation deferred as operator surface, and disclosed under §15.1:504-505 rather than dropped. **The disclosure owed at phase close is now two sentences, not one**: the audit is deferred, AND the pristine copy is absent at stage M. A confining spawn is a language question rather than a port one; `theory-questions.md` Q-008 records it |
...
**Rev 17 → Rev 18, the fifth professor round, and the only one to overturn a shipped sub-phase.**
The round opened as a plumbing question, whether stage M shares the campaign's argument template,
and closed on driver-spec §8. Three findings survived and one of the author's own arguments did not.

- **The professor supplied the decisive reading and the language-team had argued the weaker one.**
  The language-team reasoned from §8's prohibition list, which is inferential. §8's third paragraph
  defines a declared input directly, by requiring the driver to report any file that was not among
  its declared inputs. The two reached the same conclusion from different paragraphs of one clause,
  which is the agreement-across-surfaces signal this log exists to record.

- **The professor widened the scope and the widening is the finding.** The language-team framed the
  defect as a missing prompt. §8 names two structural mechanisms and the port has neither, which
  makes this a conformance gap rather than an ergonomics one. `FS-COPY-1` shipped the builtin for
  §8:336-337 and 4e used it elsewhere.

- **The professor's open question rested on a premise `LLMLL.md` refutes**, and the language-team
  checked it rather than answering it. `wasi.proc.run` has no confinement parameter, and the effect
  summary names it an opaque boundary at the top element. The port cannot be structurally stronger
  than the reference, so it reproduces the reference's posture and files the gap. Q-008 records what
  a confining spawn would cost.

- **One language-team argument is withdrawn.** It justified the repair partly by the retry-budget
  comparison class. The clause 2 pre-registration makes that class agent-determined and null on
  divergence, so repairing stage M makes it readable and not informative. The argument was removed
  rather than softened, because leaving it in hands a reviewer a true objection to a correct
  proposal.

**Rev 12 → Rev 13, the §3.6 census.** Not a professor round. The largest owed item in the phase for
three sessions, closed by computing what had been derived once by hand and keyed to an epoch. The
instrument is [`halt_census.py`](../../experiments/rfc-swarm/tools/halt_census.py); the leg is
F-21 through F-24 in the RFC-SWARM Phase 4 harness findings.

- **§3.6's table is replaced, not corrected.** All five of its rows are undecidable on their own
  terms, because line keys cannot be followed across an epoch. At HEAD `:811` and `:815` are plain
  `require` calls at pre-write, the opposite of the old first row on both axes, and that is evidence
  about the keys rather than about the row. Rev 10's refusal to renumber is vindicated and is kept
  as history.
- **The disagreement count is thirteen, not one**, over a population of 88 rather than 46. §3.6's
  eliminative-check argument survives and strengthens: the disagreement is a population, not an
  anomaly.
- **"Six of the nine spec-defined sites are corroborated" reproduces exactly**, and is invariant
  under all three readings of "wrote some artifacts" and under the doubling of the population when
  the instrument learned to follow method calls. It is the only figure in §3.6 that does not depend
  on which halts the instrument can see, which is why it outlived every key around it.
- **§3.6.1's "why no census would have found this" is REFUTED**, and the refutation is the most
  useful thing in the leg. A per-site census cannot represent one line with two dispositions; a
  census over a three-value lattice can. The instrument was not told the site existed and found
  **twelve** instances. The limitation was in the census this document imagined, not in censuses.
- **The reading was already settled and the census implements it** rather than choosing it: "wrote
  some artifacts" counts only a stage's own declared outputs, per §3.6.1's stage E case. That is
  load-bearing, since the choice moves 29 of the 88 sites.

**Rev 11 → Rev 12, the no-toolchain test tier.** Not a professor round. 4c shipped with nothing in
the pytest tier, so every check it added needed a built binary; writing that tier refuted **one
claim this document made at Rev 11 and two readings the port had of its own registry.**

- **Rev 11's own F-20 amendment was short by one, and it is corrected above.** The census is
  **four** sites, not three: `check_audit`'s `audited` is a fourth of the same shape and belongs to
  **stage G2**, outside 4c. Three readers in a row have now stated this count short, each
  enumerating the artifact they were porting, which is why the census is now **computed** by a test
  that fails on a fifth site rather than restated in prose. A count that has been wrong three times
  is not a fact to be re-checked, it is a fact to be derived.
- **Two readings of `registry.stage-tag-count` were both refuted.** It is neither the number of
  declared outputs nor the number of static agent call sites: stage D declares two outputs and holds
  **one** `agent.run` call site executed twice by a loop over `("a", "b")`, while stage K holds two
  call sites and declares one output. The relation that holds is the loop.
- **A forward hazard, now asserted rather than noted.** `stage-tag-count` is `(if (= i 3) 2 1)`,
  correct for everything ported and wrong for **K, L and M**, all three multi-invocation and all
  three unported. The test fails the moment one of them is marked ported without the table moving,
  which is the only moment the information is worth anything.

**Rev 10 → Rev 11, the 4c port.** Not a professor round and not a harness leg: **two amendments the
implementation earned**, both to §9.2, neither changing what 4c does and neither touching a proved
post. The sub-phase shipped against Rev 10 and is merged; this revision reconciles the document to
what building it established.

- **§9.2 item 1 was silent on `ConditionUnmet`, and the silence read as denial.** The item settles
  what 4c does not construct and stops, which invited "4c builds 4b's two arms." It builds three:
  `check_dispositioned:493` is `require_spec` rather than `require`, so §3.5's rule makes its halt
  spec-defined and it records `stopped`, which the rig asserts under mode `bad-barrier`. **The
  interesting part is what did not move.** `[V7-ONLY-TWO]` is a property of `verdict-outcome`'s
  codomain rather than an invariant of the port, so it stays true as written and `validate.llmll`'s
  prose was left alone: a shipped proved module is not where a later sub-phase's arithmetic goes.
  The barrier halt takes the sequencer's `halt-with … ConditionUnmet` channel instead, which is why
  `[V7-NO-STOP]` is not violated by a third arm existing.

- **F-20's tolerance is at three sites and the third is over a different artifact.** `:482` inside
  `check_dispositioned` tolerates a bare list where a dict with `rows` is expected, the same shape
  as `:713` and `:732` over `core_ids`. **The generalization is the shape, not the artifact.** The
  n=0-live-runs null result covers `core.json` only; a bare-list dispositions document is a separate
  unmeasured question, and calling `:482` "F-20's tolerance" without the distinction is how a
  narrowing gets applied to two arms and called complete.

**One document defect this revision does not repair.** The port's own plan cited the barrier
assertion as `test_rfc_pipeline_integration.py:334`, which is a **docstring line**, not the assert.
That is §3.5.1's rule ignored one sub-phase after it was written: cite the condition or the test
name, never the line. The amendments above cite the test by name.
