...
| **R-5** | `scripts/doc_path_lint.py` carries a DELETE-THIS-ROW `ALLOW` entry for `scripts/doc-claims/do-discard-on-final-step.llmll`, a fixture never created under that name. The sibling that exists is `do-notation-discard-marker.llmll`. Two citations in `driver-ll-phase01-implementation-plan.md` are stale as a result. The rule itself **does** have a test (`Spec.hs`, the `do-discard-final` case). | drop the row with the doc fix |
