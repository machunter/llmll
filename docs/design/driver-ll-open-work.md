...
status: "ACTIVE ROUTING RECORD, opened 2026-08-03. Its body describes the v0.14.80 and v0.14.81 line and is correct for that line, and sections 0 and 1 are a SNAPSHOT of that line rather than current state: the tree is now v0.23.0 with schema const 0.11.0. REMEASURED 2026-08-18, and again 2026-09-10. TWO CHANGES SINCE 2026-08-18. First, HTTP-GET-1 SHIPPED at v0.21.0 (dc2c9cf), so one of the three blockers this file filed is CLOSED and the stage A STOP it caused is lifted; stage A ported at v0.21.2 (3e1c1af). CAP-1-REAL and CONSOLE-INIT-1 are both still OPEN in Active Items, at markers PLAN and DECIDE. Second, R-3 CARRIED A WRONG CAUSE for three weeks and is corrected in place: the totp_filled failure is not a WILD-ASSUME or FACT-AG-LEN defect, it is the discarded returns key now filed as AST-RETURNS-KEY-1, and the fix is a six-key rename in the example. R-11 was promoted to HTTP-GET-1 (now shipped) and R-13 closed at v0.14.81. The routing is what is live, not the release narrative around it."
date: 2026-08-18
...
| **R-3** | `examples/totp_rfc6238/totp_filled.ast.json` fails `check-examples.sh`. Pre-existing, unrelated to this campaign, and invisible because of R-1. **CAUSE CORRECTED 2026-09-10; the original text named the mechanism that fires and not the cause, and it read "a `bytes[20]` WILD-ASSUME error from the FACT-AG-LEN line", which would send a reader into the wrong module.** WILD-ASSUME does fire, and it fires correctly: `dynamic-truncate` takes `hmac-result: bytes[20]` and the argument's type is `?`. **But the author DID declare the callee's return type.** All six definitions in that file carry a `returns` key with a null `return_type`, and `hmac-sha1-wrap` declares `{"kind": "bytes", "length": 20}` through it. The parser DISCARDS a `returns` key on a definition head, so the declaration never reaches the checker, which then instructs the author to write the annotation they had written. Measured: renaming those six keys to `return_type`, changing nothing else, moves the file from the error to `SAFE (liquid-fixpoint)`. **Owned by `AST-RETURNS-KEY-1` (G3), filed 2026-09-10**; record: [`ret-resolve-step2-baseline.md`](ret-resolve-step2-baseline.md) §8. Not a `WILD-ASSUME` defect and not a `FACT-AG-LEN` defect. | 165 passed / 1 failed |
