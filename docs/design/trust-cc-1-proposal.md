...
> **CLOSED at v0.23.7 (`04c8ab3`), and two claims below are now historical rather than current.** `SIDECAR-ADMIT-1` shipped on the reading this section gave it, with one correction. **Fixed:** the guard is keyed on the tier claim rather than on `erBodyFaithful`, so the record described here is hash-checked and downgraded, and the "Not caught" row of §12.1 no longer describes `verify --trust-report`. **Unchanged and still correct:** §12.2's account of the early exit, which is why part 2 shipped as disclosure rather than as a check, and §12.1's exit-status row, because a downgrade is not a failure and the run continues on the demoted value. **Corrected:** §12.3 part 1 expected the widened contradiction trigger to be what catches a forged record. It is not. The tier-keyed guard repairs the record first, so the contradiction pass guards only the narrower case of a compiler-version change that moves the fragment without moving the semantics tag, and its only witness is a unit test. Part 3 shipped as exit code 4, escalating from success only, and it does not fire on any single-version run for the same reason. The "sound modulo trust" question this section declines to answer was answered in the closing design turn: the sidecar hash is a member of the trusted base that `TRUST-BASE-1` will declare, so a record that skipped the hash check was outside the stated position rather than an exception to it.
...
**Folded 2026-09-09 at v0.23.0 (DOC-CONSOLIDATE M2), on settlement.** The review
below stood as `docs/design/trust-cc-1-review.md` through two rounds. Round 1
reviewed Rev 0 and produced Rev 1; round 2 reviewed Rev 1 and produced Rev 2.
Both rounds ADOPTED the proposal, and neither found a soundness objection to the
retirement. The standalone file moved to
[`../archive/professor-reviews/trust-cc-1-review.md`](../archive/professor-reviews/trust-cc-1-review.md);
a redirect stub stands at the old path for one release cycle.

> **One correction the implementation forced, recorded here rather than in the
> body above.** Section 7 case 6 says two things that do not agree. Its first
> sentence says the marker "is available" because the emit result runs before
> the solver. Its third sentence says a render that did not run the emitter
> shows no marker. Section 12.2 measured the answer: on a plain
> `llmll verify --trust-report` the emitter never runs, and no `.fq` is written.
> **The shipped implementation follows section 12.2** — `buildTrustReport` is
> pure and never receives an `EmitResult`, so the marker is applied post-emit by
> `markBodyFallback` and a plain `--trust-report` shows none. The first sentence
> of case 6 should be struck. That is a body edit and belongs to the
> language-team, not to the documentation-lead who folded this appendix.
