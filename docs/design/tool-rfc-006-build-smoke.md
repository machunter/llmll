...
| Nothing constructs a list from a count, so a 12000-line stdin fixture cannot be built in-language | **SHAPES**, predicted | `LIST-RANGE-1`, filed 2026-08-15 | `list-range` plus the existing `int-to-string` and `list-map`. The PROC-STDIN-1 stage will instead generate its fixture with one `awk BEGIN` program through `wasi.proc.run`, which is the same technique the SUBJECT already uses at its own `big.txt` fixture, so the bytes match and only the generator differs. **Predicted rather than measured**: that stage is not yet ported |
