...
> **Version:** Rev 0.3 — see revision notes.
...
> **Rev 0.3 (ABANDONED):** language-team adjudication — abandon, do not redesign, no professor turn. The only firing formulation (`:intentional ∧ inconsistent ∧ functionVerifies`) is co-extensive with two fields already emitted in the same `discriminative_axis` block (`spec_entropy_annotation: intentional` + `spec-too-tight-for-omega`), so a dedicated warning carries **zero new information**; it would also reverse decision 2 and re-add the redundant noise that decision rejected. F-002 stands settled with **no follow-on mechanism** (the correct terminal state). Retained in `docs/design/` as a cautionary record — see the Meta-lesson.
> **Date:** 2026-07-03
...
> **Status:** **Abandoned (Rev 0.3)** — the design space collapses to an already-emitted two-field conjunction (zero new information); F-002 stands settled with no follow-on mechanism. The `⚠ BLOCKING FINDING` + `Meta-lesson` below are the durable record. Retained in `docs/design/` (not archived) for discoverability of the meta-lesson. The Restatement/Design/Appendix-A below are the preserved (flawed) Rev 0.1 record.
...
- Net: on contact with the actual model, "expiring `:intentional`" largely collapses into "read `spec_entropy_annotation` + the existing too-tight warning."

**Resolution (Rev 0.3): ABANDONED.** Language-team adjudicated redesign-vs-abandon and chose abandon. The only firing formulation (`:intentional ∧ inconsistent ∧ functionVerifies`) is *co-extensive* with `(spec_entropy_annotation == "intentional") ∧ ("spec-too-tight-for-omega" ∈ warnings)` — both already in the same `discriminative_axis` block — so a dedicated W614 pre-computes a one-line client-side conjunction over two already-emitted fields and carries **zero new information** (thinner even than the reason-string's Slice-1, which at least minted a new artifact). It would also reverse decision 2 and co-fire the redundant noise that decision explicitly rejected. **F-002 stands settled with no follow-on mechanism** — the correct terminal state, since `:intentional` staleness is already observable from the two co-emitted fields. No professor turn (the value is thinner than what the professor already weighed for the reason-string). **The conditional that would flip it** (for the record): a committed CI consumer that specifically wants the pre-computed boolean and finds the two-field conjunction a real ergonomic cost — then the cheapest realization is a `TrustReport.hs`-emit boolean `intentional_stale := (annotation == :intentional) ∧ (WarnSpecTooTightForOmega ∈ warnings)`, **not** a new `CDPWarning` constructor. No such consumer exists today.

## Meta-lesson (why this slipped past three passes)

Each pass verified **input-availability** — are `identityOk`/`constOk`/`score` computed and in scope at the emit site? — and none verified **predicate-satisfiability** — can the guard's conjunction be simultaneously true? The tell was in this very document: the edge-case table's *firing* row described "real score, no trivial body satisfies," which is the impossible conjunction stated in prose, and no pass was ever required to exhibit a **concrete witness** (an actual `def` + post + candidate set) that makes the guard fire. Both this proposal and the reason-string listed their *quiet* cases exhaustively and left the *fires* case abstract — which is exactly where both the imagined value and (here) the impossibility hid.

**Discipline change (proposed, for the workflow — routes to doc-lead/UPDATE-PROTOCOL if adopted):** any proposal introducing a boolean guard over computed values must include a **positive-witness row** in its Edge-cases section — the minimal *concrete* input that makes the guard fire — and the compiler-engineer feasibility pass must **construct that witness**, not merely trace that the inputs are in scope. It is the design-workflow analogue of "write the failing test first": a firing predicate with no constructible witness is unsatisfiable by inspection.
