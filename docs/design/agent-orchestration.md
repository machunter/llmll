...
> **Status update (2026-05-13, v0.10.4).** The Phase-1 Python orchestrator described below (`## Implementation Language` → Option A; `## Recommended path` → Phase 1) has shipped in experimental form as the `experiments/repair-loop/` harness (`experiments/repair-loop/scripts/run_repair_loop.py` + `experiments/repair-loop/scripts/run_matrix.py`, ratified in v0.10.4 via commits `90a5bb9` + `5895792`) and the `experiments/minimal-agent/` harness (first-round-effectiveness measurement). The orchestration-loop diagram in `## The Orchestration Loop` and its pseudocode match the harness's actual shape — read those sections as the realized Phase-1 contract, not future work.
>
> **Phase 2 (self-hosted-in-LLMLL orchestrator) is a large, still-deferred effort.** The original feature freeze that once gated it was retired (2026-07-10), so Phase 2's prerequisites are no longer blanket-prohibited — each goes through the normal design → review → ship pipeline with its own soundness argument. The prerequisites remain substantial and cross several categories that each need explicit design: `json-parse` as a new §13 builtin (Feature-gap-analysis table), a Haskell-FFI alternative as a new FFI use (same table), and `wasi.http.post` / `wasi.fs.read` / `wasi.io.stdout` as WASI capabilities (same table). Phase 2 stays deferred on scope, not on a freeze.
>
> **Spec-context flags for any reader using this doc as input to a v0.11+ proposal.** **(1)** `def-invariant` (in `## Implementation Language` → Option B "Why this matters") is in the spec at `LLMLL.md §11.4` as "designed, not yet implemented" — the parser accepts it (reduced to `SDefLogic`) but Z3 invariant verification on AST merge is deferred. The orchestrator's contract examples in Option B are aspirational Phase-2 illustrations; they would run only after §11.4 enforcement lands. The `invariant-discovery-proposal.md` + `invariant-discovery-review.md` pair in this folder is the active design-pipeline lineage toward that work. **(2)** `HoleInfo` / `HoleResult` sketch types (in `## Implementation Language` → Option B) predate OBLIG-1's `CheckoutToken` expansion (v0.10.0, `compiler/src/LLMLL/Checkout.hs:140`) and the current `HoleEntry` shape (`compiler/src/LLMLL/HoleAnalysis.hs:76`); a Phase-2 entry condition is regenerating these against current compiler emits. `?scaffold` (Q4) and "Lead Agent" framing (`## The Orchestration Loop` opening) are *not* drift — `?scaffold` ships per `LLMLL.md §6.1`, and the "Lead Agent" / sub-agent vocabulary is correct for the orchestration question, distinct from the experiment harnesses' single-agent-under-test framing. Q1 and Q2 in `## Open Questions` are partially answered (experiment harnesses landed as separate Python packages; `llmll holes --deps` flag shipped in v0.10.1). Q3, Q4, Q5 remain open.
