...
it is sealed and opaque: no `Json` VALUE enters a body-faithful VC, because `Json` is neither scalar-like nor a measure sort and so receives no binder. Measured 2026-09-18: a `def-shell` taking `j: Json` beside an int IS body-faithful, and the emitted `.fq` binds the int and `result` only. **The `list[a]` comparison this sentence drew is BACKWARDS, and is corrected here rather than carried.** A `list[a]` parameter DOES enter a body-faithful VC: it binds at the opaque `Lst` carrier, and `listLen` over it reflects into the goal, so `(post (= result (list-length xs)))` is PROVED and not assumed. What `list[a]` cannot expose is its ELEMENTS, and that opacity is the property `Json` shares with it, not the absence of a binder. **This correction closes R-D of `native-json-proposal.md`**, which found the same defect, ran the same measurement and named these exact sites; `native-json-review.md` F8 found it independently. Three reading paths, one conclusion, and this is the first pass to change the sentences rather than record that they are wrong. The
...
| **JSON-1** (a sealed `Json` builtin, `def-shell`-only) `[CT][SPEC]` | **OPEN** — depends on EFFECT-RESP | The driver has 18 JSON call sites and LLMLL has no JSON and no dynamic type. `json-parse : string -> Result[Json, string]`, `json-serialize`, typed `Result`-returning accessors. `Json` is recursive, which is Lever C for verification and irrelevant here because it is **sealed and opaque**: no `Json` VALUE enters a body-faithful VC, because `Json` is neither scalar-like nor a measure sort and so receives no binder. Measured 2026-09-18: a `def-shell` taking `j: Json` beside an int IS body-faithful, and the emitted `.fq` binds the int and `result` only. **The `list[a]` comparison this sentence drew is BACKWARDS, and is corrected here rather than carried.** A `list[a]` parameter DOES enter a body-faithful VC: it binds at the opaque `Lst` carrier, and `listLen` over it reflects into the goal, so `(post (= result (list-length xs)))` is PROVED and not assumed. What `list[a]` cannot expose is its ELEMENTS, and that opacity is the property `Json` shares with it, not the absence of a binder. **This correction closes R-D of `native-json-proposal.md`**, which found the same defect, ran the same measurement and named these exact sites; `native-json-review.md` F8 found it independently. Three reading paths, one conclusion, and this is the first pass to change the sentences rather than record that they are wrong. Rejected alternative: `haskell.aeson` through the FFI tier, which collapses authority to ⊤ and carries an unchecked signature. |
