...
| 5 | **`consumed_guarantees` (checkout brief)** — the channel drops imported callees entirely | `Main.hs:1951` — checkout `cenv = buildContractEnv stmts` (same-file; XMOD-AG made only the verify path cache-aware) | **OPEN (confirmed)** — brief-completeness, not soundness | binary check: a same-file callee appears in the brief, the imported one is absent. Fix: cache-aware checkout `cenv`. (The `callee_tier` keying in `ObligationAssembly.trustLabel` is untested — the callee is dropped before it.) |
...
**Residual (confirmed by binary check):** the `checkout` brief's `consumed_guarantees` channel uses same-file `cenv` (`Main.hs:1951`), so an imported callee's guarantee is **absent from the brief** — a same-file callee appears in the control, the imported one does not. Brief-completeness / DX, not soundness (the guarantee is still consumed in the verify VC, which XMOD-AG made cache-aware; only the checkout path was not). Fix: seed the checkout `cenv` from the module cache like `emitFixpointWithCache`. (The downstream `callee_tier` keying in `ObligationAssembly.trustLabel` is untested — the callee is dropped before it.)
