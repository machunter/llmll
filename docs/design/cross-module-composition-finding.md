...
**Residual (sibling channel, observed during the v0.14.18 e2e, tracked as XMOD-SCOPE-BRIEF):** the same brief's `available_functions` (built from same-file statements) and `in_scope` (`runSketch` over `builtinEnv`, not cache-seeded) still omit imported names — an `(open)`-ed callable's contract is invisible in those channels even though `consumed_guarantees` now shows it when the enclosing body calls it.
