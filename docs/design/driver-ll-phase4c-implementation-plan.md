...
status: "IMPLEMENTED AND MERGED. This field said `not yet merged` from 2026-08-06 until 2026-08-18, when it was measured rather than remembered: `git rev-list --count main..driver-ll-4c/stages-d-f-g` is 0, so the branch holds nothing main does not. The work is commit b9904a6, first contained by tag v0.14.88. Stages D, F and G are ported and the acceptance cover runs them: 39 passed, 0 failed (31 before). THE RELEASE HAS NO CHANGELOG ENTRY: `sub-phase 4c` appears zero times in CHANGELOG.md, so this file and the commit are the only record of what shipped. The stale branch was deleted on 2026-08-18."
date: 2026-08-18
