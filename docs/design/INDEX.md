...
| [finding-fq-result-sort-default.md](finding-fq-result-sort-default.md) | The `result` binder's sort defaults to `int` when the return type is unannotated; bool, string, and pair returns crash | **OPEN, Rev 2 settled** (FQ-RESULT-SORT-1, alias FQ-BOOL-SORT-1); fails closed, blast radius zero in tree |
| [finding-fq-result-sort-default-review.md](finding-fq-result-sort-default-review.md) | Professor review of FQ-RESULT-SORT-1, Rev 0 and Rev 1 | **Two rounds**, standalone, not yet folded |
