...
| [finding-fq-result-sort-default.md](finding-fq-result-sort-default.md) | The `result` binder's sort defaults to `int` when the return type is unannotated; bool, string, and pair returns crash | **FIXED (stages a+b)** (FQ-RESULT-SORT-1, alias FQ-BOOL-SORT-1); Rev 3 withdrew HOLE-RET and accepts the recursive-then residual |
