...
| [tool-rfc-002-refute-crux.md](tool-rfc-002-refute-crux.md) | The 80 frozen verify verdicts in LLMLL: no BLOCKS gap, and a two-binary split (harness vs graded compiler) that 001 did not have | **Rev 0, DRAFT**, state `blocked`: the first port written RFC-first, three policy calls asked before the code |
...
| [driver-ll-phase4-proposal.md](driver-ll-phase4-proposal.md) | Eleven agent-delegated stages, the serial wave, and two checkable oracles | **Rev 12, SETTLED**: 4a (`2b82464`, v0.14.85) and 4b (`ba2f93d`, v0.14.87) shipped, 4c merged with its release ceremony held, 4d-4f open; Rev 12 corrects a census Rev 11 stated short, and no revision moved a proved post |
