...
| [cross-module-assume-guarantee-proposal.md](cross-module-assume-guarantee-proposal.md) | Body-faithful verification across `import` boundaries: seed the body-VC ContractEnv with imported contracts (plumbing gap, not a boundary); prerequisite for a modular flagship | **Shipped (v0.14.17)** — Rev 1; body-VC ContractEnv seeding (`emitFixpointWithCache`); XMOD-TIER already covered tier/staleness |
