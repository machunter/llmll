...
| Document | Summary | Status |
|---|---|---|
| [tool-ll-RESTART.md](tool-ll-RESTART.md) | Where the TOOL-LL work is: release state, gates measured, findings not to rediscover, 005 next and blocked | **LIVE** — read before the campaign doc; state authority, delegates the standard; written in ASD-STE100, and its §0 gives the four commands that measure state |
| [llmll-tooling-campaign.md](llmll-tooling-campaign.md) | TOOL-LL: the six CI gates rewritten in LLMLL and actually deciding, with the RFC standard and the gap discipline | **Rev 3, IN FLIGHT**: scope, distribution and retirement settled 2026-08-07; **4 of 6 ported** (001/002/003/004, all `oracle`); P1 tag debt cleared; **005 NEXT, unblocked at v0.14.96** by `REGEX-LOWER-1`, which took the critical path through the compiler for one release |
