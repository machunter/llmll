...
| [proc-boundary-1-proposal.md](proc-boundary-1-proposal.md) | argv as `wasi.proc.args`, terminal status as a `def-main` projection; no catalog growth | **Rev 3, SETTLED, shipped v0.14.85**; §6.3's `tcWarn` is owed, so roadmap row `PROC-BOUNDARY-1` stays open |
