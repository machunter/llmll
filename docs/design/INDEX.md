...
| [data-scope-lever-a-arrays-proposal.md](data-scope-lever-a-arrays-proposal.md) | Lever A: QF array theory for `bytes[n]`/`map[k,v]` — index-in-bounds + get-after-put in `Σ_auto` | **Rev 1.1** — review + feasibility folded (int-only keys v1; int-0/1 presence array) — A0/A1 GO, ready to build |
| [data-scope-lever-a-feasibility.md](data-scope-lever-a-feasibility.md) | Compiler-engineer A0/A1 feasibility read — probe-backed: fixpoint speaks `Map_t`/`Map_select`/`Map_store`/`Map_default` natively, both verdicts; bool-element arrays crash the SMT bridge → int-0/1 presence amendment | **A0 GO · A1 GO · A2 GO-WITH-CHANGES** — probes p1–p9 |
| [data-scope-lever-a-arrays-review.md](data-scope-lever-a-arrays-review.md) | Professor review of Lever A Rev 0 — proceed-with-revisions; F1–F3 gate Rev 1 (refutation-completeness discipline), F4 gates A2; Q1/Q2 resolved as proposed | Review — folded into Rev 1 (appendix); candidate for archive sweep after A1 ships |
...
| [string-literal-distinctness-proposal.md](string-literal-distinctness-proposal.md) | Reflect string literals as interned `Str` constants + ground pairwise-distinctness so literal comparisons verify (was Advisory); enables A2.2-string + string keys | **Rev 1** — professor-folded; Stage 1 ready to build, Stage 2 (length pinning) documented as next |
...
| [cascading-refinement-proposal.md](cascading-refinement-proposal.md) | Agent-driven recursive hole decomposition: `refine` op + growing tree + CDP/feasibility gates + Option-3 cycles | **Rev 3 — Stages 1–3 SHIPPED v0.14.13, Stage-4 REFINE-REUSE SHIPPED v0.14.29**; Option-3 cycles reconciled to REC-PARTIAL-MARK's `termination_unverified` marker (2026-07-10); feasibility (no-miracle) gate + decomposition-trust meet open |
