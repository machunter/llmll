...
| [proc-boundary-1-proposal.md](proc-boundary-1-proposal.md) | argv as `wasi.proc.args`, terminal status as a `def-main` projection; no catalog growth | **Rev 1, SETTLED, READY FOR ENGINEER**; promoted to roadmap row `PROC-BOUNDARY-1`, which sub-phase 4a is blocked on |
