...
- **`CRYPTO-2`** — open, and **unrouted**: it has no roadmap row. It is filed
  only as `R-12 REPORT-GATE-1` in
  [`driver-ll-open-work.md`](driver-ll-open-work.md), which also has no roadmap
  row. The retraction it records still holds on measurement:
  `rg 'assumed_axioms|stub-backend' compiler/` returns zero hits.
