...
> **Version:** Rev 2 — stage-3 feasibility read + observational build folded (Appendix B). Rev 1 folded the professor review (Appendix A). Rev 2 corrects three things the feasibility read surfaced: **(1)** the distinguishing witness comes from **stage-2 Ω-replay, not a stage-3 solver model** (the pipeline has no counterexample-model extraction); **(2)** the stage-3 gate must also exclude **branch-skolem** bodies (a second skolem door beyond contracted `EApp`); **(3)** stage-3 emission mechanics are cheap (~150 LOC), so stage-3 is repositioned as a **deferred SAFE-direction equivalence prover**, not the witness path — observational stages 1–2 carry the thesis and are now **built**.
> **Status:** Proposed (Rev 2). **Stages 1–2 BUILT** (worktree `agent-ad8a0f2d2cab5dd48`, `stack test` 1035/0, end-to-end verified with the real solver; uncommitted). **Stage 3 DEFERRED** (NO-GO as scoped; optional SAFE-direction prover, §3/§5).
> **Date:** 2026-07-04
