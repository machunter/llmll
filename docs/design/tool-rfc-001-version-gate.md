...
| `string-split ""` did not terminate | SHAPES | `SPLIT-EMPTY-1`, **SHIPPED v0.23.16**: an empty separator answers `[subject]`. The port keeps the fold below | A fold over the string's characters. Instead: a fold over a *literal index list* bounded at 24, with a saturation check so the bound cannot fail silently. |
| no character decomposition, no ranges | SHAPES | `SPLIT-EMPTY-1` (one row with the entry above). **This half was REFUTED 2026-08-16**: `range` shipped at v0.11 and decomposition composes as a `list-map` over `range` with `string-char-at`, which `LLMLL.md` §13.5 prints. The defect half SHIPPED v0.23.16 | A character fold. Instead: the same bounded index-list fold. Filed as one row because a `string-split ""` that terminated would also be the decomposition. |
