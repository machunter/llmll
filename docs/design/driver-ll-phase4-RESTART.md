...
Read this first, then [`driver-ll-phase4-proposal.md`](driver-ll-phase4-proposal.md) (**Rev 11**,
...
**Rev 11**, which is Rev 10 plus the two amendments the port earned and no change to what 4c does.
...
| **27** | **A no-toolchain test tier for 4c** | compiler-engineer | **PENDING, new at this session and found by writing task 25.** 4c added nothing to the pytest tier: the planned `test_driver_ll_4c.py` does not exist and pytest is unchanged at 132, so **every check 4c added needs a built binary** and while CI is unavailable 4c has no automated coverage at all. `test_driver_ll_4b.py` is the shape to follow |
...
**What 4c does not owe.** No change to what it does: Rev 11's two amendments reconcile the document
to the port and move no code. No
...
| **F-20's tolerance is at three sites** | F-20 named `:713` (stage F) and `:732` (stage G), both over `core.json`'s `core_ids`. `:482`, inside `check_dispositioned`, is a third **over the dispositions document's `rows`**, so the generalization is the shape and not the artifact | **CLOSED at Rev 11**, and sharpened by executing the citation: the n=0-live-runs null result covers `core.json`'s two sites only, and a bare-list dispositions document is a separate unmeasured question |
