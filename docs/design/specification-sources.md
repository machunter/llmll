...
> **External empirical anchor (added 2026-05-25, language-team revision per [`positioning-constraint-decay-proposal.md`](../archive/shipped-design-specs/positioning-constraint-decay-proposal.md) Rev 1).** Dente, Satriani, and Papotti (arXiv 2605.06445, May 2026) operationalize this spec-source pathway directly in their *Constraint decay* benchmark — they fix a single OpenAPI contract (RealWorld Conduit) and layer non-functional constraints around it, measuring how agent compliance degrades as the constraint stack grows (~30 percentage points L0→L3, ~46% of failures concentrating in the data layer). The fixed-contract premise validates the input-shape side of this section's bet: when the functional spec is external and explicit, the failure surface concentrates in non-functional constraint compliance — which is the surface `--trust-report` and the three-channel obligation report are designed to expose. See [`strategic-positioning.md`](strategic-positioning.md) §"External Empirical Anchor — Constraint Decay" for the full positioning treatment, and [`language-comparison-experiments.md`](language-comparison-experiments.md) §"Deferred External Benchmarks" for the recorded scope position on whether to reproduce their benchmark on LLMLL.
