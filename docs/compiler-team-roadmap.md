...
| **Non-int refinement widening** (`EVar-refined-string`, `EVar-refined-list` in strict core) | **P2 — open** | LT proposal after REF-META-3 settles; engineer build gated on `FixpointEmit.hs` path-a axiomatization (`string-length`/`list-length` as uninterpreted integer-valued functions). See [`docs/design/v0.12-direction.md §2`](design/v0.12-direction.md). |
