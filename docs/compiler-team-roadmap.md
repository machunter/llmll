...
| **F-NIW-3** (qualifier-identifier sanitization) | **Open (pre-existing)** | liquid-fixpoint rejects `-` in qualifier identifiers; a hyphenated function name with a comparison pre/post (`Q_measure-word_post_NN`) crashes the solver. Not introduced by NIW — surfaced by an NIW fixture. Engineer: sanitize function names in `extractQualifiers` qualifier-name construction. |
