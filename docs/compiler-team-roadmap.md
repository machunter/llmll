...
> **XMOD-COMP (cross-module verified composition) — goal reached.** Layers 1–3 shipped earlier (admission / type-alias type-check / trust-tier edge); layer 4 (body-VC emission — including the imported refinement-aliased-param case, verified body-faithful end-to-end) by **XMOD-AG (v0.14.17)**. Sole residual: layer 5 (`consumed_guarantees.callee_tier` bare-vs-qualified keying in `ObligationAssembly.hs`) — a reporting-surface item that does not block the `verified` tier. See [`cross-module-composition-finding.md`](design/cross-module-composition-finding.md).
