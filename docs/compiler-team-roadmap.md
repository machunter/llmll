...
| **REC-BODY-VC** (own body VC for recursive `def`/`letrec` at partial correctness) `[LT→CT]` | **Rev 1 design settled — [`rec-body-vc-proposal.md`](design/rec-body-vc-proposal.md); (b0) SHIPPED v0.14.22** | Baseline overturned the row's premise: own-contract assume-guarantee already ships (v0.14.13 §0.1). Restructured into (a) partiality marker, **(b0) def-form in the evidence hash — SHIPPED v0.14.22** (closes probe-E laundering), (b1) fail-closed default, (c) `(decreases …)` call-site strict descent. Sound at partial correctness; flips the one-pager rows "`EApp` (recursive self)" and "`letrec` (own body VC)". Motivation sharpened by F-1 (`examples/secure-channel-emergent/`): a degenerate self-call fill verifies SAFE today, guarded only by harness policy — with REC-BODY-VC + **R7** strict descent it becomes a solver refutation (no decreasing measure), retiring TERM-1's disclaimer. Remaining: (a) partiality marker + spec reconciliation → (c). |
