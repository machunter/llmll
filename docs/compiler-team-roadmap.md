...
| **XMOD-ALIAS** (cross-module refinement-aliased-param seeding) `[CT]` | **Open — fail-closed completeness follow-on to XMOD-AG (v0.14.17)** | Seed the body-VC alias map's *refinement* aliases (not just `STypeDef`s) from imports, so a cross-module caller of a function with a refinement-aliased param (the `xmod-alias safe-withdraw` fixture) verifies body-faithful instead of falling back. Fail-closed today (unresolved imported alias → contract-only fallback, never mis-verify), so completeness not soundness — the `cenv` seeding shipped in v0.14.17; only the alias-map seeding remains. |
...
> **XMOD-COMP (cross-module verified composition) — largely closed.** Layers 1–3 in v0.13.0 (admission / type-alias / tier-edge); the body-VC-emission layer by **XMOD-AG (v0.14.17)**, which dual-keys imported contracts into the caller's body-VC `ContractEnv`. Residual completeness is **XMOD-ALIAS** (refinement-aliased params, in Active Items). See [`cross-module-composition-finding.md`](design/cross-module-composition-finding.md).
