...
| **COMP-3b-general / COMP-4** (refinement-return discharge through nested / non-`Result` matches) | **COMP-3c + COMP-3b-general SHIPPED (v0.13.5 / v0.13.6); COMP-4 P2 — open** | **Shipped — COMP-3c** (finite-enumeration-as-bounded-int, v0.13.5): nullary-enum values + matches via the `desugarCtorValues` pre-translation desugar. **COMP-3b-general** (opaque-sum elimination, v0.13.6): a `Result`-typed *variable* match reaches body-faithful `verified` at any nesting depth via a binder-carrying `BranchVC` + `collectBranchBinders` + derived `SortEnv` payload-sort keys; the v0.13.3 top-level flat-`Result` special-case is subsumed. Both stay QF-LIA, exhaustiveness-only. **Remaining — COMP-4** (inductive-datatype theory): payload-bearing user ADTs / a payload-carrying `Rejected` variant for full illegal→`REJECTED` totality need SMT `declare-datatypes` + testers + selectors (escapes QF-LIA). The completeness step (consuming a matched payload's own refinement) and runtime/PBT evaluation of constructor values are the other open slices. Constructor-dependent posts stay a deliberate fallback. |
