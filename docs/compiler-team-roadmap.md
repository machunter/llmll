...
| **Non-int refinement widening** (`EVar-refined-string`, `EVar-refined-list` in strict core) | **Phase 1 shipped (unreleased) — elim-side** | Path-a axiomatization gate **cleared**: measure terms (`string-length`/`list-length`) emit as function-sorted uninterpreted functions with per-term ground range facts (REF-META-2 §4), discharged QF-LIA+EUF. Refinement-aliased params get carrier sorts and their predicate assumed in the body VC; stacked aliases conjoin (§3.4.4). Commits `1b21973` (IR) / `a0f352b` (measure values) / `0a3c5c2` (refined params). REF-META-2 §4 + REF-META-3 §4.2 + [`docs/design/v0.12-direction.md §2`](design/v0.12-direction.md). **Remaining: F-NIW-2.** |
| **F-NIW-2** (intro-side call-pre for string/list-refined params) | **Open** | Caller-side proof of a measure-refined param's predicate does not yet fire: the call-argument path translates `int` args only, and callers do not bind carrier vars passed to refined callee params. Sound today (callee verified-under-precondition; strict-core callers conservatively blocked) but caller-side checking incomplete for measure refinements. Refined `int` params already get full intro+elim. Engineer: carrier-arg translation + caller-side carrier binding. |
