...
| **DRIFT-CI-1** | **Content-satisfied (C1–C5 ✅); workflow YAML pending** | doc-lead Pass 7 (`faf1856`) + v0.10.8 release commits (`1585de2`, `5c6bdec`) brought README.md / LLMLL.md / CHANGELOG.md / package.yaml / llmll.cabal into banner equality at v0.10.8 (C1 + C2 ✅); schema `$id` (`https://llmll.dev/schemas/v0.5/ast.schema.json`) and `schemaVersion` (`0.5.0`) remain aligned with `ParserJSON.hs::expectedSchemaVersion` (C3 + C4 ✅); examples-inside-grammar reading (ii) round-trip verified Pass 6 (`623c46f`) against §12 / §4.4.5 (C5 ✅). Remaining residual: `.github/workflows/version-gate.yml` automation (infra/engineer scope) and a whole-spec round-trip harness for C5 (infra scope). |
...
| **REF-META-1** (refinement metatheory of record) | **LT proposal settled (Rev 1 landed)** | LT proposal at [`docs/design/refinement-metatheory-of-record-proposal.md`](design/refinement-metatheory-of-record-proposal.md) (Rev 1, commit `28df45e`, 2026-05-24): checking-mode intro+elim rule + six non-goals + tier-aware soundness statement; adopts amended critic's framing per [`docs/design/critique-2026-05-23-triage.md`](design/critique-2026-05-23-triage.md) §3.1. Doc-lead promotion target: `LLMLL.md §3.4 / §5`. Recommend batching professor review with LT-INV / LT-CDP / LT-PPR. |
