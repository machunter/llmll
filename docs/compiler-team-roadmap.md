...
| **F-NIW-4b** (let-bound non-call value into a later call's pre) | **Open** | A `let`-bound *non-call* (`SimpleVC`) value used in a later call's precondition is not yet threaded — `prependLB` pushes such bindings to the leaf, so `collectCallPreObligations` does not see them at the call point. No example exercises it; distinct from the call-result chaining fixed by F-NIW-4. |
