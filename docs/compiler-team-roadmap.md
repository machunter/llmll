...
> **Routing:** items below are organized into lanes by milestone target. Tags follow the `XXX-N` pattern; full triage record at [`docs/design/critique-2026-05-23-triage.md`](design/critique-2026-05-23-triage.md) §4. The v0.10.x patch lane and v0.11 architectural lane have fully shipped — their rows are retired to the **Resolved cross-cutting items** block, with full provenance in [Shipped Releases](#shipped-releases) and `CHANGELOG.md`; only genuinely-open items remain in the lanes below.
...
#### v0.10.x patch lane (in-freeze, narrowing fixes) — all shipped

> All v0.10.x patch-lane items shipped (v0.10.7–v0.11.1). Retired to the **Resolved cross-cutting items** block below; full per-item provenance retained in [Shipped Releases](#shipped-releases) (v0.10.7/v0.10.8 blocks, v0.11.0/v0.11.1) and `CHANGELOG.md`. Closed: DRIFT-1, TC-EOP-1, OBLIG-PBT-5a, INT-1, DRIFT-CI-1, F-GATE-8, VERIFY-RPT-1.

#### v0.11 architectural lane (freeze-lifted) — all shipped

> The v0.11 milestone shipped (v0.11.0–v0.11.2); all LT items below landed. Retired to the **Resolved cross-cutting items** block; full per-item provenance (design-doc links, professor-review folds, commit SHAs, test counts) is retained in the [Shipped Releases](#shipped-releases) v0.11 block (and its collapsed shipped-history) and `CHANGELOG.md`. Closed: LT-INV (incl. def-logic removal, completed v0.12.1), LT-CDP, LT-PPR, LT-INT/INT-2, INT-PRE, REF-META-1.
...
| **OBLIG-1-FOLLOWON** (populate the reserved checkout-brief / obligation-report fields) | **PARTIAL** — `expected_return_type` shipped v0.13.1 (DEF-RET) and `available_functions` shipped via DEMO-COMP; only the `assumptions` field remains reserved | The two value paths shipped: `expected_return_type` via **DEF-RET** (the return-type surface that records `HoleTyped T` for a function-body hole; shipped v0.13.1, see Shipped Releases) and `available_functions` via **DEMO-COMP** (shipped v0.13.0). Residual: wire the `assumptions` field (formerly `reserved`, returns-absent per `LLMLL.md:1823-1824`) — semantics may want a short language-team pin before build. No schema bump expected for the residual. Supersedes the never-completed v0.3.5 §C acceptance for these fields (see §C block below). |
...
> **Retired (shipped v0.12.0):** REF-META-2..5 (metatheory of record complete), Bundle B0 (per-function `effect_summary` + cross-module propagation), Non-int refinement widening Phase 1 (NIW intro+elim), and F-NIW-2/3/4/4b moved to the **Resolved cross-cutting items** block; full per-item provenance retained in the [Shipped Releases](#shipped-releases) v0.12.0 block and `CHANGELOG.md`. Bundle B1 (explicit `Command {stdout} a` annotations) was the gated follow-on; its B0-experiment gate was retired (F-B0-3, commit `049a294`) and B1 now awaits a B1-native experiment. NIW Phase 2 (ADT-field refinements) and B2/B3 remain future.

#### Research track (no v0.x targets, no Active Items rows)

| Item | Current Status | Next Action |
|------|---------------|-------------|
| **Contract discriminative power formalization** | ~~Proposed by Professor / Research track~~ → **Promoted to v0.11 CDP-0** (LT-CDP, shipped v0.11) | See [Shipped Releases](#shipped-releases) v0.11 block; [`docs/archive/research-track.md`](archive/research-track.md) §6 is the archived source |
...
| Cross-module `ContractEnv` (MOD-1) | **Shipped** with v0.10 (v0.10 prerequisite); see [Shipped Releases](#shipped-releases) v0.10 block. |
| Evidence model design (EVID-0) | **Approved** (Rev 2) — **shipped** via v0.8.1b (commit `bf98797`); see [Shipped Releases](#shipped-releases). |
| v0.10.x patch lane (DRIFT-1, TC-EOP-1, OBLIG-PBT-5a, INT-1, DRIFT-CI-1, F-GATE-8, VERIFY-RPT-1) | **All shipped** (v0.10.7–v0.11.1). Per-item provenance in [Shipped Releases](#shipped-releases) (v0.10.7/v0.10.8 blocks, v0.11.0/v0.11.1) and `CHANGELOG.md`. |
| v0.11 LT items (LT-INV, LT-CDP, LT-PPR, LT-INT/INT-2, INT-PRE, REF-META-1) | **All shipped** (v0.11). LT-INV's def-logic removal completed in v0.12.1. Per-item provenance (design-doc links, professor-review folds, SHAs) in the [Shipped Releases](#shipped-releases) v0.11 block and `CHANGELOG.md`. |
| REF-META-2..5 (solver-completeness, predicate WF, erasure, type-assignment) | **All promoted / shipped** v0.12.0; metatheory of record (1–5) complete. `LLMLL.md §5.3.3 / §3.4.4 / §3.4.5 / §3.4.6`; see [Shipped Releases](#shipped-releases) v0.12.0 block. |
...
| DEF-RET (optional return-type annotation + return-refinement discharge) | **Shipped** — Unit 1 v0.13.1 (commit `a948deb`), Unit 2 v0.13.2 (commits `0b27be5`/`12210b0`). Closed the `expected_return_type` OBLIG-1-FOLLOWON value path; schema `0.6.0 → 0.7.0` (Unit 1). See [Shipped Releases](#shipped-releases) v0.13.1/v0.13.2 blocks. |
...
**Current shipped: v0.13.2** (2026-06-21). The minor/patch line beyond v0.10 — v0.11 (Core/Shell Inversion + Evidence-Axis Enrichment), v0.12.0 (Refinement Metatheory of Record + Effect/Authority Summaries), v0.12.1 (def-logic Removal + def-invariant Node), v0.13.0 (Caller-Obligation Axis + Verified Composition), v0.13.1 (Optional Return-Type Annotation — DEF-RET Unit 1), and v0.13.2 (Return-Refinement Discharge — DEF-RET Unit 2) — is recorded under [Shipped Releases](#shipped-releases). 908 Haskell + 62 Python tests. (The v0.8–v0.10 critical-path ASCII diagram above is retired into a collapsed block; per-version status for v0.11→v0.13.2 is in Shipped Releases.)
...
## v0.13.2 — Return-Refinement Discharge (DEF-RET Unit 2) ✅ SHIPPED

**Theme:** A refinement-aliased return type discharges via the body-VC and is exported as a caller-assumable guarantee.

> Shipped 2026-06-21 (commits `0b27be5`/`12210b0`). **DEF-RET Unit 2** — a refinement-aliased return **discharges** via `augmentContractPost` (the guarantee-side dual of `augmentContractPre`): the body-VC proves it (`verified` / `refuted` / `erBodyFallback`) and it is exported as a caller-assumable guarantee (folded into the effective post in `buildContractEnv`; assume-guarantee, §5.3.4 meet). A bare `-> RetType` function is credited `verified`. The staleness hash now covers the augmented contract (a `-> RetType` annotation or alias-predicate edit invalidates a stale verified sidecar — closes a cache-poisoning hole **and the pre-existing param-side NIW staleness hole**); the solver-less `--trust-report` staleness-bypass is fixed. `schemaVersion` unchanged `0.7.0`. Design: [`def-return-annotation-proposal.md`](design/def-return-annotation-proposal.md) (Rev 3); staleness review [`def-ret-staleness-hash-review.md`](design/def-ret-staleness-hash-review.md). 908 Haskell + 62 Python tests. See `CHANGELOG.md` §v0.13.2.

## v0.13.1 — Optional Return-Type Annotation (DEF-RET Unit 1 / OBLIG-1-FOLLOWON) ✅ SHIPPED

**Theme:** Optional `-> RetType` surface on `def`/`def-shell`; populate the `expected_return_type` checkout-brief field.

> Shipped 2026-06-21 (commit `a948deb`). **DEF-RET Unit 1** — optional `-> RetType` surface on `def`/`def-shell` (both frontends + schema `0.6.0 → 0.7.0`), body checked against it; `expected_return_type` populated for a function-body hole (the OBLIG-1-FOLLOWON value path). A refinement-aliased return fell back conservatively off `verified` (discharge shipped in Unit 2, v0.13.2). Design: [`def-return-annotation-proposal.md`](design/def-return-annotation-proposal.md) (Rev 3). See `CHANGELOG.md` §v0.13.1.
