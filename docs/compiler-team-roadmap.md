...
| **Bundle B** (effect rows on `Command`) | **Reclassified** out of indexed-types research bundle per memo §4 | **B0** (function-level effect summaries in obligation report — no syntax change): in-scope for v0.12 spec + engineer. **B1** (explicit `Command {stdout} a` annotations, opt-in): v0.12 LT proposal; engineer build gated on B0 experiment result. **B2/B3**: v0.13+ at earliest. See [`docs/design/v0.12-direction.md §3`](design/v0.12-direction.md). NOT WASM-coupled. |
