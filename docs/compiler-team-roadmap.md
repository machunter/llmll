...
| **XMOD-SCOPE-BRIEF** (checkout `available_functions` / `in_scope` miss imported names) `[CT]` | **Open (confirmed) — brief-completeness, not soundness** | Observed during the XMOD-CG-BRIEF e2e check (2026-07-09): the brief's `available_functions` list is built from same-file statements and `in_scope` comes from `runSketch` over `builtinEnv` (not cache-seeded), so an `(open)`-ed imported callable like `double` appears in neither — while `consumed_guarantees` (v0.14.18) now shows it. Under the checkout-brief-sole-channel rule this hides an imported callee's callable contract from a hole-filling agent. Fix direction: seed the sketch env / functions list from the module cache (cf. `doTypecheck`'s `seedModule`, `Main.hs:1887`). |
...
> **XMOD-COMP (cross-module verified composition) — closed, all five layers.** Layers 1–3 shipped earlier (admission / type-alias type-check / trust-tier edge); layer 4 (body-VC emission) by **XMOD-AG (v0.14.17)**; layer 5 (checkout `consumed_guarantees`) by **XMOD-CG-BRIEF (v0.14.18)** — the brief's `ContractEnv` is seeded from the module cache with the verify path's recipe, and `callee_tier` resolves through the qualified trust entry. Residual (a sibling channel, not a layer of this finding): the brief's `available_functions` / `in_scope` are still same-file-only — tracked as **XMOD-SCOPE-BRIEF** in Active Items. See [`cross-module-composition-finding.md`](design/cross-module-composition-finding.md).
