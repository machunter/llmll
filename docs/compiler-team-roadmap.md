...
| **PAIR-RET** (refinement predicates over pair/tuple returns) | **SHIPPED (v0.13.11; PAIR-RET-2 v0.13.12)** | `first`/`second` reflect to datatype selectors and `(pair a b)` to the constructor (`LLMLL.md §5.3.5`), so a post like `(= (+ (first r) (second r)) k)` discharges (`verified`/`refuted`) instead of `asserted`. Encoded as the polymorphic product `data Pair2 2` — the single-constructor restriction of the COMP-4 datatype class (§5.3.3). Unblocks two-account conservation (`examples/payments-core/conserve.llmll`). **PAIR-RET-2 (v0.13.12)** extended discharge to nested / list / **admissible-sum-or-ADT** components (`(int, Box)` → `(Pair2 int Box)`, alias-aware `typeToSortA`); a `Result` or recursive-sum component falls back cleanly via the §5.3.3 firewall (previously crashed the solver). Remaining follow-on: `Result`-builtin construction (`(ok n)` body-faithfulness — a separate COMP-4 item). |
