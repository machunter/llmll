...
| **OBLIG-1-FOLLOWON** (populate reserved obligation-report fields) | **v1 + v2a SHIPPED — residual two provinces deferred** | The `assumptions` field carries: (v1, v0.14.29) refinement predicates of in-scope refinement-typed **params**, α-renamed to the binder; (v2a, v0.14.31) **let-definitional equalities `(= y e)`** for in-scope let-bindings with a QF-LIA RHS (`ScopeBinding.sbDef` + `tcDefs`). **Deferred v2 provinces**: match-scrutinee case hypotheses `(= s (Ctor x))` (needs scrutinee/ctor capture in the sketch env), and `def-invariant` axioms (needs provenance tagging → schema bump, an unverified invariant being a TCB assumption). All value paths (`expected_return_type`, `available_functions`, `assumptions`) populated. |
