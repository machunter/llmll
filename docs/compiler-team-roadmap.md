...
| **COMP-3b-general / COMP-4** (refinement-return discharge through nested / non-`Result` matches) | **P2 — open** | Generalize the shipped flat-`Result`-match discharge: **COMP-3b-general** (match nested under `let`/`if`) needs a term-type env threaded through `bodyToPredM` (SortEnv is int-only); **COMP-4** (arbitrary two-arm user ADTs) additionally needs exhaustiveness + selector theory. Constructor-dependent posts stay a deliberate fallback. |
