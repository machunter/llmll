...
| **CLASSIFY-EOP** (QF-LIA classifier `EOp`/`EApp` blind spot) `[CT]` | **Open — surfaced by REFINE-REUSE (v0.14.29)** | `classifyContractFragment` / `isQfLia` recognize only the `EApp` operator form, but the JSON parser emits `EOp` (`kind:"op"`) — so a *parsed* contract mis-classifies as non-QF-LIA. REFINE-REUSE works around it locally (`normContractOps`); audit other callers (obligation-report `ccContractFrag`, verify-path fragment routing) for the same mis-classification and normalize centrally if it bites. |
