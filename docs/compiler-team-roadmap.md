...
| **OBLIG-1-FOLLOWON** (populate the reserved checkout-brief / obligation-report fields) | **PARTIAL** — `expected_return_type` shipped v0.13.1 (DEF-RET) and `available_functions` shipped via DEMO-COMP; only the `assumptions` field remains reserved | The two value paths shipped: `expected_return_type` via **DEF-RET** (the return-type surface that records `HoleTyped T` for a function-body hole; shipped v0.13.1, see [Shipped Releases](#shipped-releases)) and `available_functions` via **DEMO-COMP** (shipped v0.13.0). Residual: wire the `assumptions` field (formerly `reserved`, returns-absent per `LLMLL.md:1823-1824`) — semantics may want a short language-team pin before build. No schema bump expected for the residual. Supersedes the never-completed v0.3.5 §C acceptance for these fields (provenance in [`docs/archive/roadmap-shipped-history.md`](archive/roadmap-shipped-history.md) §v0.3.5). |
| **INT-3** (`MachineInt` QF-BV alias) | **P3 — open** | LT design when freeze lifts again; promote to P1 (freeze-exception candidate) if INT-PRE shows TOTP regression > 5× (INT-PRE cleared at 1.015×, so dormant) |
...
| **COMP-3b-general / COMP-4** (refinement-return discharge through nested / non-`Result` matches) | **P2 — open** | **COMP-3b** (refinement-aliased return over a *flat* two-arm `Result`-variable match) landed on branch `feat/comp-3b-result-ematch`, pending release. The driver-level fix fires only when the function body *is* the match; generalizing to a match nested under `let`/`if` (**COMP-3b-general**) needs a term-type env threaded through `bodyToPredM` (the SortEnv is int-only), and to arbitrary two-arm user ADTs (**COMP-4**) additionally needs exhaustiveness + selector theory. Constructor-dependent posts stay a deliberate fallback (the synthetic free guard cannot express per-constructor obligations). |
