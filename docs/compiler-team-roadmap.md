...
| **REC-BODY-VC** (own body VC for recursive `def`/`letrec` at partial correctness) `[LT→CT]` | **Rev 1 design settled — [`rec-body-vc-proposal.md`](design/rec-body-vc-proposal.md); (a) SHIPPED v0.14.23, (b0) SHIPPED v0.14.22** | Baseline overturned the row's premise: own-contract assume-guarantee already ships (v0.14.13 §0.1). Restructured into **(a) partiality marker — SHIPPED v0.14.23** (`termination_unverified` + `partial_fns`; D1/D2 spec reconciliation done, one-pager/README rows split), **(b0) def-form in the evidence hash — SHIPPED v0.14.22** (closes probe-E laundering), (b1) fail-closed default, (c) `(decreases …)` call-site strict descent. Sound at partial correctness. F-1 (`examples/secure-channel-emergent/`): a degenerate self-call fill verifies SAFE + body-faithful and is now flagged `termination_unverified`; with (c) + **R7** strict descent it becomes a solver refutation (no decreasing measure), retiring TERM-1's disclaimer. **Remaining: (b1) + (c) — REC-DESCENT `(decreases …)` surface (schema 0.7.0→0.8.0).** |
