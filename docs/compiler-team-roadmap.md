...
| **PAIR-RET** (refinement predicates over pair/tuple returns) | **SHIPPED (v0.13.11)** | `first`/`second` now reflect to datatype selectors and `(pair a b)` to the constructor (`LLMLL.md §5.3.5`), so a post like `(= (+ (first r) (second r)) k)` discharges (`verified`/`refuted`) instead of `asserted`. Encoded as the polymorphic product `data Pair2 2` — the single-constructor restriction of the COMP-4 datatype class (§5.3.3); scoped to QF-LIA-scalar components, opaque-pair / non-`Σ_auto` component falls back. Unblocks two-account conservation (`examples/payments-core/conserve.llmll`). Follow-on: nested / list / sum-component pairs (PAIR-RET-2). |
