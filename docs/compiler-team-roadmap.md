...
| **REC-BODY-VC** (own body VC for recursive `def`/`letrec` at partial correctness) `[LT→CT]` | **Rev 1 design settled; (a) SHIPPED v0.14.23, (b0) SHIPPED v0.14.22, (c) Phase 1 SHIPPED v0.14.24** — [`rec-body-vc-proposal.md`](design/rec-body-vc-proposal.md) | Baseline overturned the row's premise: own-contract assume-guarantee already ships (v0.14.13 §0.1). Restructured into **(a) partiality marker — SHIPPED v0.14.23**, **(b0) def-form in the evidence hash — SHIPPED v0.14.22** (closes probe-E laundering), (b1) fail-closed default, **(c) `(decreases …)` strict descent — Phase 1 (surface + schema 0.8.0, inert) SHIPPED v0.14.24.** F-1 (`examples/secure-channel-emergent/`): a degenerate self-call fill verifies SAFE + body-faithful, now flagged `termination_unverified`; Phase 2 descent turns it into a `measure-not-decreasing` solver refutation, retiring TERM-1's disclaimer. **Remaining: REC-DESCENT Phase 2 (well-foundedness + per-call-site strict descent + `measure-not-decreasing` verdict + measure-in-`canonicalDefEvidenceHash`) + Phase 3 (discharge feedback: drop `termination_unverified` + `checkCalleeAdmissibility` lift, partial→total). Professor rulings settled: hash the measure (Q1); admit different-measure mutual recursion via common-ℕ Floyd criterion, ≥0 floor gates the whole SCC, sound-but-incomplete per-edge (Q2).** |
