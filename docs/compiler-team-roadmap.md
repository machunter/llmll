...
| **OBLIG-1-FOLLOWON** (populate reserved obligation-report fields) | **v1 SHIPPED (v0.14.29) — residual v2 deferred** | The `assumptions` field is wired (v0.14.29, checkout brief): refinement predicates of in-scope refinement-typed **params**, α-renamed to the binder. v1 is params-only (sound-but-incomplete, professor-scoped); **deferred v2 provinces**: let-definitional equalities `y=e` (surface as equalities — the body VC already assumes them, a disclosed brief-vs-verifier asymmetry), match-scrutinee case hypotheses, and `def-invariant` axioms (needs provenance tagging → schema bump, since an unverified invariant is a TCB assumption). All value paths (`expected_return_type`, `available_functions`, `assumptions`) now populated. |
...
| **REFINE-REUSE** (reuse retrieval for `refine`: surface existing defs that subsume a spawned sub-contract) | **✅ SHIPPED v0.14.29** `[LT→CT]` | Design [`refine-reuse-gate-proposal.md`](design/refine-reuse-gate-proposal.md) (Rev 1, professor-confirmed) implemented in `LLMLL.RefineReuse`: advisory `reuse_suggestions` + non-blocking `W-REUSE` on an exact contract-equivalent; subsumption = contract subtyping (`preₛ⟹pre_D ∧ post_D⟹postₛ`, α-normalized) via two standalone liquid-fixpoint Horn constraints; signature pre-filter + QF-LIA gate; graceful no-solver skip. Stage 4 of Cascading Refinement. See Shipped Releases. |
| **CLASSIFY-EOP** (QF-LIA classifier `EOp`/`EApp` blind spot) `[CT]` | **✅ SHIPPED v0.14.30** | Audit found it BROADER than first thought — **both** parsers emit `EOp` for operators (S-expr `Parser.hs:897`, JSON `ParserJSON.hs:558`), so `classifyContractFragment`/`isQfLia` mislabeled *every* operator-bearing contract `non_qf_lia` and downgraded its obligation tier to Advisory (verifier unaffected — `FixpointEmit` normalizes `EOp→EApp`). Fixed centrally in `isQfLia` (`EOp`→`EApp`); REFINE-REUSE's local `normContractOps` workaround removed. Classification/reporting fix, not soundness. |

> **Recently retired (shipped):** REF-META, Bundle B0, NIW Phase 1 (v0.12.0); COMP-4 / PAIR-RET lines (v0.13.x); PROOF-ARTIFACT, R5 differential-implementation-pressure (v0.14.x); **REC-BODY-VC — the whole line: (b0) hash integrity v0.14.22, (a) partiality marker v0.14.23, (c) REC-DESCENT surface v0.14.24 + discharge/admission v0.14.25 (design [`rec-body-vc-proposal.md`](design/rec-body-vc-proposal.md); R7 single-slice descent promoted, TERM-1 disclaimer retired for discharged recursion).** **MATCH-WIDEN-2 (n-arm sums + sequential matches, v0.14.26)** — the body-faithful match fragment widened from two arms to any arity + sequential composition (`match-widen-stretch-plan.md` §S4). Provenance in [Shipped Releases](#shipped-releases) + `CHANGELOG.md`. Still future: Bundle B1 (awaits a B1-native experiment), NIW Phase 2, B2/B3; R5's forced-diversity follow-on is experiment-lead-owned.
