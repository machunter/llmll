...
| **CLASSIFY-MEASURE** (classifier–emitter drift on the measure class + string literals) `[CT]` | **Open — medium (reporting, the CLASSIFY-EOP/A3 defect family)** | `isQfLia` and `exprToPred` disagree outside arrays: a measure-class contract (`(>= (string-length s) 1)`) classifies `non_qf_lia` yet discharges body-faithful (obligation tier under-labeled), and a string-literal contract classifies `qf_lia` yet falls back (over-labeled). Same fix shape as A3: derive classification from the emitter's own predicates. Found by the A3 pass (v0.14.38). |
...
| **R3 — Spec-from-RFC pipeline** (RFC → LLMLL contracts with `:source` clause provenance) | **Evaluation run EXECUTED (v0.14.39) — all six §4 criteria PASS** | Pipeline validated by procedure on an unseen RFC: [`examples/rfc1982_serial/`](../examples/rfc1982_serial/) (RFC 1982 serial arithmetic — `verified: 3`, two refuting mutants incl. the historical naive-`<` DNS bug, weak-spec exhibit, first persisted G1 clause inventory); findings in [`experiments/rfc1982-eval/findings.md`](../experiments/rfc1982-eval/findings.md) (G4 closed). Remaining: fold the doc revisions F-1982-3/-4/-5 (C5 width adaptation, S4.4 shape-dependent flag floor, S1 source-text input) into the pipeline doc — language-team, small. |
...
|---|---|---|---|---|
| **A** | **Theory of arrays** for `bytes[n]` / `map[k,v]` | QF theory of arrays (`select`/`store` + extensionality) — **decidable**, polite-combines with QF-LIA; **stays in `Σ_auto`** | array index-in-bounds (real memory-safety, not a length proxy); map get-after-put / key-presence | **A0–A3 SHIPPED (v0.14.33/34/37/38) — arrays verify AND report coherently; next = A4 flagship data example** — design [`data-scope-lever-a-arrays-proposal.md`](design/data-scope-lever-a-arrays-proposal.md) (Rev 1.1) / [`-review.md`](design/data-scope-lever-a-arrays-review.md) / [`-feasibility.md`](design/data-scope-lever-a-feasibility.md). A3 = classification derives from the emitter's own §6.1 guards (`contractArrGuardsBlock`; exactly-reflectable array contracts classify `qf_lia`, residue stays out); checkout briefs list the eight ops as type-relevance-gated `builtin` vocabulary with PROVE pres; v2a assumptions admit array-op RHS; `examples/bytes-bounds/` = the regression witness. **Open residue:** A2.1 (string/bool-valued map reflection, cross-call map assume-guarantee, non-pure map-returning bodies — read-modify-write `(map-put m k (+ (map-get m k) 1))` falls back) + A3.x (reuse-retrieval driver abstains on array contracts — sort-splitting; CDP array-sorted candidate bases; classifier callee-leg imprecision). Language-team dispositions SETTLED (2026-07-12): v1 `bytes-zero` context rule blessed (addendum in the proposal appendix; widening levers recorded unscheduled); contract-position reads = total selects, sound both directions, status-quo + scoped lint ([`contract-position-reads-disposition.md`](design/contract-position-reads-disposition.md) → CONTRACT-READ-LINT row). |
