...
| **F-GATE-8** | **☑ Shipped (commit `f62a38b`, 2026-05-29)** | engineer: `pbtTrustWriteback` in [`compiler/src/LLMLL/PBT.hs:641–755`](../compiler/src/LLMLL/PBT.hs) builds `delegateBodies :: Set Name` from any `SDefShell` with `EHole(HDelegate _)` / `EHole(HDelegateAsync _)` body; `processRun` gains `guardDelegate` helper that suppresses `DLTested` lift and emits informational diagnostic on both the OBLIG-PBT-4 explicit-annotation path and the singleton-head-position path. Secondary: `evalBuiltinApp` in [`compiler/src/LLMLL/Contracts.hs`](../compiler/src/LLMLL/Contracts.hs) gains `string-length` and `string-empty?` (literal-only constant-folding; previously registered in TypeCheck.hs:109,118 but absent from static evaluator). 6 regression tests FG8-1–FG8-6 in `Spec.hs`. No schema delta; `trust_report_version` stays `1.2.0`. Evidence runs at [`experiments/minimal-agent/runs/20260528T204620Z/`](../experiments/minimal-agent/runs/20260528T204620Z/). 783 Haskell + 62 Python tests. |
| **VERIFY-RPT-1** | **☑ Shipped (v0.11.1, commit b914587, 2026-06-06)** — all three defects + refuted trust status; `trust_report_version` 1.3.0; 793→801 Haskell. Spec promoted to LLMLL.md §3.4.3 / §5.3.4 / §4.4 / §5.3.5. | Three verify-path reporting defects (proof engine + patch gate correct; reporting layer diverges). Full repro, root-cause citations, and regression-test list at [`docs/design/verify-reporting-defects-2026-06-04-bug.md`](design/verify-reporting-defects-2026-06-04-bug.md). **(1) High:** `verify` fails open on UNSAFE — `fqResultToReport (FQUnsafe ids)` sets `reportSuccess = null diags` at [`DiagnosticFQ.hs:106`](../compiler/src/LLMLL/DiagnosticFQ.hs#L106); the real trigger is `extractIds` ([`DiagnosticFQ.hs:74-83`](../compiler/src/LLMLL/DiagnosticFQ.hs#L74-L83)) failing to parse the solver's ANSI `Unsafe:` output → `FQUnsafe []` → `success:true`, exit 0, empty diagnostics. Fix: `FQUnsafe → reportSuccess=False` unconditionally + `exitFailure` on `FQUnsafe`; `fixpoint --json` extraction (`{"tag":"Unsafe","contents":[_,[ids]]}`) for pointer recovery; route exit through the `FQVerifyResult` constructor as `reVerify` already does. **(2) High:** `verify --trust-report` renders `post: asserted` for a function whose `.verified.json` carries `post: verified` — **root cause is `sidecarNeedsRevalidation` at [`VerifiedCache.hs:218-232`](../compiler/src/LLMLL/VerifiedCache.hs#L218-L232)** invalidating the whole sidecar (returns `Map.empty`, `:204`) because the v0.10.8 INT-1 field-absence guard fires on v0.11 sidecars that legitimately omit `overflow_tainted` (taint disarmed on `int`, [`FixpointEmit.hs:516`](../compiler/src/LLMLL/FixpointEmit.hs#L516)); **not** a `buildTrustReport`/`_cache` shadowing issue — `mergeCS` is correct. Fix: neutralize the field-absence trigger. **(3) Medium:** `--strict-verified-core` passes a body-faithful UNSAFE (refuses only fallback/overflow-tainted at [`Main.hs:1149-1165`](../compiler/app/Main.hs#L1149-L1165)). Cross-cutting: same rebasing gap empties the `PatchVerifyError` diagnostic payload. Reproduced on v0.11.0 binary. **Spec-track companion (Defect 3):** [`docs/design/verified-contract-refuted-status-proposal.md`](design/verified-contract-refuted-status-proposal.md) (Settled Rev 2) pins the solver-SAFE conjunct on `verified` / `--strict-verified-core` (§5.3.4:848/863, §3.4.3:313) and adds a `refuted` trust status (orthogonal flag, not a `DisplayLevel`; `trust_report_version` 1.2.0 → 1.3.0) so a disproved contract is never rendered `asserted`. Professor-reviewed; QF-LIA-decidability side-condition + VCgen REF-META-4 anchor. |
...
| **LT-INV** (core/shell grammar inversion) | **Settled (Rev 2) — review folded; engineer build shipped on branch** | LT proposal at [`docs/design/core-shell-inversion-proposal.md`](design/core-shell-inversion-proposal.md) settled Rev 2 (commit `5f31580`, 2026-05-25) incorporating seven gaps + two author-question answers from professor review; review folded as §"Appendix — Professor review log" (commit `f1ef0c0`); standalone archived to [`docs/archive/professor-reviews/core-shell-inversion-review.md`](../docs/archive/professor-reviews/core-shell-inversion-review.md). Engineer build shipped: `--grammar=core-inversion` opt-in flag active on all subcommands; `GrammarCoreInversion` mode parses `def`/`def-shell`, enforces `isCoreBodySyntactic` whitelist and `checkCalleeAdmissibility` (body-faithful | trustedPrelude | builtinEnv); two new diagnostic kinds `core-grammar-violation`/`core-membership-violation`; full 15-module fan-out; 28 LT-INV regression tests (`INV-P/W/A/C/G` series); 744 Haskell + 58 Python tests total. Schema bump `0.5.0 → 0.6.0` + default flip to `GrammarCoreInversion` gated on §8 empirical-validation gate; `CDPScope` flip to `CDPScopeCoreOnly` shipped (commit `3af3c06`, 2026-05-29; scope-filtering wired in `CDP.hs:computeCDPFor`). Spec-drift finding closed (2026-05-30 spec-track doc pass): `builtinEnv` admission formalized at `LLMLL.md §5.3.5:892-893` (three-leg NOTE block — body-faithful `EvidenceRecord`, `trustedPrelude` membership, `builtinEnv` membership; trust-tier propagation into caller via lattice meet §4.4.1; contract-clause EApp parity) and `LLMLL.md §12:1804-1806` (grammar production comment). No language-team action owed. Subsumes STRICT-CORE-1 sub-item. Cross-proposal shipping under gate outcomes per [`v0.11-cross-proposal-rollback-discipline.md`](design/v0.11-cross-proposal-rollback-discipline.md). **F-GATE-1 patch (commit `cabb1fd`, 2026-05-28):** `parseJSONAST` now enforces `GrammarCoreInversion` — `{"kind":"def-logic"}` in `.ast.json` input is rejected with a `core-grammar-violation` diagnostic (exit 1) under `--grammar=core-inversion`; §8 empirical-validation gate unblocked for re-run. 3 new tests INV-P9/P10/P11; **767 Haskell + 58 Python** tests total. **F-GATE-1b patch (2026-05-28):** `letrec` enforcement added to both parser paths — `{"kind":"letrec"}` rejected under `GrammarCoreInversion` in `ParserJSON.hs`; `pDefLogic`/`pLetrec` moved to `GrammarLegacy` arm of `pStatement` in `Parser.hs`, closing the S-expression-path asymmetry. INV-P6 inverted; INV-P12/P13/P14 added. **770 Haskell + 58 Python** tests total. Routing-note closures: S-expression-path asymmetry (INV-P6) resolved by F-GATE-1b. F-GATE-2 (`LLMLL.md §4.1` prose gap) was already resolved — `LLMLL.md §4.1` and `getting-started.md §4.14` already stated `def-logic` and `letrec` are rejected under `--grammar=core-inversion`; no `language-team` action owed. **§8 gate nominal pass — default flip provisional (2026-05-28):** Postmortem-004/005 established axis (c) `?proof-required` emission 0/6 → 3/6; §8 Rev 2 conjunctive criterion nominally met. Post-run evaluator fix (F-GATE-7) and compiler fix (F-GATE-8, commit `f62a38b`, 2026-05-29) identify confounders in the PM-005 cell set; PM-005 is not a clean reference run. `GrammarCoreInversion` default flip at commit `5cab1b7` is provisional; redesigned gate experiment pending post-E3-fix before the flip is confirmed. If the redesigned run fails, rollback path (1) applies. See [`docs/design/core-shell-inversion-proposal.md §8 Gate Outcome`](design/core-shell-inversion-proposal.md). **773 Haskell + 62 Python** tests total. **Schema bump + examples migration shipped (commit `afe80df`, 2026-05-28):** `expectedSchemaVersion` bumped to `"0.6.0"`; `docs/llmll-ast.schema.json` bumped; all `examples/**` migrated `def-logic`/`letrec` → `def-shell`; DRIFT-CI-1 C3 PASS. §8.4 step-4 sequence complete. **EL-5 clean gate (2026-05-30): §8 gate PASS confirmed definitively.** Run `20260530T052351Z` (PM-006, [`experiments/minimal-agent/findings/postmortem-006-el5-clean-gate.md`](../experiments/minimal-agent/findings/postmortem-006-el5-clean-gate.md)): axis (c) 8/8 (100%), grade A 5/5 claude + 2/3 gemini, 0 exclusions. F-GATE-7+8 contamination fully eliminated. **CE-3 re-applies grammar default flip + schema bump** (compiler side; `examples/**` migration for 37 remaining files still pending). DRIFT-CI-1 C3+C4 PASS. **783 Haskell + 62 Python tests.** **Spec-track doc pass 2026-05-30:** `?delegate`/`?delegate-async` (`def-shell` and `def` pre-resolution) verification-matrix rows added to `LLMLL.md §5.3.5`; §6 cross-reference NOTE added. F-GATE-8 `asserted`-tier and `DLTested`-guard behavior now formally documented in the spec. **F-EL5-3 adjudicated + shipped:** language-team adjudication 2026-05-30 — `def + hole-delegate` → `asserted` unconditionally, pre-clause evaluability notwithstanding; `delegateBodies` in `pbtTrustWriteback` extended to `SDef` forms, commit `63b9bb3`; tests FG8-7/FG8-8 added. `LLMLL.md §4.4.5` PBT-Lift rule generalized (`SDefLogic` premise → `contractedNames`; SC-7 delegation-body suppression side condition added to both PBT-Lift and PBT-Lift-Annotated); §5.3.5 row 883 and §6 NOTE box pending markers closed (doc pass, 2026-05-30). **CE-3 examples migration complete (commit `f09c498`, 2026-05-30):** 20 `.ast.json` files migrated `schemaVersion 0.5.0/0.1.3 → 0.6.0`; 145 `def-logic` + 6 `letrec` nodes → `def`/`def-shell` per §6 classifier; boundary-form distribution 47.7% `def` satisfies §8.1 ≥25% gate. Pending: 17 `.llmll` S-expression files. **785 Haskell + 62 Python tests.** **Parser defect closed (commit `8542033`, 2026-05-30):** `ParserJSON.hs:133-134` admitted `"def"` and `"def-shell"` unconditionally under all grammar modes — language-team §4.1 audit found the gap post-CE-3; symmetric `legacy-grammar-violation` guards added; `GrammarMode` threaded through `parseJSONASTValue`, `applyPatch`, `doPatch`, `Serve.handlePatch`. INV-P15/P16/P17 added; OBLIG-3 fixture migrated `def-logic` → `def`. **788 Haskell + 62 Python tests.** **LT-INV Rev 4 (2026-05-31):** def-logic deprecation timeline settled — v0.12 removes `def-logic` entirely (parse error under all grammar modes; no auto-rewrite); `.llmll` S-expression migration complete (commit `17d0da6`, 2026-05-31); v0.12 def-logic-removal gate cleared. **Hub-import grammar-mode spec NOTE (2026-05-31):** `LLMLL.md §8.2` NOTE documents that all imported modules (including hub cache) are parsed under the invoking command's `GrammarMode`; closes the `checkStatement (SImport imp)` deferred item from the TypeCheck GrammarMode API collapse (2026-05-30). **Sexp example migration complete (2026-05-31, commits `283f4e5`/`5241965`/`17d0da6`):** 17 `.llmll` S-expression files migrated `def-logic` → `def`/`def-shell`; 38 functions → `def` (49.4%), 39 → `def-shell` (50.6%) per `isCoreBodySyntactic` + `checkCalleeAdmissibility`; 4 `parseStatements GrammarLegacy` benchmark test calls → `GrammarCoreInversion`; 3 `SDefLogic` golden patterns → `SDef`. All `examples/**` fully migrated. No test count delta. **788 Haskell + 62 Python tests.** **Schema-update subtask closed (v0.11.1, commit `d3de0da`, 2026-06-02):** `DefCore` (`kind:"def"`) and `DefShell` (`kind:"def-shell"`) added to `docs/llmll-ast.schema.json` `$defs`; `Statement.oneOf` updated to reference both; `DefLogic` deprecated in-place and removed from `oneOf` (v0.12 `$defs` removal gate cleared); `spec_entropy` field backfilled on `DefCore`/`DefShell` (LT-CDP JSON-AST gap); `TrustDecl` description updated. SCHEMA-1..4 tests added. **793 Haskell + 62 Python tests.** **def-logic removal shipped on branch `lt-inv/def-logic-removal` (commit `4362ebd`, 2026-06-16, released v0.12.1):** Rev 4 closed — `def-logic` is a hard parse error under all grammar modes (S-expr `pDefLogicRemoved` + JSON `fail`; new `removed-construct` diagnostic kind suggesting `def`/`def-shell`); `def-invariant` promoted to its own `SDefInvariant` AST node so AstEmit round-trips it faithfully (was lossily emitted as `def-logic`); dead `DefLogic` `$def` removed from the schema (no `schemaVersion` bump). `letrec`/`--grammar=legacy` retained. Follow-up (commit `0256839`): parse-error diagnostics no longer recommend the removed `def-logic` (point at `def`/`def-shell`). Released in **v0.12.1** (2026-06-17): version pins bumped to 0.12.1, CHANGELOG `## v0.12.1`, spec-prose def-logic reconciliation across `LLMLL.md` (§2/§8.2/§8.3/§8.5) and `docs/getting-started.md` (§4.14). **862 Haskell + 62 Python tests.** (v0.12.0 milestone "✅ SHIPPED" relocation — REF-META/Bundle-B0/NIW rows → Shipped Releases — remains a deferred roadmap close-out.) |
| **LT-CDP** (contract discriminative power evidence axis) | **Shipped on branch (commit `121815a`)** | Engineer-shipped on `lt-cdp/discriminative-power-axis`: new [`compiler/src/LLMLL/CDP.hs`](../compiler/src/LLMLL/CDP.hs) module owns score / partition / typed warnings per proposal §5; closed v0.11 candidate-set enumeration in [`compiler/src/LLMLL/WeaknessCheck.hs`](../compiler/src/LLMLL/WeaknessCheck.hs) `generateCDPCandidates` per proposal §4.3.1; `(spec-entropy …)` annotation parsed in both frontends; `trust_report_version` bumped `1.1.0 → 1.2.0` with additive `discriminative_axis` per entry; new `--cdp` CLI flag at [`compiler/app/Main.hs`](../compiler/app/Main.hs); `CDPScope` default flipped to `CDPScopeCoreOnly` (commit `3af3c06`, 2026-05-29); `SDef`-only measurement live; scope-filtering wired in `CDP.hs:computeCDPFor`; `WarnSpecTooTightForOmega` wire-line label `"spec-too-tight-for-omega"` active (F-005 CE follow-up complete). 712 Haskell + 58 Python tests (LT-CDP adds 22 regression C1–C22; F-001 adds 4 DF-1–DF-4, commit `e5e6d04`; F-006/F-005 adds 6 F6-1–F6-6, commit `6f2ea39`). **JSON-AST `schemaVersion` not bumped** — deferred to LT-INV bundle (Outcomes 0/1, `0.5.0 → 0.6.0`) or coordinates with LT-PPR (Outcome 2, `0.5.0 → 0.5.1`) per [`v0.11-cross-proposal-rollback-discipline.md`](design/v0.11-cross-proposal-rollback-discipline.md) §2. **Owed to `experiment-lead`:** CDP-0 baseline DP report at `experiments/cdp-0/runs/<ts>-baseline/baseline.json` *before* the LT-INV §8 gate runs (proposal §2 Rev 2 baseline-first sequencing). Subsumes DP-FORM-1 + TRUST-DP-1 sub-items. |
| **LT-PPR** (predicate-carrying `?proof-required`) | **Settled (Rev 2) — review folded; engineer build shipped (3391713)** | LT proposal at [`docs/design/proof-required-predicate-carrier-proposal.md`](design/proof-required-predicate-carrier-proposal.md) settled Rev 2 (commit `5f31580`, 2026-05-25) incorporating six gaps + two author-question answers from professor review; review folded as §"Appendix — Professor review log" (commit `f1ef0c0`); standalone archived to [`docs/archive/professor-reviews/proof-required-predicate-carrier-review.md`](../docs/archive/professor-reviews/proof-required-predicate-carrier-review.md). Engineer build shipped (3391713); supersedes deferred-exploration at [`docs/design/proof-required-predicate-carrier.md`](design/proof-required-predicate-carrier.md). Cross-proposal shipping under gate outcomes per [`v0.11-cross-proposal-rollback-discipline.md`](design/v0.11-cross-proposal-rollback-discipline.md). |
...
| **PROOF-ARTIFACT** (unified reproducible verification artifact) | **DESIGN PENDING** — `[LT+CT]`; origin: two-round external critique (convergent recommendation, both rounds) | A single object carrying source hash, AST hash, erased core, VC(s), solver result + solver version, callee assumptions, sidecar hashes, fallback reasons, and the evidence/discriminative/effect axes — *consolidating* the existing scattered surface (`.fq`, `--trust-report`, `--obligation-report`, `.verified.json` sidecar), NOT a fragment expansion. Language-team proposal at [`docs/design/proof-artifact-proposal.md`](design/proof-artifact-proposal.md) (Rev 1, drafted in parallel). Tag `[LT+CT]`: design first, then build touching [`TrustReport.hs`](../compiler/src/LLMLL/TrustReport.hs) / [`VerifiedCache.hs`](../compiler/src/LLMLL/VerifiedCache.hs) / [`ObligationAssembly.hs`](../compiler/src/LLMLL/ObligationAssembly.hs) / [`FixpointEmit.hs`](../compiler/src/LLMLL/FixpointEmit.hs) + [`docs/llmll-ast.schema.json`](llmll-ast.schema.json). |
| **OBLIG-1-FOLLOWON** (populate the three reserved checkout-brief / obligation-report fields) | **PARTIAL** — `expected_return_type` value path **shipped v0.13.1** (DEF-RET); `available_functions` shipped via DEMO-COMP; `assumptions` still reserved | Populate `expected_return_type`, `available_functions`, `assumptions` (formerly `reserved`, returns-absent per `LLMLL.md:1823-1824`). The hole type already exists in the sketch path (`holeInferredType`, [`HoleAnalysis.hs`](../compiler/src/LLMLL/HoleAnalysis.hs); emitted by `llmll holes --json` / `typecheck --sketch`) and the schema already defines the fields ([`docs/llmll-ast.schema.json`](llmll-ast.schema.json) `:1068`, v0.3.5-tagged) — wire the sketch hole-type into the [`Checkout.hs`](../compiler/src/LLMLL/Checkout.hs) brief assembly. `expected_return_type` shipped first via **DEF-RET** (the return-type surface that records `HoleTyped T` for a function-body hole; see DEF-RET row); `available_functions` populated via DEMO-COMP; `assumptions` semantics may want a short language-team pin before build. No schema bump expected. Supersedes the never-completed v0.3.5 §C acceptance for these two fields (see §C block below). |
| **DEF-RET** (optional return-type annotation + return-refinement discharge) | **✅ Shipped — Unit 1 v0.13.1, Unit 2 v0.13.2** | Unit 1 (commit `a948deb`, v0.13.1): optional `-> RetType` surface on `def`/`def-shell` (both frontends + schema `0.6.0 → 0.7.0`), body checked against it, `expected_return_type` populated for a function-body hole (the OBLIG-1-FOLLOWON value path); a refinement-aliased return fell back conservatively off `verified`. Unit 2 (commits `0b27be5`/`12210b0`, v0.13.2): a refinement-aliased return **discharges** via `augmentContractPost` (the guarantee-side dual of `augmentContractPre`) — the body-VC proves it (`verified` / `refuted` / `erBodyFallback`), and it is **exported as a caller-assumable guarantee** (folded into the effective post in `buildContractEnv`; assume-guarantee, §5.3.4 meet); a bare `-> RetType` function is credited `verified`. The staleness hash now covers the augmented contract (a `-> RetType` annotation or alias-predicate edit invalidates a stale verified sidecar — closes a cache-poisoning hole **and the pre-existing param-side NIW staleness hole**); the solver-less `--trust-report` staleness-bypass is fixed. **No schema change in Unit 2** (`schemaVersion` stays `0.7.0`). Design: [`def-return-annotation-proposal.md`](design/def-return-annotation-proposal.md) (Rev 3); staleness review [`def-ret-staleness-hash-review.md`](design/def-ret-staleness-hash-review.md). 908 Haskell + 62 Python tests. |
| **XMOD-STALE** (persistent ModuleCache coherence under LSP/Serve/watch) | **Tracked — build-driver obligation, not trust-logic** | Invariant: an importer's verified verdict is valid only relative to the callee-sidecar digests it was checked against; **a callee source/sidecar change must evict the cached callee env.** `collectAllContractStatus` is sound given a fresh cache — it recomputes each callee's augmented hash over the callee's *own* live statements (alias map in scope) and compares to the callee's persisted `erVerifiedHash`, so the CLI (recompute-per-build) is unaffected and no cross-module recompute lacks the callee's alias map. The only residual hazard is a **long-lived `ModuleCache`** (`Serve.hs`/LSP/watch) that fails to invalidate a callee `ModuleEnv` on a callee source-or-sidecar mtime change — a build-driver obligation, **not** trust-logic. Per [`def-ret-staleness-hash-review.md`](design/def-ret-staleness-hash-review.md) §G2 (GHC `mi_usages` / Bazel early-cutoff analogs). Confirm whether any long-lived-cache consumer exists today before building. |
| **CDP default-on** (promote `--cdp` / `--weakness-check` / `--spec-coverage` into the default serious-verify path) | **DECISION OPEN** — origin: round-1 + round-2 external critique | Currently opt-in. Pending: (a) a warn-not-error false-positive tier, (b) the `§4.4.6` observational-vs-semantic caveat surfaced in the headline (not just the body), (c) `(spec-entropy :intentional)` over-annotation handling. MAY fold into PROOF-ARTIFACT. Argument (round-2 category-theory framing): if "patches conservatively weaken obligations" is to be a *law* and not a *lint*, CDP cannot stay opt-in. |
...
| **REF-META-2..5** (solver-completeness, erasure, predicate WF, typing judgment) | **REF-META-2..5 all promoted (✅ shipped v0.12.0) — metatheory of record complete** | REF-META-2 (Σ_auto solver-completeness, `LLMLL.md §5.3.3`) + REF-META-3 (predicate WF, `§3.4.4`) promoted 2026-06-12; REF-META-4 (erasure theorem, `§3.4.5`) promoted 2026-06-13 ([`ref-meta-4-erasure-proposal.md`](design/ref-meta-4-erasure-proposal.md), Settled Rev 3); REF-META-5 (type-assignment judgment with hole-directed checking, `§3.4.6`) promoted 2026-06-14 ([`ref-meta-5-type-assignment-proposal.md`](design/ref-meta-5-type-assignment-proposal.md), Settled Rev 2) — documents the checker as local type inference (Pierce–Turner) completing REF-META-1's checking-mode rule, not a "complete bidirectional system" (D–K §3.1). The REF-META metatheory-of-record (1–5) is complete. Sequenced in [`docs/design/v0.12-direction.md §1`](design/v0.12-direction.md). |
| **Bundle B** (effect rows on `Command`) | **Reclassified** out of indexed-types research bundle per memo §4 | **B0** ✅ shipped v0.12.0 (commit `b2d9c1a`, 2026-06-14): per-function `effect_summary` in `--obligation-report` (sound may-over-approximation; `"unbounded"`/⊤ at delegate/FFI boundaries; informational, orthogonal to trust; obligation-report `orSchemaVersion` 0.11.0→0.12.0). Settled proposal [`bundle-b0-effect-summary-proposal.md`](design/bundle-b0-effect-summary-proposal.md) (Rev 2). **B1** (explicit `Command {stdout} a` annotations, opt-in): v0.12 LT proposal; engineer build gated on the B0 experiment (`experiments/minimal-agent/001-two-agent-auth`). **B2/B3**: v0.13+ at earliest. See [`docs/design/v0.12-direction.md §3`](design/v0.12-direction.md). NOT WASM-coupled. |
| **Non-int refinement widening** (`EVar-refined-string`, `EVar-refined-list` in strict core) | **Phase 1 shipped v0.12.0 — intro+elim** | Path-a axiomatization gate **cleared**: measure terms (`string-length`/`list-length`) emit as function-sorted uninterpreted functions with per-term ground range facts (REF-META-2 §4), discharged QF-LIA+EUF. Refinement-aliased params get carrier sorts and their predicate assumed in the body VC; stacked aliases conjoin (§3.4.4). Commits `1b21973` (IR) / `a0f352b` (measure values) / `0a3c5c2` (refined params) / `0b05916` (intro-side call-pre). REF-META-2 §4 + REF-META-3 §4.2 + [`docs/design/v0.12-direction.md §2`](design/v0.12-direction.md). Caller-side complete (F-NIW-2). Phase 2 (ADT-field refinements, [v0.12-direction §2](design/v0.12-direction.md)) remains future. |
| **F-NIW-2** (intro-side call-pre for string/list-refined params) | **Shipped v0.12.0** — commit `0b05916` | Intro-side call-pre now fires: carrier-aware call-argument translation (bare-var args translate to their `FQVar` regardless of sort) + caller-side carrier binding (`collectCallArgCarrierVars`) let a caller passing a value to a measure-refined param prove the refinement at the call site; fixed a latent `applySubst` gap on measure-application (`FQApp`) args. Measure-refined params now get full intro+elim, matching `int`-refined. e2e: a plain string passed to a `Word` param is refuted; a `Word`-typed caller param chains SAFE. |
| **F-NIW-3** (qualifier-identifier sanitization) | **Shipped v0.12.0** — commit `25c489d` | LLMLL identifiers admit `-`/`.`/`?` (`Lexer.hs:314-315`) but liquid-fixpoint accepts only `[A-Za-z0-9_]`; a kebab-case name with a comparison clause crashed the solver (`unexpected '-'`). Sanitized (`sanitizeFQId`, non-legal char → `_`) at the single `FixpointIR` emission chokepoint — qualifier names, binders, refs, `freshName` result vars, type/ctor names — identity on already-legal names (byte-identical `.fq`). Surfaced during NIW, not NIW-caused. Known limitation: the mapping is not injective (`a-b` vs `a_b`); pathological for kebab-case source. |
| **F-NIW-4** (sound compositional call-pre over let-bound call results) | **Shipped v0.12.0** — commit `a801112` | A let-bound call result used as a later call's argument left the intermediate a free variable; `banking_ledger/withdraw-twice` was *spuriously* `verified` (masked pre-F-NIW-3 by hyphen mis-lex; F-NIW-3 surfaced it). Fix: alias the let var to the call result var; thread prior calls' (result var, assumed post) into the later call's call-pre obligation (assume-guarantee). `banking_ledger` now genuinely SAFE + passes `--strict-verified-core` — the LT-INT-row claim at `:170`/`:329` was spurious until this commit. Pre-existing, F-NIW-3-exposed; professor-confirmed sound. The chained rule is now stated in `LLMLL.md §5.3.4` (spec-record catch-up). |
| **F-NIW-4b** (let-bound non-call value into a later call's pre) | **Shipped v0.12.0** — commit `3b74d24` | A `let`-bound *non-call* (`SimpleVC`) value used in a later call's precondition was not threaded — `prependLB` parks such bindings at the leaf, so `collectCallPreObligations` did not see them at the call point. Fix: `subtreeLbs` collects the `CallVC` continuation's let-bindings and `collectCallPreObligations` threads them into the call-pre environment as defining equalities (`v = rhs`), filtered by an `inScopeLbs` partition fixpoint to the subset whose free vars are all in scope at the call (param binders + prior call results) — branch-local and after-call bindings excluded. Completes compositional call-pre for the non-call case; F-NIW-4 covered call results. |
...
**Current shipped: v0.13.0** (2026-06-19). The minor/patch line beyond v0.10 — v0.11 (Core/Shell Inversion + Evidence-Axis Enrichment), v0.12.0 (Refinement Metatheory of Record + Effect/Authority Summaries), v0.12.1 (def-logic Removal + def-invariant Node), and v0.13.0 (Caller-Obligation Axis + Verified Composition) — is recorded under [Shipped Releases](#shipped-releases). 900 Haskell + 62 Python tests. (The v0.8–v0.10 critical-path ASCII diagram above is retired into a collapsed block; per-version status for v0.11→v0.13.0 is in Shipped Releases.)
...
**Theme:** Complete the REF-META metatheory of record; add per-function effect/authority summaries.

> Shipped 2026-06-15 (commit `f80e081`). REF-META 1–5 (solver-completeness `§5.3.3`, predicate WF `§3.4.4`, erasure `§3.4.5`, type-assignment `§3.4.6`); Bundle B0 per-function `effect_summary` in `verify --obligation-report` with cross-module composition (∅-iff-fully-walked; obligation-report `orSchemaVersion` `0.12.0`); NIW Phase 1 refinement-aliased non-int measures (QF-LIA+EUF); JSON-AST `module`-form flattening + `wasi.fs` namespace reconciliation. `schemaVersion` `0.6.0`; `trust_report_version` `1.3.0`. 855 Haskell + 62 Python tests. See `CHANGELOG.md` §v0.12.0.
...
> [!NOTE]
> **C5 (monomorphize polymorphic Σ signatures) — specced, not shipped — see OBLIG-1-FOLLOWON.** C5 can be implemented as a `Map Name Type` substitution pass over the `available_functions` list in the checkout response: when Γ contains `xs : list[int]`, rewrite `list-head : list[a] → Result[a, string]` to `list-head : list[int] → Result[int, string]`. This is a straightforward find-and-replace, not unification. Implement after C1–C4 land. (~1 day)

| # | Action | Module | Status |
|---|--------|--------|--------|
| C5 | Monomorphize polymorphic Σ signatures against concrete Γ types in checkout response via `Map Name Type` substitution (`monomorphizeFunctions`). INV-2: presentation-only, no `builtinEnv` mutation. | `Checkout.hs` | ⚠ (superseded — never shipped; tracked under OBLIG-1-FOLLOWON) |
...
> [!NOTE]
> **Acceptance reconciliation (2026-06-20, language-team drift finding).** Of the two `checkout`-response fields named above, only `in_scope` actually shipped (v0.11.2). `expected_return_type` and `available_functions` were specced here but never wired — they remained `reserved` / returns-absent through v0.11.2 (per `LLMLL.md:1823-1824`). The first two acceptance bullets therefore over-claimed for those two fields. Completion of `expected_return_type` and `available_functions` (plus the related `assumptions` field) is now tracked under the **OBLIG-1-FOLLOWON** Active-Items row (v0.12+ post-freeze lane). C5 monomorphization (the NOTE above) is part of that follow-on, not shipped here.
