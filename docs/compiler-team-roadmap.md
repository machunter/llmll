...
| **COVERAGE-TIER** (`--spec-coverage` tier column ignores solver evidence) `[CT]` | **Open — medium (report surface, TERM-REPORT-PLAIN family)** | The coverage summary prints `Verified: 0 / Asserted: 3` while the same tree's trust report says `verified: 3` — reproduced with and without `--strict-verified-core` (found by the RFC 1982 run, F-1982-1; independently re-reproduced). Same-visit item: the PBT multi-callee diagnostic still says "wait for :subject metadata in OBLIG-PBT-4" though `:subjects` is shipped and works (F-1982-2). |
