...
| **TERM-REPORT-PLAIN** (plain `verify --trust-report` misses same-run descent marks) `[CT]` | **Open — medium (report surface, not soundness)** | The plain (non-strict, non-CDP) trust-report path never applies `markDescentDischarged`/`markMeasureNotDecreasing` — a same-run discharged `(decreases)` still renders `termination_unverified: true` + `partial_fns`, and a bad measure gets no `measure-not-decreasing` drift stamp, even though the sidecar records `termination_verified: true`. Strict-core and CDP paths (`Main.hs:1619/1649`) are correct; the plain early render (`Main.hs:1192`) is not. Makes the v0.14.25 CHANGELOG claim false on the plain path. Found by the examples-hardening pass (v0.14.35). |
| **RUN-EXEC** (`llmll run` broken for argument-less programs) `[CT]` | **Open — low** | Pre-existing, found by the LEVER-A0 pass: `doRun` never passes the executable name to `stack exec` (`app/Main.hs:881-882`), so `llmll run` on a program without args always fails with stack usage text. One-line fix + a runtime test. |
...
|---|---|---|---|---|
| **A** | **Theory of arrays** for `bytes[n]` / `map[k,v]` | QF theory of arrays (`select`/`store` + extensionality) — **decidable**, polite-combines with QF-LIA; **stays in `Σ_auto`** | array index-in-bounds (real memory-safety, not a length proxy); map get-after-put / key-presence | **A0+A1 SHIPPED (v0.14.33/34) — `bytes[n]` verifies; next = A2 map discharge** — design [`data-scope-lever-a-arrays-proposal.md`](design/data-scope-lever-a-arrays-proposal.md) (Rev 1.1) / [`-review.md`](design/data-scope-lever-a-arrays-review.md) / [`-feasibility.md`](design/data-scope-lever-a-feasibility.md). A1 = bytes params/returns lower to the native array sort under an activation gate; `bytes-get`/`set`/`length`/`zero` reflect exactly (§6.1 exact-reflection rule normative); index-in-bounds + value-range are PROVE call-site obligations; the §11 off-by-one crux refutes with localization; 85-example verdict inventory = zero flips. **A2 open (4–6d)**: map reflection (int-0/1 presence array — bool-element arrays crash the pinned fixpoint SMT bridge, probe-proven), `ELet` array-typed RHS, composite map-valued pair-threading, and re-rooting `injectRangeFacts` byte-range synthesis on bytes-sorted terms (A1 landmine comment in `FixpointEmit.hs`). Routed to language-team: v1 `bytes-zero` context-rule blessing; contract-position `bytes-get` well-formedness (pre-existing partial-function-in-contract class). Then A3 (classifier/brief integration) + A4 (flagship data example). |
