...
|-----|-------------|---------|
| **LEAN-GA** | Real Leanstral integration (non-mock proofs). When shipped, adds `verified-lean` evidence kind. **Blocker re-scoped (2026-07-04, spike):** external availability is now *partly cleared* by Mistral's Leanstral 1.5 (open weights + free API + Lean LSP MCP), but the real gate is an **internal three-layer rebuild** — (1) `LeanTranslate.hs` translates the contract not the obligation (body-decoupled → misstated theorems), (2) nonlinear-body obligations land at `erBodyFallback→asserted` and are never routed to `?proof-required`, (3) `lean-lsp-mcp` is a *checker* not a *prover* (needs the model-search + kernel-check architecture). See [`docs/design/leanstral-integration-scope.md`](design/leanstral-integration-scope.md). | Internal three-layer rebuild (external availability partly cleared) |
