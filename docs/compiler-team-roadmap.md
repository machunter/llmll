...
|---|---|---|---|---|
| **A** | **Theory of arrays** for `bytes[n]` / `map[k,v]` | QF theory of arrays (`select`/`store` + extensionality) — **decidable**, polite-combines with QF-LIA; **stays in `Σ_auto`** | array index-in-bounds (real memory-safety, not a length proxy); map get-after-put / key-presence | **A0 SHIPPED v0.14.33 (verification-inert surface) — next = A1 bytes discharge** — design [`data-scope-lever-a-arrays-proposal.md`](design/data-scope-lever-a-arrays-proposal.md) (Rev 1.1) / [`-review.md`](design/data-scope-lever-a-arrays-review.md) / [`-feasibility.md`](design/data-scope-lever-a-feasibility.md); exact-reflection rule normative (§6.1), int-only keys v1, int-0/1 presence array (bool-element arrays crash the pinned fixpoint SMT bridge — probe-proven). A0 = the eight builtins typecheck/compile/run with PROVE-polarity pres as runtime assertions, byte-inert (85-example sweep, 0 `.fq` diffs). A1 (3–4d) = `bytes[n]` reflection + off-by-one refute crux, gated on the examples verdict inventory (`typeToSort` TBytes/TMap currently defaults `FQInt`); A2 (4–6d) = maps + pair-threading. Pending language-team blessing: the v1 `bytes-zero` context rule (whole body of a literal `-> bytes[n]` def — no let-annotation surface exists to widen it). |
