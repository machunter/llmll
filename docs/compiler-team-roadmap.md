...
|---|---|---|---|---|
| **A** | **Theory of arrays** for `bytes[n]` / `map[k,v]` | QF theory of arrays (`select`/`store` + extensionality) — **decidable**, polite-combines with QF-LIA; **stays in `Σ_auto`** | array index-in-bounds (real memory-safety, not a length proxy); map get-after-put / key-presence | **A0+A1+A2 SHIPPED (v0.14.33/34/37) — `bytes[n]` AND `map[int,int]` verify; next = A3 classifier/brief integration** — design [`data-scope-lever-a-arrays-proposal.md`](design/data-scope-lever-a-arrays-proposal.md) (Rev 1.1) / [`-review.md`](design/data-scope-lever-a-arrays-review.md) / [`-feasibility.md`](design/data-scope-lever-a-feasibility.md). A2 = two-array int-0/1 presence encoding (`m$has`/`m$val`); `map-has`/`map-get`/`map-put`/`map-empty` reflect exactly; key-presence = PROVE call-site obligation; let-bound put/get pipelines discharge (`expandMapLets`); aliased-key + dropped-put twins refute; the A1 `injectRangeFacts` landmine resolved (byte-range facts bytes-rooted only, phantom-fact unsoundness demonstrated absent); F7 sweep zero diffs. **A2.1 residue (open)**: string/bool-valued map reflection, cross-call map assume-guarantee (component-aware substitution), non-pure map-returning bodies (read-modify-write `(map-put m k (+ (map-get m k) 1))` falls back). **A3 next**: classifier/brief integration — `contract_fragment` recognizes exactly-reflected array contracts (today they classify `non_qf_lia`→Advisory while discharging body-faithful), checkout-brief array vocabulary; acceptance fixture = `examples/bytes-bounds/`. Then A4 (flagship data example). Routed to language-team: v1 `bytes-zero` context-rule blessing; contract-position `bytes-get`/`map-get` well-formedness (partial-function-in-contract class). |
