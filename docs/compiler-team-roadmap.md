...
> **Status:** Active — v0.10.5 shipped (PBT Complex-Type Generators + Trust-Report Write-Back). 614 Haskell + 37 Python tests passing  
...
| 8 | **OBLIG-PBT-4** | **[CT]** PBT linkage v2 — `:subject` keyword metadata on `(check ...)` blocks (`:subject f` for singleton explicit, `:subjects [f g]` for joint-evidence opt-in) and coverage-instrumented `evaluatedSamples` (distinguish witnessing evaluations from vacuous-implication evaluations via QuickCheck `classify` / `cover`). Annotated check blocks bypass the head-position scan; existing programs continue to work. Spec disclosure in `LLMLL.md §4.4.1` / §5.1.1 stops describing `n` as "a lower bound on assertions of the postcondition" once coverage-instrumented counts ship. Designed in `docs/design/oblig-pbt-3-proposal.md` §11 / §5 as scope deferred from OBLIG-PBT-3. | OBLIG-PBT-3 | ☐ |
