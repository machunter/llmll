...
| **OBLIG-1-FOLLOWON** (populate the three reserved checkout-brief / obligation-report fields) | **OPEN** — `[CT]`, small/plumbing; origin: language-team drift finding 2026-06-20 | Populate `expected_return_type`, `available_functions`, `assumptions` (currently `reserved`, returns-absent per `LLMLL.md:1823-1824`). The hole type already exists in the sketch path (`holeInferredType`, [`HoleAnalysis.hs`](../compiler/src/LLMLL/HoleAnalysis.hs); emitted by `llmll holes --json` / `typecheck --sketch`) and the schema already defines the fields ([`docs/llmll-ast.schema.json`](llmll-ast.schema.json) `:1068`, v0.3.5-tagged) — wire the sketch hole-type into the [`Checkout.hs`](../compiler/src/LLMLL/Checkout.hs) brief assembly. `expected_return_type` ships first; `available_functions` depends on it (monomorphized Σ); `assumptions` semantics may want a short language-team pin before build. No schema bump expected. Supersedes the never-completed v0.3.5 §C acceptance for these two fields (see §C block below). |
