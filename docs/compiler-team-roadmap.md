...
| **CONTRACT-READ-LINT** (lint for contract-position partial reads) `[CT]` | **Open — low** | Per the settled [`contract-position-reads-disposition.md`](design/contract-position-reads-disposition.md): contract reads are total selects and sound in both directions, but a literal-index-out-of-literal-`n` read in a `pre`/`post` is an author-error surface (verified-yet-always-crashing contract). Ship a non-blocking W-series warning, JSON-visible (the F-001 lesson), scoped to the decidable literal slice. |
...
| **R3 — Spec-from-RFC pipeline** (RFC → LLMLL contracts with `:source` clause provenance) | **Evaluation run EXECUTED (v0.14.39) — all six §4 criteria PASS** | Pipeline validated by procedure on an unseen RFC: [`examples/rfc1982_serial/`](../examples/rfc1982_serial/) (RFC 1982 serial arithmetic — `verified: 3`, two refuting mutants incl. the historical naive-`<` DNS bug, weak-spec exhibit, first persisted G1 clause inventory); findings in [`experiments/rfc1982-eval/findings.md`](../experiments/rfc1982-eval/findings.md). Pipeline doc at **Rev 1** (F-1982-3/-4/-5 folded; G1+G4 closed; evaluation-outcome appendix). Remaining gaps: G2 (per-conjunct `:source`, deferred behind a schema-bump occasion), G3 (accepted). The R3 line is **complete** absent a new RFC candidate or G2's occasion. |
