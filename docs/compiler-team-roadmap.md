...
| **R5 — Differential implementation pressure** (`llmll checkout --multi N`; N agents fill one hole; `diverge-report` divergence analysis) | **SHIPPED (observational stages 1-2, v0.14.7) + hardened + validated at scale** | `DivergenceCheck.hs` (status partition + Ω-replay witness) + `Checkout.hs` (`checkoutHoleMulti` / `diverge-report`, isolated scratch per token). An existential **under-constraint witness generator** (`under-constraint-witness` / `no-divergence-observed` / `suppressed-intentional`), not a CDP-score extension. Post-ship fixes: **sibling-call classification (v0.14.9)** — helper-calling fills classify in the shared-program context (pinned to trusted siblings) instead of dropping to `type-error`; **concurrency-safe temp file (v0.14.10)**. Validated at scale (`experiments/minimal-agent/r5-campaign/`, 10-hole tightness×composition grid × 3 frontier families × 5 reps): witnesses are sound and never spurious, sibling-suppression incidence 0; **headline finding — a loose contract is necessary but NOT sufficient for a witness** (witness rate ≈ {looseness} × {model divergence}; `no-divergence-observed` never certifies a tight spec — `range_mid` is the live counterexample). Remaining: stage-3 (SAFE-direction dedup prover) is a deferred NO-GO (marginal; the pipeline has no counterexample-model extraction, so the witness comes from stage-2 Ω-replay); the open experimental follow-on is forced-diversity knobs (temperature / more families) to separate the looseness × divergence factors. Proposal Rev 4 [`differential-implementation-pressure-proposal.md`](design/differential-implementation-pressure-proposal.md); `r5-campaign/findings.md`. |
