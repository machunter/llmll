...
> **XMOD-COMP (cross-module verified composition) — goal reached.** Layers 1–3 shipped earlier (admission / type-alias type-check / trust-tier edge); layer 4 (body-VC emission — including the imported refinement-aliased-param case, verified body-faithful end-to-end) by **XMOD-AG (v0.14.17)**. Sole residual (confirmed empirically): the `checkout` brief's `consumed_guarantees` channel uses same-file `cenv` (`Main.hs:1951` — XMOD-AG made only the verify path cache-aware), so an imported callee's guarantee is **absent from the brief** (control: a same-file callee appears; the imported one does not). Brief-completeness / DX, **not soundness** — the guarantee is still consumed in the verify VC and the composer verifies; the fix is to make the checkout `cenv` cache-aware. See [`cross-module-composition-finding.md`](design/cross-module-composition-finding.md).
