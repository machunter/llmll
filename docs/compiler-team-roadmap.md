...
| **COMP-3b-general / COMP-4** (refinement-return discharge through nested / non-`Result` matches) | **Phase 1 SHIPPED v0.13.5; remainder P2 — open** | **Shipped (Phase 1, v0.13.5):** nullary-enum types matched and used as values in a `def` body reach body-faithful `verified` via a pre-translation desugar (`desugarCtorValues`: ctor-value `EVar→ELit` tag, enum `EMatch→` nested `EIf`); stays QF-LIA, sidesteps the term-type-env work below for the nullary case. **Remaining:** **COMP-3b-general** (match nested under `let`/`if`) still needs a term-type env threaded through `bodyToPredM` (SortEnv is int-only); **COMP-4** (two-arm user ADTs with payloads) additionally needs exhaustiveness + selector theory; **Phase 2** (engineer-surfaced) — payload-carrying variants for full illegal→`REJECTED` totality, plus runtime/PBT evaluation of constructor values (currently verify-time only). Constructor-dependent posts stay a deliberate fallback. *(Taxonomy note: the desugar slice does not map onto the original COMP-3b-general/COMP-4 split; a clean re-cut is a language-team/compiler-engineer call.)* |
