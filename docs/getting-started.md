...
- Schema `schemaVersion` is `0.5.0`. A bump to `0.6.0` is deferred pending the redesigned §8 gate experiment post-F-GATE-8 (rollback path 1, 2026-05-29); submitted `.ast.json` files must carry `"schemaVersion": "0.5.0"`. `kind:"def"` / `kind:"def-shell"` nodes are available to opt-in consumers under `--grammar=core-inversion`.
