...
3. **One verdict across `check` and `build`.** Remove the cases where `check` passes and the Haskell build then fails, and where a diagnostic misleads: a binding named `show`, cross-module constructors, a type name declared by two modules, a `returns` key accepted and ignored, `diverge-report` run without a solver. (G3)
...
8. **Instruments over the repository.** The published JSON schemas reject what the compiler emits today (SCHEMA-TRUTH-1, SCHEMA-DRIFT-1); a shipped example does not type-check (TOTP-CHECK-1); some gates report success when they cannot decide. (G8)
