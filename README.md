...
> **v0.10.7 is shipped.** Patch release — `EOp` arity and argument-type checking fix (TC-EOP-1): `inferExpr (EOp op args)` at `compiler/src/LLMLL/TypeCheck.hs:981` now mirrors the `EApp` `structuralUnify` loop, so `(+ 1)`, `(+ "x" 1)`, `(not 1)`, `(= 1 "1")`, `(and true 0)` and other arity-bad or type-incorrect operator calls that pre-v0.10.7 silently typechecked now produce structured `type-mismatch` and arity diagnostics; polymorphic `=` / `!=` unify both operands against one bound `TVar` (no `any × any → bool` degrade). Plus joint PBT witness scalar exclusion (OBLIG-PBT-5a): `:subjects [f g …]` joint lifts (OBLIG-PBT-4) previously over-credited the scalar `tested` count by N for one shared property body; v0.10.7 computes joint-witness hashes (post-clause hashes appearing on ≥2 distinct subjects) and demotes pure-joint `DLTested` entries to `DLAsserted` at trust-report classify time, with a per-entry `joint_pbt_witness: true` JSON flag and a top-level `joint_pbt_witnesses: [{hash, subjects}]` grouping. Solo+joint mixes keep their `+1` credit; source-annotated empty-witness `DLTested` is unaffected. Underlying `EvidenceRecord` and `pbt_witnesses` stay intact for the post-freeze clean fix (OBLIG-PBT-5b). `trust_report_version` stays `1.1.0`; JSON-AST `schemaVersion` stays `0.5.0`; verification fragment unchanged. 656 Haskell + 37 Python tests passing. See [`CHANGELOG.md`](CHANGELOG.md).
