...
| `llmll diverge-report <file.ast.json> <session>` | R5: collect a divergence session's fills and emit the `divergence_witness` record. The session id is the one returned by `checkout --multi`. A fill the solver could not grade (no solver, or no verdict) is listed under `status_partition.unavailable`, the record carries `solver_available`, and the command exits 3. |
