...
**Compiler-prerequisite CLOSED (2026-06-15):** cross-module `effect_summary` propagation shipped — `85d2a7d` (build, 851 → 855 tests; the `∅`-iff-fully-walked invariant, combined-statements implementation), docs `6215add` (CHANGELOG + `LLMLL.md §11.2`), settlement recorded in `docs/design/bundle-b0-cross-module-addendum.md` (`f78c1ca`); the disproved B0-experiment-gates-B1 condition was retired in `docs/design/v0.12-direction.md` (`f78c1ca`). **The 004 experiment itself remains OPEN** — gated on the separate Hub-interface opacity layer (a Hub-fetched interface that publishes a signature + `effect_summary` with the body absent; a local imported `.llmll` is agent-readable and telegraphs). The compiler can now propagate cross-module effects; constructing a non-telegraphing-yet-counted temptation still needs that opaque-interface layer.
