...
- **`llmll test` now executes `(check ...)` blocks whose `for-all` bindings include complex types.** Prior to this change, any property whose bindings included `TPair`, `TList`, `TResult`, `TSumType`, or a `TCustom` alias resolving to a non-primitive type silently skipped with reason "Property contains non-constant expressions — requires full runtime evaluation". The PBT sample generator (`PBT.hs:generateValue`) had a catch-all that returned `LitInt` for any non-primitive, producing type-incorrect samples that the static evaluator could not reduce. Closes F-032 (repair-loop apparatus postmortem Addendum 16). Empirical: a property with binding `[p: (int, int)]` and body `(= (+ (first p) (second p)) (+ (second p) (first p)))` now runs `PBTPassed, samples_run: 100`; Phase-2 cell c01's three properties (`runs/20260512T031938Z-.../solution.llmll`) lift from `0/3 skipped` to `3/3 passed` under `llmll test`.
- **`generateValue` retyped from `Type → IO Literal` to `TypeAliasEnv → Int → Type → IO Expr`.** New cases for `TPair` (recurse into both sides, return `EPair`), `TList` (length-bounded cons-chain via `EApp "cons" [..]` / `EApp "nil" []`), `TResult` (random tag + recurse into payload, return `EApp "Success"/"Error" [..]`), `TSumType` (random constructor + recurse, return `EApp ConsName [..]`), `TCustom` (resolve via `expandAliasPure` against the `TypeAliasEnv` built from module `STypeDef`s, with cycle guard). Recursion bounded by `maxGenDepth = 5` and per-list `listMaxLen = 8` for guaranteed termination on self-referential aliases.
- **`evalExprStaticWith` extended for `EPair` and `ELambda`.** Pairs reduce element-wise (`EPair a b → EPair va vb`); lambdas are first-class values passed through to higher-order builtins. Same call sites used by contract VC generation at `Contracts.hs:275, 286` so the extension benefits VC constant-folding as a side effect.
- **`evalBuiltinApp` signature refactored from `Name → [Expr] → Maybe Expr` to `FuncEnv → Int → Name → [Expr] → Maybe Expr`.** New builtin reductions covering the §13 core required for realistic property bodies: `pair` / `first` / `second` (TPair); `cons` / `nil` / `list-head` / `list-tail` / `list-is-empty?` / `list-length` / `list-append` (TList); `list-fold` / `list-map` (higher-order, fuel-decremented via new `applyLambda` helper that beta-reduces an `ELambda` against argument values); `unwrap-or` / `some` / `none` / `is-some` (Option-shaped helpers).
- **`maxFuel` raised from 64 → 256** to give realistic agent emissions (transfer → find-balance → list-fold chains) room to reduce. With F-032's list generators bounded at length 8 and depth 5, this restores termination headroom that the prior 64-step budget exhausted before reaching a terminal Bool. The 4× bump preserves the non-termination guard on self-recursive `def-logic` (which would hit any finite cap eventually).
- **`tryQuickCheck`'s `isSimpleType` whitelist removed.** The prior `TInt | TBool | TDependent _ TInt _` gate excluded `TString` and every complex type from the QuickCheck-fallback path. Replaced with universal fall-through: any property where the static-eval path produces `Nothing` on at least one sample now routes through `runQC`, which uses the broadened generator and reports actual sample counts.
...
- **Tests:** 10 new tests under two `OBLIG-PBT-2` describe blocks in `Spec.hs` covering (a) `TPair` / `TList` / `TResult` / `TSumType` / `TCustom`-alias PBT execution end-to-end, (b) recursive `STypeDef` depth-cap termination, (c) `evalExprStaticWith` on `EPair`, (d) `evalBuiltinApp` `first`/`second`, `list-fold` (sum-fold over 3-element cons-chain via lambda → 6), `list-length ∘ list-append`. 594 → 604 Haskell tests; 37 Python unchanged.

### Compiler — Surfaced Follow-up (F-033, PBT-to-Trust-Report Write-Back)

- **F-032's implementation surfaces a separable upstream gap.** PBT now executes on complex-type properties (above), but `PBTPassed` results are not written back into the trust report — `DLTested n` evidence records are only constructed from source-annotated `:trust tested` markers (`Parser.hs:381`, `ParserJSON.hs:289`) and the `.verified.json` cache (`VerifiedCache.hs:52`). Empirical: `llmll test c01.llmll` reports `passed: 3`, but `llmll verify --trust-report c01.llmll` reports `tier_profile.tested: 0`. The "PBT passes → obligation crosses `asserted → tested` in the trust report" claim from the v0.10.3 CHANGELOG (MOD-PBT-1 closing paragraph) is aspirational; the linkage code does not exist.
- **Routed to a follow-up compiler-engineer turn.** F-033 is the orthogonal write-back finding: extend `llmll verify` (or `llmll test`) to invoke `runPropertyTests` and thread `PBTPassed` results into the trust-report enrichment pass (`TrustReport.hs:enrichEntry`). Out of scope for OBLIG-PBT-2.
