...
### Known Limitation — F-033 PBT-to-Trust-Report Write-Back Gap

- **`PBTPassed` results do not yet lift trust-report obligations from `asserted` to `tested`.** OBLIG-PBT-2 makes `(check)` blocks executable on complex types, but `DLTested n` evidence is only constructed from source-annotated `:trust tested` markers (`Parser.hs:381`, `ParserJSON.hs:289`) and the `.verified.json` cache (`VerifiedCache.hs:52`); no code path threads `runPropertyTests` outputs into `TrustReport.hs:enrichEntry`. Empirically: `llmll test c01.llmll` reports `passed: 3`, but `llmll verify --trust-report c01.llmll` reports `tier_profile.tested: 0`.
- **Routed as OBLIG-PBT-3** in `docs/compiler-team-roadmap.md` row 7. Requires a `language-team` design decision on the mapping (which `def-logic` entries earn `DLTested` credit from a passing `(check)` block) before implementation.
