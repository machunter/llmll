...
- **Not yet:** caller-side proof of measure-refined params (F-NIW-2 — the call-argument path is int-only; sound as verified-under-precondition, strict-core callers conservatively blocked). No JSON-AST schema change. **Tests: 824 Haskell + 62 Python** (inherited from engineer `stack test`).
