...
**The fix moved `sha1` out of the population of the check that found it.** `emitApp` now answers `(sha1_hash …)`, so the fold's classifier no longer sees the mangled builtin name at the head of the emission and files the name under the hand-written cases, which it exempts. That is why the pinned class size moves 71 to 70. Read plainly: the guard that caught this defect cannot catch a typo in the equation that repairs it, and the same is true of the other ten hand-written cases.

**So a build is the only oracle left for them, and the fixture now calls one.** [`scripts/build-smoke/smoke.llmll`](scripts/build-smoke/smoke.llmll) gains `smoke-sha1`. Nothing greps for the emitted call: if the equation names an identifier the preamble does not bind, the fixture does not compile and `BUILD-GATE-1` stage 4 fails. `sha1_hash` joins the emitted-preamble name lists in [`scripts/build_smoke.sh`](scripts/build_smoke.sh) **and** in its LLMLL port in the same change, which is the discipline the port's own comment asks for after two names went missing from that list by drift.

**The remaining gap is a measured number rather than an impression.** Counted over `scripts/build-smoke/*.llmll`: of the twelve hand-written equations the corpus called three (`pair`, `list-length`, `string-length`) and now calls four. **Eight are still called by no build fixture**: `first`, `second`, `bytes-length`, `bytes-get`, `bytes-set`, `list-nth`, `string-slice`, `string-char-at`. Five of those eight wrap an argument in `fromIntegral` at the `LT-INT` call seam, where a wrong wrapper is a GHC type error that nothing before a build would see.
...
hspec 1713 to 1768: 70 per-name examples in place of 16, plus one class-size assertion. **The test landed before the fix and was seen to fail**, once, naming `sha1` and the absent binding. A guard that has never fired is not known to work. Negative control: a probe calling `sha1`, `(sha1 (sha1 k))` and `hmac-sha1` builds through GHC and emits `(sha1_hash (k))`; mutating that call site back to the pre-fix lowering and rebuilding gives `Variable not in scope: sha1` at two sites, so the probe discriminates.

**`BUILD-GATE-1` was then run end to end against the widened fixture**: 13 stages PASS, one NOT EXERCISED (the `LC_ALL=C` encoding claim, which macOS cannot decide), stage 4 reporting **0 missing preamble definitions**. The fixture discriminates the same way the probe does, and this is the assertion that now lives in CI: rewriting the emitted `smoke_sha1` call site from `sha1_hash` back to `sha1` and rebuilding the generated package gives `Variable not in scope: sha1 :: [Word8] -> [Word8]`, so stage 4 fails on the reverted lowering rather than passing on it.

**The `TOOL-RFC-006` differential cover was run because this change touches both implementations.** It reports **0 MISSING** and its control cell passes with 14 reference stage verdicts graded, so the two sides still agree stage for stage: adding a called builtin to the shared fixture and a name to both lists creates no new stage and no new verdict line, which is why the port needed no counterpart and the `STAGES` table needed no row. **The cover still exits 1**, on cell 6, which is the open port-side existence-check divergence recorded as decision 1 of `tool-ll-RESTART.md` and predates this change; cell 6 was already failing and nobody had chosen its fix. It is fixed below.

**GRADED ON LINUX.** Run `31985443527` on `08c6088`, `version-gate` success in 26m10s, `docker-publish` success in 19m2s. Every figure above was macOS-local when written and CI confirms them: `1768 examples, 0 failures` and `179 passed, 6 skipped`. **`NOT EXERCISED` appears zero times in the run**, so Linux decided every stage of `BUILD-GATE-1`, including the `LC_ALL=C` claim that is a structural no-op on macOS and that failed on its first real execution the last time it reached CI. Both covers agree there as they do locally: `9 of 9 cell(s) agree (0 MISSING, 1 not run)` and `17 cells agree (3 negative controls, 14 mutations)`.

### `TOOL-006`: the port ran a binary that was not there, and called it eight defects

**Cover cell 6 was failing, and both sides exiting non-zero is what hid it.** That cell lets the execution fixture build, removes its binary, and plants a working stale one elsewhere. [`scripts/build_smoke.sh`](scripts/build_smoke.sh)'s `exe_path` makes two checks: the install root is non-empty, **and** an executable exists under its `bin/`. [`tools/build-smoke/buildsmoke.llmll`](tools/build-smoke/buildsmoke.llmll) made only the first, taking `stack path` answering at all as proof the binary was there. It then ran an absent path, read an empty capture, and reported **all seven** of stage 5's behaviour assertions as failures: one real defect described as seven imaginary ones, naming `wasi.fs.sha256`, `wasi.proc.run` and `json-parse` as broken when nothing had run. (Seven is the length of the port's `x-miss` list. The restart record said nine and this entry first said eight; both were counting the semicolons in the joined output, and two of the seven messages carry one internally.) An exit-code comparison grades that green, because both sides did fail. The cover compares the **cause**, which is the whole reason it compares verdict text rather than statuses.

**`P-C1` was measured before the fix was written**, it being measurement 3 of [`tool-ll-RESTART.md`](docs/design/tool-ll-RESTART.md) and owed to this fix by name. `wasi.fs.list` on a directory holding exactly one file answers `RList n=1` with `list-contains` true for that file's name, agreeing with the shell's own listing on both the count and the membership. So the entries are **basenames**, not paths, and containment on the basename decides existence; the runtime is `listDirectory` followed by `sort`. A new `Bin5` phase lists the install root's `bin/` and refuses with the reference's own message, which is one message because `exe_path`'s two internal notes reach a single `fail`.

**`-x` is not reproduced, and the difference is recorded rather than closed.** A listing decides existence and says nothing about the permission bit, so a present-but-unexecutable binary passes the port and fails the reference. That is narrower than the divergence it replaces, where the port ran a binary that was not there at all. `FS-STAT-1` is the row that would close it.

**Measured:** the cover goes from `COVER FAIL: 1 of 9 cell(s) diverged or missed their expectation`, that one being cell 6, to **`COVER PASS: 9 of 9 cell(s) agree (0 MISSING, 1 not run)`**, exit 0. Cell 6's two declared assertions `prior-stages-agree` and `exit-codes-agree` were previously reported under `ASSERTIONS NEVER REACHED`, having measured nothing; that section is now empty. Cell 7 is still opt-in behind `--slow` and was not run. Closes decision 1 of the restart record's three.
