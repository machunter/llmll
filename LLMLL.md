# LLMLL: Large Language Model Logical Language (v0.14.38)

**`llmll`** is a programming language designed specifically for AI-to-AI implementation under human direction. It prioritizes contract clarity, token efficiency, and ambiguity resolution over human readability.

> **Current version: v0.14.38.** See [`CHANGELOG.md`](CHANGELOG.md) for release notes and [`docs/compiler-team-roadmap.md`](docs/compiler-team-roadmap.md) for the schedule.
...
| `EApp` (map ops: `map-has`/`map-get`/`map-put`/`map-empty`, §13.12) | ✅ | ✅ | ✅ (two-array int-0/1 presence encoding: presence select / presence-gated value select / paired stores / const arrays) | ✅ (array class §5.3.3; key-presence as a PROVE call-site obligation; `map[int,int]` v1, incl. let-bound put/get pipelines; activation-gated) | ✅ | string/bool-valued maps, cross-call map contracts, non-pure map-returning bodies, whole-map `=` → contract-only, whole (exact-reflection rule) |
