...
> [!NOTE]
> **`int` arithmetic and the verifier (v0.10.8, INT-1).** The verifier reasons over Z3 mathematical integers (unbounded), but `int` lowers to Haskell `Int` (`Int64`) at codegen — the documented gap at §5.3.5. As of v0.10.8, body-faithful verified evidence whose function body contains LLMLL-level integer arithmetic over non-literal operands is marked `overflow_tainted: true` in the trust report, obligation report, and `.verified.json` sidecar, and the `--strict-verified-core` flag refuses tainted verified clauses. See §5.3.5 for the mechanism, the discharge paths (`?proof-required` + Leanstral; v0.11 INT-2 codegen switch making `int` lower to `Integer`; post-freeze `machine-int` under QF-BV), and the soundness reasoning. The `int` row above describes the codegen lowering only; the verifier-side semantics are at §0.1 (semantic foundation) and §5.3.5 (verification matrix + gap callout).
...
> [!IMPORTANT]
> **`overflow_tainted` marking (v0.10.8, INT-1).** When the verifier proves a contract whose function body contains LLMLL-level integer arithmetic over non-literal operands, the resulting body-faithful verified evidence is marked `overflow_tainted: true` in the trust report, the obligation-report trust channel, and the `.verified.json` sidecar. The `--strict-verified-core` flag refuses tainted verified clauses with a distinct diagnostic from the existing body-fallback refusal. The marking is purely syntactic — it does not consult refinement predicates that might witness bounds — and the trigger set is `EOp` / `EApp` applications of `+`, `-`, `*`, `/`, `mod`, `rem`, `^`, `**` whose operands are not all integer literals whose folded value fits `Int64`. Compile-time constant arithmetic like `(+ 40 2)` is cleared. The principled discharge paths for an overflow-tainted clause are: (i) wrap the post-condition in `?proof-required` and complete via Leanstral; (ii) wait for the v0.11 INT-2 codegen switch that makes `int` lower to unbounded `Integer` (after which the trigger set is empty on `int`); or (iii) once available post-freeze, opt into the bounded `machine-int` primitive under QF-BV verification per INT-3. The taint never propagates transitively across calls — it is per-function-body — because the call-site verification still proves the callee's post against the caller's pre under Z3's unbounded-integer semantics.
