# LLMLL: Large Language Model Logical Language (v0.14.7)

**`llmll`** is a programming language designed specifically for AI-to-AI implementation under human direction. It prioritizes contract clarity, token efficiency, and ambiguity resolution over human readability.

> **Current version: v0.14.7.** See [`CHANGELOG.md`](CHANGELOG.md) for release notes and [`docs/compiler-team-roadmap.md`](docs/compiler-team-roadmap.md) for the schedule.
...
| **Inductive properties** | **Designed, not shipped** | Lean 4 via Leanstral MCP | Scaffolding exists (`LeanTranslate.hs`, `MCPClient.hs`, `ProofCache.hs`), but it is **contract-only / body-decoupled** — it states the contract, not the body obligation, so it does not yet faithfully encode the theorem. Currently runs in **mock mode only** (`--leanstral-mock`). And `lean-lsp-mcp` is a *checker*, not a *prover*: a real integration is a rewrite, not a wiring (scoped in [`docs/design/leanstral-integration-scope.md`](docs/design/leanstral-integration-scope.md)). |
...
**Coverage:** `ELet` with alpha-renaming (shadowing-safe), `EIf` with path-sensitive constraint emission, `EApp` to contracted functions (assume-guarantee), a **two-arm sum `EMatch`** — `Result`, or a user ADT whose *both* arms are single-payload constructors with admissible-scalar payloads — **at any nesting depth** (under `let`/`if`), consuming the matched payload's declared refinement, an **admissible-sum constructor application** `(Ctor e)` (via the datatype theory §5.3.3), and all QF-LIA operators. An `EMatch` with more than two arms or a nullary arm, a constructor of a non-admissible (recursive) sum, `letrec` (own body), and non-linear expressions fall back conservatively to contract-only verification.
