# LLMLL: Large Language Model Logical Language (v0.13.2)

**`llmll`** is a programming language designed specifically for AI-to-AI implementation under human direction. It prioritizes contract clarity, token efficiency, and ambiguity resolution over human readability.

> **Current version: v0.13.2 (shipped).** Patch on the v0.13 line. **A refinement-aliased return now discharges (DEF-RET Unit 2).** A declared `-> A` whose alias `A ≜ (where [x:τ] p)` folds into the function's effective postcondition via `augmentContractPost` (the guarantee-side dual of `augmentContractPre`): the body-VC **proves** `p[result/x]` (`verified` when `p` is `Σ_auto` and the VC is SAFE; `refuted` when the body violates it; `erBodyFallback` when `p` is non-`Σ_auto` — the §3.4.5 firewall), and the refinement is **exported as a caller-assumable guarantee** (folded into the effective post in `buildContractEnv`, consumed via assume-guarantee; the composed tier rides the §5.3.4 meet). A bare `-> RetType` function (no explicit `post`) is now credited `verified`. This replaces the Unit-1 conservative fallback. **Soundness:** the staleness hash now covers the augmented contract, so editing a `-> RetType` annotation or redefining an alias predicate invalidates a stale verified sidecar (also closing the pre-existing param-side NIW hole); existing v0.13.1 sidecars are one-time stale on upgrade and regenerate on next verify. The optional `-> RetType` surface itself shipped in v0.13.1 (Unit 1). No schema change (`schemaVersion` `0.7.0`; `trust_report_version` `1.4.0`). 908 Haskell + 62 Python tests. See [`CHANGELOG.md`](CHANGELOG.md) for full release notes and [`docs/compiler-team-roadmap.md`](docs/compiler-team-roadmap.md) for the implementation schedule.
...
The annotation is **optional and checking-mode**, consistent with the synthesis-primary checker (§3.4.6): omit it and the return type is fully inferred (`mRet = Nothing`, byte-identical to prior behavior); declare it and the body is checked against `RetType`. A bare-hole body under a declared return records `HoleTyped RetType`, so a function-body hole reports its type in the checkout brief / obligation report (the `expected_return_type` field, §11.2) — previously absent on this canonical case because `def`/`def-shell` had no return surface. A concrete (non-hole) body is synthesized and unified against `RetType`, with a mismatch attributed to the function. A refinement-aliased return (`-> PositiveInt`) additionally joins the §3.4.1 introduction obligation `p[body/result]` to the body-VC set and — since DEF-RET Unit 2 (v0.13.2) — discharges it: the predicate folds into the effective post via `augmentContractPost` (the guarantee-side dual of `augmentContractPre`), so the body-VC proves it (`verified` / `refuted` / `erBodyFallback` per §5.3.3) and it is exported as a caller-assumable guarantee (assume-guarantee; tier rides the §5.3.4 meet); see §3.4.5 and §5.3.5. See [`docs/design/def-return-annotation-proposal.md`](docs/design/def-return-annotation-proposal.md) (Settled Rev 3).
