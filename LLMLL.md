# LLMLL: Large Language Model Logical Language (v0.13.10)

**`llmll`** is a programming language designed specifically for AI-to-AI implementation under human direction. It prioritizes contract clarity, token efficiency, and ambiguity resolution over human readability.

> **Current version: v0.13.10.** See [`CHANGELOG.md`](CHANGELOG.md) for release notes and [`docs/compiler-team-roadmap.md`](docs/compiler-team-roadmap.md) for the schedule.
...
- **Datatype class — complete (acyclic; shipped v0.13.9, COMP-4 a/c).** A constructor / selector over an *admissible* two-arm sum — one whose reachable type-closure is acyclic, decided by `admissibleDatatype` (`FixpointEmit.hs`) — is the SMT theory of recursive data types (Barrett–Shikanian–Tinelli, *An Abstract Decision Procedure for the Theory of Recursive Data Types*, CADE 2007: constructors, selectors, distinctness; decidable for the non-recursive fragment), combined with QF-LIA by **polite-theory combination** (Ranise–Ringeissen–Zarba, *Combining Data Structures with Nonstably Infinite Theories*, FroCoS 2005). `typeToSortA` lowers an admissible payload-sum binder to its native datatype sort; `bodyToPredM`/`exprToPred` reflect an `(Ctor e)` application to the constructor term. Nullary enums (int-tag, COMP-3c) and opaque-received sums (skolem-branch, d-elim/(b)) stay pure QF-LIA. A **recursive** datatype is firewalled and falls back — its constructors are rejected at the strict-core gate (`def-shell` required).
