# LLMLL: Large Language Model Logical Language (v0.14.44)

**`llmll`** is a programming language designed specifically for AI-to-AI implementation under human direction. It prioritizes contract clarity, token efficiency, and ambiguity resolution over human readability.

> **Current version: v0.14.44.** See [`CHANGELOG.md`](CHANGELOG.md) for release notes and [`docs/compiler-team-roadmap.md`](docs/compiler-team-roadmap.md) for the schedule.
...
| String-literal (dis)equality | string literals in `=`/`≠` (interned nullary `Str` constants) | **QF_EUF auto** (content-interned constants + ground pairwise-distinctness, `injectStrLitDistinct`; STRLIT Stage 1 — literal-length pinning is Stage 2) |
...
```
  Σ_auto  =  QF-LIA core  ∪  ( measure class | path-(a) )  ∪  ( datatype class | admissible sums )  ∪  ( array class | bytes[n], exact reflection )
...
| `EApp` (map ops: `map-has`/`map-get`/`map-put`/`map-empty`, §13.12) | ✅ | ✅ | ✅ (two-array int-0/1 presence encoding: presence select / presence-gated value select / paired stores / const arrays) | ✅ (array class §5.3.3; key-presence as a PROVE call-site obligation; `map[int,{int,bool}]` v1 — bool values via the int-0/1 bridge, each value read range-pinned to `{0,1}`; incl. let-bound put/get pipelines, read-modify-write bodies, cross-call assume-guarantee, map-returning results; activation-gated) | ✅ | string-valued maps, whole-map `=`, and a bool-get used outside `=`/`!=` → contract-only, whole (exact-reflection rule) |
...
Operations over the compound types `bytes[n]` and `map[k,v]` (§3.2). Reads carry **PROVE-polarity preconditions** — the caller owes the obligation (there is no `Result`-wrapped read form). Both families **discharge statically** in the array class of `Σ_auto` (§5.3.3): bytes index-in-bounds and value-range, and map **key-presence**, are solver-checked call-site obligations — an out-of-bounds read, a read without a presence proof, or a dropped update in a verified function is *refuted*, not merely asserted. Map discharge covers `map[int,int]` (int keys are the v1 gate; **int values** are the v1 reflected class), including read-modify-write bodies, cross-module assume-guarantee, and map-returning callee results; only string/bool-**valued** maps fall back whole (never partially). All preconditions are additionally runtime-asserted in generated code regardless of static discharge.
