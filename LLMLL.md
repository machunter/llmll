# LLMLL: Large Language Model Logical Language (v0.26.5)

**`llmll`** is a programming language designed specifically for AI-to-AI implementation under human direction. It prioritizes contract clarity, token efficiency, and ambiguity resolution over human readability.

> **Current version: v0.26.5.** See [`CHANGELOG.md`](CHANGELOG.md) for release notes and [`ROADMAP.md`](ROADMAP.md) for the roadmap.

> **For AI code generators:** Every section contains at least one complete, compilable example. When generating LLMLL code, you must use only the constructs defined in this document. If a required construct is missing, emit a named `?hole` and document the gap — do not invent syntax.

---

## 0. What Is Proved and What Is Not

A `verified` verdict means the solver decided a verification condition built from the function body, and that decision is complete only inside the signature `Σ_auto`: linear integer arithmetic without `*`, `/` or `mod`, the `string-length` and `list-length` measures, acyclic datatypes, and `bytes`/`map` arrays ([§5.3.3](#533-verification-scope)).
Inside `Σ_auto`, `llmll verify` returns SAFE or UNSAFE on a fixed condition, and an UNSAFE body is reported as `refuted`, with a JSON pointer to the failing body and the violated constraint; no counterexample input is printed.
Outside it no completeness is claimed: nonlinear arithmetic and `regex-match` are not auto-discharged, and the cryptographic builtins are opaque stubs whose callers are capped at `asserted` ([§13.11](#1311-cryptographic-operations)).
The verification matrix ([§5.3.5](#535-verification-matrix)) states, construct by construct, what reaches the solver and what each construct falls back to.
The soundness statement of record ([§3.4.3](#343-soundness-statement-of-record-tier-aware-path-a)) needs every obligation discharged body-faithfully, faithful codegen for the constructs involved, and no opaque primitive; values from `?delegate`, `?delegate-async` and `?scaffold` holes are outside it, and it is a written commitment, not a mechanized theorem.
The refinement surface deliberately has no refinement subtyping, dependent pattern matching, type-level computation, proof terms, sigma types or propositional equality ([§3.4.2](#342-non-goals-exhaustive)).
...
Every sentence in §0.1 and §1 ends with an empty anchor of the form `<a id="nc-011"></a>`, which renders as nothing and makes the sentence linkable as `#nc-011`. The identifier `NC-011` names a row in the norm-claim registry, [`scripts/norm-claims/registry.json`](scripts/norm-claims/registry.json), which records what stands under the sentence: a fixture under `scripts/doc-claims/`, a refute-crux suite, an open roadmap row, an explicit assumption, or the note that the sentence is informative. A CI gate, [`scripts/norm_claims_gate.py`](scripts/norm_claims_gate.py), checks the spec and the registry against each other; an identifier is assigned once and is never renumbered or reused. The registry's README gives the sentence rule and the five dispositions.

---

## 1. Core Philosophy

1. **Strict Immutability:** There are no variables, only constants.<a id="nc-009"></a> State is transformed, never mutated.<a id="nc-010"></a> Re-binding the same name in the same scope is a compile error.<a id="nc-011"></a> Shadowing in nested scopes (e.g., a `let` binding that reuses a parameter name) is permitted but discouraged — the verifier alpha-renames shadowed bindings internally.<a id="nc-012"></a>
2. **Hole-Driven Development:** Ambiguity is a first-class citizen represented by Holes (`?`).<a id="nc-013"></a> A program with holes can be analyzed and type-checked.<a id="nc-014"></a> `build` accepts it, and a hole that execution reaches aborts the program with `hole: <name>` (`emitHole`), so the program does no useful work until the holes are filled.<a id="nc-035"></a> Always prefer a typed hole over a hallucinated implementation.<a id="nc-015"></a>
3. **Typed Logic:** Every expression has a type.<a id="nc-016"></a> The type system prevents null pointer dereferences, type mismatches, and unguarded IO.<a id="nc-017"></a>
4. **Design by Contract with Stratified Verification:** Logic functions declare `pre` and `post` conditions as formal specifications.<a id="nc-018"></a> These contracts are the trust interface between agents.<a id="nc-019"></a> Verification is stratified: contracts in the decidable arithmetic fragment (QF-LIA) are verified at compile time via liquid-fixpoint / Z3; a function whose contract or body leaves that fragment is reported as `body-fallback` with a recorded `fallback_reason`, its contracts are enforced as runtime assertions, and an author may route the obligation to Lean with a `?proof-required` hole (§5.3.3).<a id="nc-020"></a> An interactive proof path (Lean 4 via Leanstral, `--leanstral`) ships as an experimental, opt-in demo that records `verified-lean`; the production Lean tier is designed and deferred (see §5.3.3).<a id="nc-021"></a> Each contract clause carries a *display level* — `verified`, `tested`, or `asserted` — so a caller can inspect trust without reading the implementation.<a id="nc-022"></a>
5. **Transitive Trust Propagation:** Trust levels propagate through call chains: no `verified` conclusion rests silently on an `asserted` assumption.<a id="nc-023"></a> A function's effective display level is the lattice meet of its own `post` level and the `post` levels of all transitively reachable callees; a callee's `pre` level does not lower it (§4.4.4, TRUST-PRE).<a id="nc-024"></a>
6. **Compositional Verification:** Verification extends beyond isolated functions.<a id="nc-025"></a> When a function calls a contracted callee, the verifier proves the caller satisfies the callee's precondition and assumes the callee's postcondition holds (assume-guarantee reasoning).<a id="nc-026"></a> This enables body-faithful verification across multi-function call chains without inlining.<a id="nc-027"></a> Recursive cycles are verified compositionally via the mutual-recursion assume-guarantee rule, not contract-only: at **partial** correctness by default, and at total correctness with a discharging `(decreases …)` clause (§4.2).<a id="nc-028"></a>
7. **Declared Effect Namespaces:** A module reaches no part of the system it has not imported, and the compiler rejects a `wasi.*` call whose namespace the calling module did not declare.<a id="nc-029"></a> Imports are non-transitive, so every module states its own reach.<a id="nc-030"></a> The `capability` clause on an import records the intended verb and target but is **not yet enforced** (§7); the checked property is namespace declaration, not least authority.<a id="nc-031"></a> Every side effect is modeled as a `Command` value returned from pure logic, never performed directly, and what performing it produced comes back as a `Response` (§9.7).<a id="nc-032"></a> See §7 for the sandbox implementation and the enforcement gap.<a id="nc-033"></a>
...
| String-literal (dis)equality + length | string literals in `=`/`≠` (interned nullary `Str` constants), and `string-length` on a literal | **QF_EUF + QF-LIA auto** (content-interned constants + ground pairwise-distinctness `injectStrLitDistinct`, plus each literal's code-point length pinned `strLen(strlit_s) = |s|` via `injectStrLitLen`; string *structure* stays out) |
...
| `def-shell` | `SDefShell` | None | `verified` (body-faithful, when the body stays in the fragment), otherwise `tested`, `asserted` |
...
The lattice is a chain. It has no element for *`verify` ran and the emitter refused this body*; that state is reported by the orthogonal `body_fallback` marker (§4.4.4).
...
Every pair is comparable, so no meet drops to `asserted` except through `asserted` itself. `verified` covers `tested` (`evidenceCovers (DLVerified _) _ = True`).

> **`contract-checked` is not a level.** The name denotes no tier, on two independent grounds. First, no producer can match its definition: the sentence "pre ⇒ post is valid, holds for all models of the contract pair" admits two readings, and one is false for any contract that constrains `result` while the other is vacuous. Second, `mkFnRecord` in `ProofArtifact.hs`, the LCF kernel of §5.7, **rejects** a positive tier carrying a `fallback_reason`, and a solver-checked-but-not-body-checked tier would be positive, so no acquisition path can mint such evidence.
>
> **What reads the name.** Two persisted formats accept it on the READ side only, because a fail-closed reader would break existing files: a `.verified.json` sidecar carrying it reads as `asserted` and prints one warning (the surrounding evidence record is kept), and a `proof-artifact.json` record carrying it reads as the non-positive `asserted` tier so it still replays. Nothing writes it. The `(trust f :level contract-checked)` surface is **rejected** at parse time. Design of record: [`docs/design/trust-cc-1-proposal.md`](docs/design/trust-cc-1-proposal.md).

> **A persisted record cannot claim a tier it carries no evidence for.** A `.verified.json` clause whose `display_level` is `verified` or `verified-lean` while its `body_faithful` flag is absent or false is **downgraded to `asserted` on read**, and the downgrade is unconditional: the `verified_hash` attests that the source has not drifted since the record was written, never that a proof happened, so a hash cannot supply the missing evidence. A record carrying the flag keeps the three staleness cases (absent hash, hash drift, no live definition). The rule holds on both channels a record reaches: the entry file's own sidecar, and an imported module's sidecar, where an unbacked tier could otherwise lift a caller through the callee meet (§4.4.3). A downgrade is not a verification failure; the run continues on the demoted value and re-proves.

> [!NOTE]
> **Epistemic status distinction.** `verified` provides **logical evidence** over the function body: the solver proved the body satisfies the contract for all well-typed inputs, so no counterexample exists within the modelled semantics. `tested` provides **statistical evidence** over a random sample of size N (default 100): the property was not falsified, but may fail on the N+1th input. These are categorically different kinds of evidence and should not be treated as interchangeable trust signals.

> [!NOTE]
> **Design divergence from Liquid Haskell.** LLMLL admits a statistical evidence channel (`tested`) into the trust-report partial order. This is a deliberate departure from Liquid Haskell (Vazou et al., *Refinement Types for Haskell*, POPL 2014), which restricts its refinement-display to logical evidence only. The rationale is that AI-agent-emitted code is often outside the QF-LIA fragment that admits liquid-fixpoint discharge, and an empirical channel — explicitly statistical, per the epistemic-status note above — gives the trust report something to surface for that majority. The chain keeps `tested` strictly below the proving tops, so a reader never mistakes statistical evidence for logical evidence: a function whose only evidence is `tested` never displays `verified`, and a `verified` caller is floored to `tested` by the meet over its transitive callees (§4.4.4, `NC-024`).
...
|------|---------------------|-------------|
| `--contracts=full` | All contracts (verified + tested + asserted) | `llmll test` |
...
- The function's own verification level (verified/tested/asserted) for pre and post clauses
...
Use `--trust-report --json` for machine-readable JSON output suitable for CI or downstream tooling. The JSON emit carries a `trust_report_version` field plus a fixed-arity `tier_profile` aggregate `{verified, proved, contract_checked, tested, tested_joint, asserted, no_contract}` over per-function effective tier classifications, intended for downstream tooling that needs a fixed-arity summary without scalarizing the lattice. `contract_checked` is a **structural zero**, since no level produces it: the key stays because it is listed in the schema's `required` array for both `TrustSummary` and `TierProfile`, so removing it would make every emitted report fail the published schema. The human-readable summary does not print it; see [`docs/llmll-trust-report.schema.json`](docs/llmll-trust-report.schema.json) for the full shape. A function entry may also carry an additive `assumed_facts` array, one row per control-tag fact its verdict rests on (`tag`, `builtin`, `arm`, `predicate`, `category`, `premise`; §9.7), absent outside a requesting module; the text render prints each row as an `≈ assumes` line.

**`termination_unverified` — the partiality marker.** Every function in a recursive call-graph cycle (a cyclic SCC over the whole-program call graph, entry plus cached modules) carries a per-entry `termination_unverified: true` flag, and the report gains a top-level `partial_fns` list of these names. Like `refuted` / `overflow_tainted`, it is an **orthogonal informational marker**, not a `DisplayLevel` element: it is **derived at report-build time** from the call graph — never persisted to the sidecar — so it is present even on a solver-less `--trust-report` render (unlike `refuted_fns`, which is verify-time only). It does not feed `evidenceMeet`, the effective level, `refutedClosure`, or strict-core admission; a recursive `def-shell` keeps whatever tier its body VC earned (typically `verified` post at partial correctness) and simply carries the flag. It marks that termination is unverified for the cycle, per the §4.2 partial-correctness caveat; strict descent under a discharging `(decreases …)` clause (§4.2) discharges it. `termination_unverified` arrived at `trust_report_version` `1.5.0`.

**`termination_assumed`: the callers of an undischarged cycle.** `partial_fns` lists cycle members only, and a function that calls a cycle is proved at partial correctness too. The report therefore also carries a top-level `termination_assumed_fns` list of `{name, via}` objects and a per-entry `termination_assumed: true` flag (only-on-true): every contracted function that reaches an undischarged cycle over the whole-program call graph (entry plus cached modules, names module-qualified), including the cycle members (each its own `via`) and callers in other modules. The text report lists the callers under "Proved only if a called function terminates". Like `termination_unverified` it is an informational marker and does not feed `evidenceMeet`, the effective level, or strict-core admission. It is the same set the `verify` headline names (§5.3). `termination_assumed_fns` arrived at `trust_report_version` `1.7.0`.

**`body_fallback` — the body-refusal marker.** A function whose body the emitter refused carries a per-entry `body_fallback` object with two fields: `cause`, one value of the closed `fallback_reason` vocabulary, and `constructs`, a closed label set naming what refused the body. In the text report it sits on the post line, as `post: asserted   [body_fallback: body-outside-fragment; match-wildcard-payload]`. It exists because `asserted` alone merges two different states: *`verify` ran and the emitter refused this body* and *nothing ever examined this function*. The lattice has no element for the first state, so the marker reports it.
...
- **It appears only where the emitter ran in the same invocation.** `--strict-verify` and `--proof-artifact` show it; a plain `llmll verify --trust-report` exits before the emitter runs and shows **no** marker. Its absence is therefore never a claim of proof. It is also **suppressed** for the causes `no-post` and `unfilled-hole`, because neither lost a proof goal: one has no postcondition to prove, the other has a scaffold body with nothing written to prove. The plain-`--trust-report` render states this in its own output: it prints that the report is sidecar-only and was not validated against a run of the VC emitter.

`body_fallback` arrived at `trust_report_version` `1.6.0` as an additive key.

**`open_spec_rows` — what is specified and not enforced.** The report carries a top-level `open_spec_rows` array naming the open `[SPEC]` roadmap rows whose surface *this program writes*, each with the row tag, the surface, and what is not enforced. In the text report it prints as a `Specified but not enforced` section. Today one row qualifies: a program that writes a `capability` clause on an import is told that the clause records the intended verb and target and is **not** enforced (`CAP-1-REAL`, `NC-031`), so the checked property is namespace declaration rather than least authority.

The mapping source is the `row`-disposition claims in `scripts/norm-claims/registry.json`, which is already where the project records "this normative sentence is answered by an open roadmap row rather than by a fixture". Like `termination_unverified` it is **derived from the live source**, so it is present on every report path including a solver-less render, and one line is emitted per row rather than per occurrence of the surface. It is disclosure only: it changes no tier, feeds no meet and gates no admission. Additive key at `trust_report_version` `1.6.0`.

**`unvouched_cdp_meet` — the decomposition-trust meet.** Under `--cdp`, each function's `discriminative_axis` carries `unvouched_cdp_meet`: the lattice meet (weakest) of contract-discriminative-power scores over the function's *unvouched* transitive-callee subtree (subtree contracts with a present `pre`/`post` clause lacking a `:source` anchor), scoring the quality of an invented `refine` decomposition — a cascade with one hollow invented sub-contract reads weak even when every node is `verified`. It is two-axis: a `quality_meet` (`hollow ⊏ scored ⊏ strong`, `null` when no member is measured, kept distinct from hollow) plus a `coverage` object exposing the measured/unmeasured split and the `:source`-anchored members excluded from the meet (`excluded_fns`, so a forged `:source` exclusion of a hollow spawn is visible). A contract-only cyclic-SCC member sets `floored_by_cycle` without collapsing the quality meet. Like `refuted` / `termination_unverified` it is an **orthogonal report-only marker**: it does not feed `evidenceMeet`, the effective level, or strict-core admission. A cooperative-author diagnostic — adversarial `:source`-forgery is caught by differential-implementation checking, not by this signal. `unvouched_cdp_meet` is an additive `discriminative_axis` field: it grew within the `trust_report_version` 1.x major without a bump, and first emits under `1.5.0`.

**Tier-profile pre/post split.** The trust-report JSON carries two top-level fields parallel to `tier_profile`: `tier_profile_pre` and `tier_profile_post`. Each classifies functions by their per-clause effective level (the clause's own evidence record meeting the transitive-callee effective level), rather than by the per-function meet of pre and post. A function with `pre = asserted` and `post = tested n` increments `tier_profile_pre.asserted` and `tier_profile_post.tested`, where the scalar `tier_profile.asserted` collapses both clauses via the lattice meet at §4.4.1. Downstream tooling that needs the post-side empirical signal (the repair-loop harness's `Cred(R)` predicate is the canonical consumer) reads `tier_profile_post`.
...
**Multiple clauses:** A contract side may be authored as several `(pre ...)` or `(post ...)` clauses, each carrying its own `:source`. The effective predicate is the left `and`-fold of the clauses in author order; every clause keeps its citation (`contractPreClauses` / `contractPostClauses`). The trust report surfaces them as `pre_sources` / `post_sources` arrays (author order); `.verified.json` evidence records carry them as `sources`. For the decomposition-trust vouched predicate (§4.4), a multi-clause side is vouched only when **every** clause carries `:source`.
...
> **Illustrative, not currently runnable.** `string-char-at` is a real, codegen-backed builtin, but `random-int`, `hex-encode`, and `random-bytes` are not: none of the three exists anywhere in the compiler. A call to `random-int` is rejected as an unknown function. These examples show the intended `gen` declaration syntax and are reserved names for future random-generation builtins, not code you can build today.
...
**Reading the headline.** `SAFE (liquid-fixpoint)` is the solver's verdict on the emitted constraint set, and a function that fell back from body-faithful verification contributes its postcondition as an assumption. The mark says how much was proved. `✅` means every function carrying a postcondition was proved body-faithfully, no such proof reaches a recursive cycle without a discharging `(decreases …)` measure, and none uses an imported contract that is not proved. Otherwise the line starts `⚠️` and reports `partial: k of n contracted functions proved; m assumed, not proved: <names>` (a fallback body, a non-linear skip, or a `?hole` under a postcondition), `nothing proved: no function carries a postcondition`, or `k of n contracted functions proved`, followed by one or both of two clauses. `j proved only if they terminate: <names>` names every proved function that reaches an undischarged recursive cycle in any module, including a non-recursive caller: its proof is partial correctness, since a call that never returns meets any postcondition. `i proved on unproved imported contracts: <names>` names every other proved function that reaches an imported contract whose level is below `verified` (the import fell back, was never verified, or changed since its sidecar was written). Each name carries `(via <function>)`, the cycle member or import it reaches; imported names are module-qualified, as in `use (via loop.spin)`. The exit code is 0 in every case; `--strict-verified-core` is the mode that fails on assumed functions and on callers of unproved imports. With `--json`, a SAFE result carries `all_proved`, `proved_count`, `contracted_count`, `assumed_fns`, `termination_assumed_fns` and `import_assumed_fns` (the last two are lists of `{name, via}`); `all_proved` is false when any of the three name lists is non-empty.
...
- **QF-LIA core — complete, and a proper subset of QF-LIA.** Quantifier-free linear integer arithmetic is decidable (NP-complete); Z3 is a complete decision procedure. The verifier operates under unbounded mathematical integers rather than machine words, so QF-LIA's decidability is the result that applies here and QF-BV's is not. **The shipped core is narrower than QF-LIA, and the gap is in the symbol set rather than in the theory.** The symbol set above omits `*`, `/` and `mod`, so a term QF-LIA admits is still refused when it is written with one of them: `(* 8 r)`, `(* 1 r)` and `(mod r 8)` are all linear, and each reports `nonlinear:*` or `nonlinear:mod` and routes to the fallback channel. **Completeness is not weakened by the omission.** A sub-theory of a decidable theory is decidable, and Z3 stays complete on the narrower set; what the omission costs is expressiveness. The `nonlinear:` label names the refused operator and is not a judgement that the particular term is nonlinear. The omission stands because no program in this repository needs the wider set, and not because the wider set is undecidable.
...
> [!IMPORTANT]
> **The production Lean verification path is deferred; an experimental `--leanstral` demo path ships.** SMT verification (Z3/liquid-fixpoint) produces `verified`; the opt-in, experimental `--leanstral` path produces **`verified-lean`** (`DLVerifiedLean`) — a *distinct* evidence kind, a **peer** of SMT `verified` (§5.3.4), kernel-checked by Lean 4 + Mathlib — for a **faithfully-translatable** obligation only (the demo class is nonlinear arithmetic). This is **not** the production tier: faithful translation across all escape classes remains deferred to the production tier (see [`docs/compiler-team-roadmap.md`](docs/compiler-team-roadmap.md)). No SMT-`verified` claim rests on Leanstral, and a `verified-lean` result requires a live kernel check against a local Lean 4 + Mathlib project — it fails closed without `LLMLL_LEANSTRAL_API_KEY` + that project, and the `sanitizeProof` guard rejects any degenerate (`sorry`/`admit`/empty) proof term. Obligations outside the demo's faithful-translation class — including general inductive properties — remain tracked as `asserted` with explicit `?proof-required` holes until the production tier ships.

#### 5.3.4 Body-Faithful Verification

The `.fq` emitter encodes function bodies as verification conditions for functions in the decidable `Σ_auto` fragment (QF-LIA + measure + acyclic-datatype + Bool, §5.3.3). For a function with postcondition Q, precondition P, and body B, the emitter generates constraints of the form:
...
| `EOp`/`EApp` (+, -, =, <, <=, >=, >, !=) | ✅ | ✅ | ✅ | ✅ | ✅ | — |
| `EOp`/`EApp` (*, /, mod) | ✅ | ✅ | ❌ | ❌ | ✅ | runtime + `?proof-required`; refused on the operator, not on linearity (§5.3.3) |
...
| `EMatch` n-arm admissible sum (mixed nullary/payload, nested, sequential) | ✅ | ✅ | ✅ | ✅ (n-ary int-tag chain; exhaustiveness type-checked) | ✅ | recursive/non-admissible payload → runtime; a requesting def's `Response` arm binder carries the declared builtin fact under a proved control tag (§9.7, control-tag facts), still QF-LIA |
...
> [!IMPORTANT]
> **A resolved `τ_ret` can put `result` on a name-decided range fact.** A `verified` verdict may rest on a ground range fact that the emitter injects for an array whose bytes-rootedness was decided from a variable-name suffix rather than from evidence, and return-type resolution can make `result` such an array. No program in this tree does so, and the disclosure does not rest on that count: it has to hold for every program a user writes, not for the files this repository happens to contain. See the `ARR-RANGE-NAME` row in [`docs/compiler-team-roadmap.md`](docs/compiler-team-roadmap.md). If that row closes, so that `bytesRootedArr` decides on evidence instead of on a default-true name test, this disclosure dissolves.
...
> **The verbs the parser names.** Nine verbs are recognised: `read-write`, `read`, `write`, `connect`, `serve`, `post`, `get-bytes`, `get` (the verb `wasi.http.get` documents) and `monotonic-read`; the parser tries `get-bytes` before `get`, longest match first, so both parse. Any other identifier, `exec`, `stdout` and `delete` among them, parses as a custom verb. `(capability get-bytes :deterministic true)` checks clean. As the warning above says, no verb is enforced.
...
> There is no `rust.*` namespace. The Haskell backend uses Tier 1 Hackage imports (e.g. `haskell.aeson`, `haskell.warp`) as direct replacements with no stub required.
...
> [!IMPORTANT]
> **Authority is checked at compile time, not here.** Two steps are **not implemented**, and the runtime performs neither. There is no runtime permission check and no `CapabilityError`: `CodegenHs` emits no capability code at all. There is no guarded mode and no interception of sensitive commands such as `wasi.fs.delete`. What the compiler does enforce is described in §7. This gap is tracked as `CAP-1-REAL` in [`docs/compiler-team-roadmap.md`](docs/compiler-team-roadmap.md).
...
> [!IMPORTANT]
> **`:on-done` is the canonical place to print end-of-game messages**, and it is the
...
**Control-tag facts.** A builtin's declared fact (§13.9) reaches a program through
...
Design: [`docs/design/resp-fact-proposal.md`](docs/design/resp-fact-proposal.md) (Rev 6).
...
**Obligation ids name their site, and the contract channel is not hole-only.** An obligation id is `oblig:<function>:<channel>:<site>:<fingerprint>`. The site is a **name, not a position**: a hole obligation's site is the hole's name, and a per-call-site precondition obligation's is the call's argument vector alpha-normalized against the enclosing function's parameters. Reordering a call moves its id with the call; renaming a parameter does not move it. A callee that is a builtin carries no surface argument vector, so those obligations keep the un-sited form. `contract_channel` and `trust_channel` are carried by `hole-obligation`, `contract-obligation` and `precondition-obligation`; on a precondition obligation the trust channel describes the **caller**, whose evidence the reader is judging. `type_channel` is carried by `hole-obligation` alone, because `expected_type` has no referent where there is no hole. A `termination-obligation` and a `branch-obligation` carry no channel, and a branch obligation instead emits `path_condition`, one structural entry holding that arm's own constructor guard over the scrutinee, and `postcondition_goal`, the enclosing function's post, as top-level keys. **The trust channel's `assumptions` array is populated**: each row is a `builtin-axiom` naming a sealed builtin, its assumed post and the stamp it rides, an `inherited-axiom` naming the callee it came from, or a `ground-fact` naming the family. A builtin axiom splits its post into tagged `conjuncts`, `reflection` for the conjunct that defines the encoding and `lemma` for a theorem about the operation. An inherited row whose callee predates the disclosure reports as unrecorded rather than as axiom-free (§13.12). An empty array means the function assumes none of the three. Across every kind `postcondition_goal` is the goal and `preconditions` is the assumption set, so a precondition obligation's goal is the **callee's** precondition instantiated with that call's own arguments, and its `preconditions` is the **caller's** own. Per-kind scope and the polarity rule: [`docs/design/oblig-0-spec.md`](docs/design/oblig-0-spec.md) §2.3 and §4.2.0.
...
> [!WARNING]
> **Parses on both surfaces; invariant *enforcement* not yet implemented.** `def-invariant` produces its own `SDefInvariant` AST node — a first-class node, not a reduction to `SDefLogic`. It parses from **both** JSON-AST and S-expression source (the S-expression form in `Parser.hs`), is type-checked, and registers as a logic predicate in the VC environment. What is **not** implemented is enforcement: Z3 verification, on AST merge, that declared invariants are preserved. A `def-invariant` in `.llmll` source is accepted but has no invariant-enforcement effect.
...
This is the S-expression form (§12); it parses and produces a real `SDefInvariant` node — though this particular example additionally references undefined helpers (`sum`, `map-values`, …), so it type-checks only once those are defined. The equivalent JSON-AST:
...
TRUST_LEVEL = "verified" | "tested" | "asserted" ;
              (* "contract-checked" is rejected at parse time (§4.4.1).        *)
...
(* The binding head is a pattern, not just an identifier.                 *)
...
| `wasi.fs.delete` | `string -> Command` | `(import wasi.fs (capability delete PATH))` | Delete file at path (**sensitive**; see the note below). Idempotent on a missing path. A path naming a **directory** delivers `RErr` naming `wasi.fs.rmdir` |
| `wasi.fs.rmdir` | `string -> Command` | `(import wasi.fs (capability delete PATH))` | Remove an **empty** directory at path (**sensitive**). Idempotent on a missing path. `RErr` when the directory is not empty, and `RErr` when the path is a file. **Recursive removal is deliberately absent**: there is no runtime capability check (§7), so a recursive delete would run under a root that is declared and unenforced |
...
rule of §9.7. Every fact rides `codegen_semantics_version` (§3.5), and the
...
> split by kind: **`wasi.fs.delete` on a directory delivers `RErr`** naming `wasi.fs.rmdir`.
> Answering `RNone` there would be the collapse this section keeps warning
...
> capability code runs (§7). Capsicum and WASI preview 1 resolve a path beneath a
...
> empty string for absence would make the two indistinguishable: a caller would read a missing
> value as a present empty one while the program still type-checks and verifies. **The conflation that remains is on the name side, not the value side.** An unset
...
| `sha1` | `bytes[20] → bytes[20]` | FIPS 180-4 (SHA-1) | Input is `bytes[20]`, output is 20-byte hash. **Lowers to the preamble's `sha1_hash` binding, not to `sha1`**: the two names differ. The declared input is narrower than FIPS 180-4, which admits any length. |
...
>
> **The two sections differ in what they deliver.** `wasi.fs.sha256` is backed by
...
> from it. Neither is fit for any security purpose.

> [!IMPORTANT]
> **Stub backends, and what the trust report actually records.** Distribution builds intended for production use **must** replace the preamble `sha1` / `hmac-sha1` stub in `CodegenHs.hs` with a verified crypto backend (`crypton` or `cryptohash-sha1`). The evidence model records this through the ordinary `asserted` tier and through nothing else: every function whose trust closure reaches `sha1` or `hmac-sha1` is capped at `asserted` (the lattice tier per §4.4.1), and that cap is real and emitted. **No separate stub-backend channel, field, or tier exists**; in particular, the trust report has no `asserted-with-stub-backend` annotation. One consequence is worth stating plainly: the report does not distinguish "asserted because the algorithm is opaque" from "asserted over a known-incorrect runtime implementation", so a consumer needing that distinction must read this section rather than the report. The symbols `sha1` and `hmac-sha1` keep their RFC 2104 / FIPS 180-4 contract names, and are not renamed to `sha1_stub`, because the contract is the standards specification; the stub status is an implementation defect.
...
**The sealed-builtin axiom set.** Four operations in this section reach the solver carrying an **assumed post** that no obligation discharges: `bytes-get`, `bytes-set`, `bytes-zero` and `map-get`. Each post equates the operation's result to the theory term it reflects to; `bytes-set` and `bytes-zero` additionally carry a length fact (`bytesLen(result) = bytesLen(b)` and `bytesLen(result) = n` respectively). These are **axioms about sealed builtins**. Each is valid because codegen emits the operation the term describes, so each rides the `codegen_semantics_version` stamp (§3.5) rather than a solver discharge. **The set is closed.** No other builtin contributes an assumed post, and adding one is a change to the trusted base rather than an implementation detail.

Three **ground-fact families** ride the same stamp and are disclosed beside them: `measure-nonneg` (`m(t) ≥ 0` for each occurring measure application), `byte-range` (`0 ≤ select(b,i) ≤ 255` for each bytes-rooted read) and `bool-value-range` (`0 ≤ select(m$val,k) ≤ 1` for each bool-valued map read). These are injected per occurring term rather than per operation, so the report names the families a function's constraints used rather than one row per fact.

Every such axiom is **disclosed rather than silent**. `llmll verify --trust-report` names each one on the function whose body VC assumed it, under the category `codegen-determined`, beside the `assumed_facts` rows that disclose the control-tag facts of §9.7. A function also **inherits** the axiom set of every function it calls, transitively; an inherited row names the function the axiom originated at, so a caller's justification surface names the sealed builtins its evidence rests on even when its own body uses none. A callee whose `.verified.json` predates the disclosure reports as **unrecorded** rather than as axiom-free, because the absence of a record is not evidence that no axiom was assumed. §5.4's anti-laundering invariant governs whether a record's own fields cohere; this disclosure governs what the record's tier rests on. The two are complementary and neither subsumes the other.

**Map keys are `{int, string}`** — a map operation at any other key type is a typechecker diagnostic on the *operation*; the `map[k,v]` type former itself is unrestricted.
...
**Trust tier.** The parser and serializer are sealed builtins backed by Haskell in the codegen preamble; their correctness is `asserted` (the §13.11 precedent). Unlike the §13.11 stubs they carry an oracle: the repository's build gate, at its execution stage, runs a round trip and a ten-case adversarial parse battery against hand-computed answers.
