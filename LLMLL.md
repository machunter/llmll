...
> **Note (v0.11 LT-INV):** Under `--grammar=core-inversion`, the strict-core equivalent of `def-logic` is `def`, which restricts the function body to a whitelist of verifiable constructs (linear-arithmetic `EOp`, `ELet`, `EIf`, `EApp` to admitted callees, `EMatch` on `Result` 2-arm). The permissive form is `def-shell` (no body restriction). Both parse only when `--grammar=core-inversion` is active. Under `--grammar=core-inversion`, `def-logic` and `letrec` are **not accepted**: the compiler emits a `core-grammar-violation` diagnostic and exits non-zero. Under the default `--grammar=legacy`, `def-logic` and `letrec` remain the canonical forms and `def`/`def-shell` are unavailable. See [`docs/getting-started.md §4.14`](docs/getting-started.md) for a worked example and `LLMLL.md §12` for the EBNF.
