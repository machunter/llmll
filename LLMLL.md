# LLMLL: Large Language Model Logical Language (v0.14.77)

**`llmll`** is a programming language designed specifically for AI-to-AI implementation under human direction. It prioritizes contract clarity, token efficiency, and ambiguity resolution over human readability.

> **Current version: v0.14.77.** See [`CHANGELOG.md`](CHANGELOG.md) for release notes and [`docs/compiler-team-roadmap.md`](docs/compiler-team-roadmap.md) for the schedule.
...
| `EApp` (bytes ops: `bytes-get`/`bytes-set`/`bytes-length`/`bytes-zero`, §13.12) | ✅ | ✅ | ✅ (reflected: `select`/`store` / ground `bytesLen(b)=n` / const array) | ✅ (array class §5.3.3; index-in-bounds + value-range as PROVE call-site obligations; activation-gated to functions mentioning a bytes op) | ✅ | whole-bytes `=`, mixed bytes+map obligations, any non-exact reflection → contract-only (exact-reflection rule) |
