...
A `Word`/`Letter` refinement (measure class) reaches `verified` (measure axiomatization shipped, commit `0a3c5c2`; caller-side proof for measure-refined params is tracked as roadmap F-NIW-2); a `BlockID` refinement (`regex-match`) stays `asserted` until the deferred string theory. The v0.12 measure catalog is **closed** at `{string-length, list-length}`; extension requires team consensus with a totality+range argument.
