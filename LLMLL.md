# LLMLL: Large Language Model Logical Language (v0.25.4)

**`llmll`** is a programming language designed specifically for AI-to-AI implementation under human direction. It prioritizes contract clarity, token efficiency, and ambiguity resolution over human readability.

> **Current version: v0.25.4.** See [`CHANGELOG.md`](CHANGELOG.md) for release notes and [`docs/compiler-team-roadmap.md`](docs/compiler-team-roadmap.md) for the schedule.
...
LLMLL's operational semantics are defined by the generated Haskell program.[NC-001] The compiler is the reference implementation: if the generated Haskell compiles and runs, that is the correct behavior.[NC-002] There is no separate formal semantics document.[NC-003] Verification conditions emitted by `llmll verify` are sound with respect to this generated-program semantics under mathematical-integer (unbounded) semantics — a verified contract holds for all well-typed inputs of the generated Haskell code, modulo the `Int64` overflow gap documented in §5.3.5.[NC-004] Compositional reasoning: when function `f` calls contracted function `g`, the verifier proves that `f` satisfies `g`'s precondition (obligation) and assumes `g`'s postcondition (hypothesis).[NC-005] This assume-guarantee composition is sound when both functions are independently verified.[NC-006] Functions in recursive call cycles are verified compositionally by the **mutual-recursion assume-guarantee rule** — each cycle member's body-VC assumes its callees' postconditions and proves its own body — which is sound at **partial** correctness (Hoare 1971; Apt, *Ten Years of Hoare's Logic*, TOPLAS 1981): a non-terminating recursion vacuously satisfies its postcondition.[NC-007] Termination is discharged only by a `(decreases …)` clause (§4.2, §5.3.3), which upgrades the cycle to total correctness; without one the verdict is partial.[NC-008] The trust report marks every member of an undischarged cycle `termination_unverified` (§4.4.4).[NC-034] The soundness statement above rests on components this specification does not itself verify, and they are named here so a reader can see the boundary.[NC-036] GHC compiles the generated program, so its code generation is taken as faithful to the Haskell the compiler emits.[NC-037] `CodegenHs` and the codegen preamble produce that program, so the verification conditions describe what they emit rather than the source text a reader sees.[NC-038] The VC emitter, liquid-fixpoint and Z3 decide every obligation, so a defect in any of them is a defect in the verdict.[NC-039] The `.verified.json` sidecar carries evidence between runs, so the hash that gates a persisted tier belongs to this base rather than sitting outside it (§4.4.1).[NC-040]

## 0.2 Normative Sentence Markers

Every sentence in §0.1 and §1 ends with a marker of the form `[NC-011]`. The identifier names a row in the norm-claim registry, [`scripts/norm-claims/registry.json`](scripts/norm-claims/registry.json), which records what stands under the sentence: a fixture under `scripts/doc-claims/`, a refute-crux suite, an open roadmap row, an explicit assumption, or the note that the sentence is informative. The `DRIFT-CT-3` gate checks the spec and the registry against each other in CI; an identifier is assigned once and is never renumbered or reused. The registry's README gives the sentence rule and the five dispositions.

---

## 1. Core Philosophy

1. **Strict Immutability:** There are no variables, only constants.[NC-009] State is transformed, never mutated.[NC-010] Re-binding the same name in the same scope is a compile error.[NC-011] Shadowing in nested scopes (e.g., a `let` binding that reuses a parameter name) is permitted but discouraged — the verifier alpha-renames shadowed bindings internally.[NC-012]
2. **Hole-Driven Development:** Ambiguity is a first-class citizen represented by Holes (`?`).[NC-013] A program with holes can be analyzed and type-checked.[NC-014] `build` accepts it, and a hole that execution reaches aborts the program with `hole: <name>` (`emitHole`), so the program does no useful work until the holes are filled.[NC-035] Always prefer a typed hole over a hallucinated implementation.[NC-015]
3. **Typed Logic:** Every expression has a type.[NC-016] The type system prevents null pointer dereferences, type mismatches, and unguarded IO.[NC-017]
4a. **Design by Contract with Stratified Verification:** Logic functions declare `pre` and `post` conditions as formal specifications.[NC-018] These contracts are the trust interface between agents.[NC-019] Verification is stratified: contracts in the decidable arithmetic fragment (QF-LIA) are verified at compile time via liquid-fixpoint / Z3; a function whose contract or body leaves that fragment is reported as `body-fallback` with a recorded `fallback_reason`, its contracts are enforced as runtime assertions, and an author may route the obligation to Lean with a `?proof-required` hole (§5.3.3).[NC-020] An interactive proof path (Lean 4 via Leanstral, `--leanstral`) ships as an experimental, opt-in demo that records `verified-lean`; the production Lean tier is designed and deferred (see §5.3.3).[NC-021] Each contract clause carries a *display level* — `verified`, `tested`, or `asserted` — so a caller can inspect trust without reading the implementation.[NC-022]
4b. **Transitive Trust Propagation:** Trust levels propagate through call chains: no `verified` conclusion rests silently on an `asserted` assumption.[NC-023] A function's effective display level is the lattice meet of its own `post` level and the `post` levels of all transitively reachable callees; a callee's `pre` level does not lower it (§4.4.4, TRUST-PRE).[NC-024]
5. **Compositional Verification:** Verification extends beyond isolated functions.[NC-025] When a function calls a contracted callee, the verifier proves the caller satisfies the callee's precondition and assumes the callee's postcondition holds (assume-guarantee reasoning).[NC-026] This enables body-faithful verification across multi-function call chains without inlining.[NC-027] Recursive cycles are verified compositionally via the mutual-recursion assume-guarantee rule, not contract-only: at **partial** correctness by default, and at total correctness with a discharging `(decreases …)` clause (§4.2).[NC-028]
6. **Declared Effect Namespaces:** A module reaches no part of the system it has not imported, and the compiler rejects a `wasi.*` call whose namespace the calling module did not declare.[NC-029] Imports are non-transitive, so every module states its own reach.[NC-030] The `capability` clause on an import records the intended verb and target but is **not yet enforced** (§7); the checked property is namespace declaration, not least authority.[NC-031] Every side effect is modeled as a `Command` value returned from pure logic, never performed directly, and what performing it produced comes back as a `Response` (§9.7).[NC-032] See §7 for the sandbox implementation and the enforcement gap.[NC-033]
...
The lattice was a **diamond** until v0.23.0. `contract-checked` sat in the middle beside `tested` and no component of the compiler ever assigned it; `TRUST-CC-1` retired it. The state it was named after is real and is now reported as the orthogonal `body_fallback` marker (§4.4.4).
...
Every remaining pair is comparable, so no meet drops to `asserted` except through `asserted` itself. The paragraph that stood here recorded the incomparability of `contract-checked` and `tested`, and it **retired with the element rather than moving**. Its stated purpose was to stop a `tested`-only function from reading as equivalent to a solver-checked one; once no solver-checked-but-not-body-checked level exists, `tested` has nothing left to be conflated with. Note also that `verified` has always covered `tested` (`evidenceCovers (DLVerified _) _ = True`), so those two were never the incomparable pair.

> **`contract-checked` was retired at v0.23.0 (`TRUST-CC-1`).** v0.22.1 measured that no shipped code path assigned the level and left the design question open; this settles it. Two independent grounds. First, the level could not be given a producer matching its own definition: the sentence "pre ⇒ post is valid, holds for all models of the contract pair" admits two readings, and one is false for any contract that constrains `result` while the other is vacuous. Second, `mkFnRecord` in `ProofArtifact.hs` — the LCF kernel of §5.7 — already **rejects** a positive tier carrying a `fallback_reason`, and it counted `contract-checked` as positive, so the acquisition path the old text described could never have minted evidence. The level had elimination rules and no introduction rule.
>
...
> The `verified` level carries a prover tag (e.g., `verified (liquid-fixpoint)`). This tag appears in `.verified.json` sidecars and `--trust-report` output but does not affect the surface grammar. The `(trust ...)` declaration accepts three keywords: `verified`, `tested`, `asserted`. A fourth, `contract-checked`, was accepted until v0.23.0 and is now **rejected** at parse time with an error naming `TRUST-CC-1`. Body-faithfulness is what a `verified` post rests on; a function whose body the emitter refused stays `asserted` and carries the `body_fallback` marker (§4.4.4).
...
Use `--trust-report --json` for machine-readable JSON output suitable for CI or downstream tooling. The JSON emit carries a `trust_report_version` field plus a fixed-arity `tier_profile` aggregate `{verified, proved, contract_checked, tested, tested_joint, asserted, no_contract}` over per-function effective tier classifications, intended for downstream tooling that needs a fixed-arity summary without scalarizing the lattice. `contract_checked` is retained as a **structural zero** after the v0.23.0 retirement (`TRUST-CC-1`): the key is listed in the schema's `required` array for both `TrustSummary` and `TierProfile`, so removing it would make every emitted report fail the published schema. The human-readable summary no longer prints its line — see [`docs/llmll-trust-report.schema.json`](docs/llmll-trust-report.schema.json) for the full shape. A function entry may also carry an additive `assumed_facts` array, one row per control-tag fact its verdict rests on (`tag`, `builtin`, `arm`, `predicate`, `category`, `premise`; §9.7), absent outside a requesting module; the text render prints each row as an `≈ assumes` line.
...
**`body_fallback` — the body-refusal marker.** A function whose body the emitter refused carries a per-entry `body_fallback` object with two fields: `cause`, one value of the closed `FALLBACK-REASON-CONST-1` vocabulary, and `constructs`, the closed `SHELL-FALLBACK-SILENT-1` label set naming what refused the body. In the text report it sits on the post line, as `post: asserted   [body_fallback: body-outside-fragment; match-wildcard-payload]`. It exists because `asserted` alone merges two different states: *`verify` ran and the emitter refused this body* and *nothing ever examined this function*. Before v0.23.0 the first state had a name in the lattice, `contract-checked`, that nothing assigned; the marker reports it instead (`TRUST-CC-1`).
...
> [!IMPORTANT]
> **A resolved `τ_ret` can put `result` on a name-decided range fact (`RET-RESOLVE`, v0.23.8).** A `verified` verdict may rest on a ground range fact that the emitter injects for an array whose bytes-rootedness was decided from a variable-name suffix rather than from evidence, and `RET-RESOLVE` can newly make `result` such an array. No program in this tree does so, and the disclosure does not rest on that count: it has to hold for every program a user writes, not for the files this repository happens to contain. See the `ARR-RANGE-NAME` row in [`docs/compiler-team-roadmap.md`](docs/compiler-team-roadmap.md). If that row closes, so that `bytesRootedArr` decides on evidence instead of on a default-true name test, this disclosure dissolves.
...
TRUST_LEVEL = "verified" | "tested" | "asserted" ;
              (* "contract-checked" was a fourth keyword until v0.23.0.        *)
              (* It is now rejected at parse time (TRUST-CC-1, §4.4.1).         *)
...
> [!IMPORTANT]
> **Stub backends, and what the trust report actually records.** Distribution builds intended for production use **must** replace the preamble `sha1` / `hmac-sha1` stub in `CodegenHs.hs` with a verified crypto backend (`crypton` or `cryptohash-sha1`). The evidence model records this through the ordinary `asserted` tier and through nothing else: every function whose trust closure reaches `sha1` or `hmac-sha1` is capped at `asserted` (the diamond-lattice tier per §4.4.1), and that cap is real and emitted. **No separate stub-backend channel, field, or tier exists.** A previous statement in this section, that the trust report annotates such dependencies as `asserted-with-stub-backend`, was never implemented; it is retracted, and the roadmap tracks the retraction as `CRYPTO-2`. One consequence is worth stating plainly: the report does not distinguish "asserted because the algorithm is opaque" from "asserted over a known-incorrect runtime implementation", so a consumer needing that distinction must read this section rather than the report. The symbols `sha1` and `hmac-sha1` keep their RFC 2104 / FIPS 180-4 contract names, and are not renamed to `sha1_stub`, because the contract is the standards specification; the stub status is an implementation defect, tracked as `CRYPTO-1`.
