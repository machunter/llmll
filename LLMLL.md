...
> **Note (v0.11 LT-INV):** Under `--grammar=core-inversion`, the strict-core equivalent of `def-logic` is `def`, which restricts the function body to a whitelist of verifiable constructs (linear-arithmetic `EOp`, `ELet`, `EIf`, `EApp` to admitted callees, `EMatch` on `Result` 2-arm). The permissive form is `def-shell` (no body restriction). Both parse only when `--grammar=core-inversion` is active. Under `--grammar=core-inversion`, `def-logic` and `letrec` are **not accepted**: the compiler emits a `core-grammar-violation` diagnostic and exits non-zero. The grammar default remains `GrammarLegacy` (rollback path 1, 2026-05-29; redesigned §8 gate experiment pending); pass `--grammar=core-inversion` explicitly to activate `def`/`def-shell`. Under `--grammar=legacy` (or without the flag), `def` and `def-shell` are unavailable. See [`docs/getting-started.md §4.14`](docs/getting-started.md) for a worked example and `LLMLL.md §12` for the EBNF.
...
(* ============================================================ *)
(* Core/shell grammar — GrammarCoreInversion opt-in only (v0.11 LT-INV; grammar default GrammarLegacy pending redesigned §8 gate, rollback path 1, 2026-05-29) *)
(* Pass --grammar=core-inversion to activate def/def-shell. No --grammar=legacy flag needed to avoid core-inversion. *)
(* ============================================================ *)
