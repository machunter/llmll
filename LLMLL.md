...
| `TCustom` alias predicate obligation at intro site (non-QF-LIA `p`) | ✅ | ✅ | ❌ | ❌ | ✅ | `erBodyFallback` → contract-only / `asserted` tier; **no** runtime assertion for a refinement-typed binding (`augmentContractPre` verifier-local — unlike a non-QF-LIA *contract* obligation, which does assert; §3.4.5) |
