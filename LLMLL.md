# LLMLL: Large Language Model Logical Language (v0.14.34)

**`llmll`** is a programming language designed specifically for AI-to-AI implementation under human direction. It prioritizes contract clarity, token efficiency, and ambiguity resolution over human readability.

> **Current version: v0.14.34.** See [`CHANGELOG.md`](CHANGELOG.md) for release notes and [`docs/compiler-team-roadmap.md`](docs/compiler-team-roadmap.md) for the schedule.
...
- **Array class — complete (`bytes[n]`, exact reflection).** The four bytes operations (§13.12) reflect into the quantifier-free extensional theory of arrays — `bytes-get`→`select`, `bytes-set`→`store`, `bytes-zero`→the const array, `bytes-length b`→the ground fact `bytesLen(b) = n` per binder — decidable per Stump–Barrett–Dill–Levitt (LICS 2001), with const arrays per de Moura–Bjørner's combinatory array logic (FMCAD 2009, Z3's native procedure), combined with QF-LIA by the same polite-combination citations as the datatype class. Byte-range facts (`0 ≤ select(…) ≤ 255`) are emitted ground per occurring read — the path-(a) discipline extended. The read/write index-in-bounds and value-range preconditions are PROVE-polarity call-site obligations. Emission is **activation-gated** (a function participates only if its contract or body mentions a bytes op) and governed by the **exact-reflection rule** (design record: `docs/design/data-scope-lever-a-arrays-proposal.md` §6.1): a body-faithful VC — hence a `refuted` verdict — is emitted only when every symbol in the obligation reflects exactly; whole-`bytes`/`map` `=` never reflects to array equality, and map operations are not yet reflected — both route to the fallback channel.
...
| `EApp` (map ops: `map-has`/`map-get`/`map-put`/`map-empty`, §13.12) | ✅ | ✅ | ❌ | ❌ | ✅ | contract-only + runtime-asserted preconditions (reflection is the data-scope track's next stage) |
...
Operations over the compound types `bytes[n]` and `map[k,v]` (§3.2). Reads carry **PROVE-polarity preconditions** — the caller owes the obligation (there is no `Result`-wrapped read form). The **bytes operations discharge statically**: they reflect into the array class of `Σ_auto` (§5.3.3), index-in-bounds and value-range become solver-checked call-site obligations, and an out-of-bounds read in a verified function is *refuted*, not merely asserted. The **map operations are not yet reflected** — their preconditions are enforced as runtime assertions (§5.3.4 backstop) and a contract mentioning them classifies out-of-fragment, routing through the §5.3.3 fallback. All preconditions are additionally runtime-asserted in generated code regardless of static discharge.
