...
What the compiler **proves** versus what is **trusted**, for the connected demo. (Unlike a crypto-bearing RFC demo, this scenario has *no* opaque/`asserted` core — every obligation lands in QF-LIA plus the non-recursive datatype theory, so the solver is the catcher throughout.)
...
**Trusted (TCB):** the LLMLL compiler, liquid-fixpoint, and z3, the same as any LLMLL `verify`. Nothing demo-specific is assumed.
