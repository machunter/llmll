...
**Scope honesty:**
- **Real enum states/events AND a real outcome sum.** `ConnState` and `Event` are real nullary-enum sum types, matched and compared as values (COMP-3c / COMP-3b-general — verified). The multi-outcome RESULT is a real payload-bearing sum, `PayOutcome` (`Paid(int)` / `Rejected(int)`), **constructed natively** (COMP-4 (a)/(c), v0.13.9): the post discharges by constructor equality and refutes by injectivity, into Z3's non-recursive datatype theory. No int sentinel remains — `Rejected(0)` is a first-class constructor value, distinct from every `Paid(n)`.
